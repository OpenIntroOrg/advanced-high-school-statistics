...
Chapter~\ref{inferenceForCategoricalData} \Cut{introduces}\Add{summarizes inference procedures for a single proportion and presents additional inference procedures for categorical data.}   inference in the setting of categorical data. We use these methods to answer questions like the following:
\begin{itemize}
...
\Add{
We will find that the methods we learned in the previous chapter are very useful in these settings. The logic and the steps for carrying out hypothesis tests and confidence intervals remain the same, regardless of the particular test.  In this chapter we will encounter a new distribution for handling one-way and two-tables, but the core ideas will remain the same.
}
...
\end{enumerate}
If these conditions are met, then the sampling distribution of $\hat{p}$ is nearly normal with mean $p$ and standard deviation
...
Typically we do not know the true proportion, $p$, so we substitute some value to check conditions and to estimate the standard error. For confidence intervals, usually $\hat{p}$ is used to check the success-failure condition and compute the standard error. 

\begin{termBox}{\tBoxTitle{When the SD cannot be calculated exactly}
When there is no known or hypothesized value for $p$, we substitute in the sample proportion $\hat{p}$.  This estimate for the SD is called the SE (Standard Error).
\begin{align*}
SE_{\hat{p}} = \sqrt{\frac{\hat{p}(1-\hat{p})}{n}}
\end{align*}}
\end{termBox}

For hypothesis tests, typically the null value -- that is, the proportion claimed in the null hypothesis -- is used in place of $p$. Examples are presented for each of these cases in Sections~\ref{confIntForPropSection} and~\ref{htForPropSection}.
...
We may want a confidence interval for the proportion of Americans who approve of the job the Supreme Court is doing. Our point estimate, based on a \Add{random }sample of size $n = 976$ from the NYTimes/CBS poll, is $\hat{p} = 0.44$. To use the general confidence interval formula from Section~\ref{aFrameworkForInference}, we must check the conditions to ensure that the sampling distribution of $\hat{p}$ is nearly normal. We also must determine the standard error of the estimate.
...
Estimate the standard error of $\hat{p}=0.44$ using Equation~\eqref{seOfPHat}. Because $p$ is unknown and the standard error is for a confidence interval, use $\hat{p}$ in place of $p$.\footnote{$SD = \sqrt{\frac{p(1-p)}{n}} \approx \sqrt{\frac{0.44(1-0.44)}{976}} = 0.016$}
\end{exercise}
}

\begin{termBox}{\tBoxTitle{Constructing a confidence interval for a proportion \color{red} CUT THIS BOX}\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for a proportion \color{red} ADD THIS BOX}\vspace{-1mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\begin{itemize}
\item point estimate = $\hat{p}$
\item critical value $Z^*$ = 1.96 for a 95\% CI; \newline otherwise $Z^*$ is found using the t-table at row $\infty$
\item SE = $\sqrt{\frac{\hat{p} - (1-\hat{p})}{n}}$
\end{itemize}
\item Evaluate the CI and \textbf{write in the form ( $\_$ , $\_$  )}.
\item \textbf{Interpret the interval}:  ``We are XX\% confident that the true proportion of [...] is between [...] and [...]."
\item State your \textbf{conclusion} to the original question.
\end{enumerate}}
\end{termBox}

\begin{example} 
{Based on a 90\% confidence interval for $p$, is there evidence that the proportion of Americans who approve of the job the Supreme Court is doing is less than 50\%?.  A random sample of size $n = 976$ from the NYTimes/CBS poll found a sample proportion $\hat{p} = 0.44$. }
\begin{description}
\item[$\bullet$] We will construct a 1 proportion Z interval.
\item[$\bullet$] We have a random sample and \newline $976(0.44) \ge 10 \text{ and } 976(1-0.44) \ge 10$
\item[$\bullet$] $0.44 \pm 1.645\sqrt{\frac{0.44(1-0.44)}{976}}$
\item[$\bullet$] (0.409, 0.471)
\item[$\bullet$] We are 90\% confident that true proportion of Americans who approve of the job the Supreme Court is doing is between 0.409 and 0.471.
\item[$\bullet$] Because the \emph{entire} interval is below 0.5, we have evidence that the true percent that approve is less than 50\%.
\end{description}
\index{data!supreme court|)}
\label{seOfPropOfAmericansJobApprovalOfSupremeCourt}
\end{example}

\begin{exercise}Estimate and interpret the SD of the sample proportion $\hat{p}$ in the previous example.\footnote{SD $\approx\sqrt{\frac{0.44(1-0.44)}{976}} = 0.016$.  The SD is estimated as 1.6\%, which means that, on average, the sample proportion from a random sample of size $n=976$ will be 1.6\% away from the true population proportion.  Because the sample size is large, the average error is relatively low.}
\end{exercise}
...
Because this is a simple random sample that includes fewer than 10\% of the population, the observations are independent. In a one-proportion hypothesis test, the success-failure condition is checked using the null proportion, $p_0=0.5$: $np_0 = n(1-p_0) = 500\times 0.5 = 250 \ge 10$. With these conditions verified, the normal model may be applied to $\hat{p}$.
...
\begin{termBox}{\tBoxTitle{Hypothesis test for a proportion \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for a proportion \color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\end{itemize}
\item Write the \textbf{hypotheses} in plain language, then set them up in mathematical notation.
\begin{itemize}
...
\end{itemize}
\item Calculate the \textbf{test statistic}.
$$\text{Z} = \frac{\text{point estimate} - \text{null value}}{\text{SE of estimate}}$$
\begin{itemize}
\item point estimate = $\hat{p}$
\item SE = $\sqrt{\frac{p_0(1-p_0)}{n}}$
\end{itemize}
\item Find the \textbf{p-value} and compare it to $\alpha$.
\item State the \textbf{conclusion}.  
\begin{itemize}
\item Reject H$_0$ and have evidence for the alternate hypothesis  \newline OR
\item Do not reject H$_0$ and do not evidence for the alternate hypothesis.
\end{itemize}
\end{enumerate}}
\end{termBox}

\begin{example}{In the last section we used a confidence interval to answer the question: ``is there evidence that the proportion of Americans who approve of the job the Supreme Court is doing is less than 50\%".  Use a test of hypothesis to answer this question.  Recall that a random sample of size n = 976 from the NYTimes/CBS poll found a sample proportion $\hat{p} = 0.44$.}
\begin{itemize}
\item[$\bullet$] We will carry out a 1 proportion Z test.
\item[$\bullet$] H$_0$: $p=0.5$\newline H$_A$: $p<0.5$
\item[$\bullet$] $\alpha = 0.05$
\item[$\bullet$] We have a random sample and $976(0.5) \ge 10$ and $976(1-0.5) \ge 10$.  Note that here we used the null proportion rather than the sample proportion to verify the condition.
\item[$\bullet$] $Z = \frac{0.44-0.5}{\sqrt{\frac{0.5(0.5)}{976}}} = -3.78$
\item[$\bullet$] p-value = 7.94x10$^{-5} < \alpha$
\item[$\bullet$] Reject H$_0$.  We have very strong evidence that the proportion of Americans who approve of the job the Supreme Court is doing is less than 50\%.
\end{itemize}
\end{example}

\subsection{Calculator: The 1 proportion Z test and CI}

\begin{termBox}{\tBoxTitle{TI calculator: Carrying out the 1 proportion Z test}Use \textbf{STAT, TESTS, 1-PropZTest}.
\begin{enumerate}
\item Choose STAT.
...
\begin{exercise}Use a calculator to evaluate the Z score and find the p-value for the question: ``is there evidence that the proportion of Americans who approve of the job the Supreme Court is doing is less than 50\%."  Recall that a random sample of size $n = 976$ from the NYTimes/CBS poll found a sample proportion $\hat{p} = 0.44$.\footnote{Let $p_0 = 0.5$. x needs to be the number of yes's, so x$ = 0.44(976)=492.44$, which must be rounded to 492.  Let n = 976, choose $<$ and Calculate.  $z=-3.777$ and $p=7.936E-5$, or 0.00007936.}  
\end{exercise}

\begin{termBox}{\tBoxTitle{TI calculator: Calculating the 1 proportion Z CI}Use \textbf{STAT, TESTS, 1-PropZInt}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
...
\begin{exercise}A newspaper collects a simple random sample of 500 likely voters in the district and estimates Toohey's support to be 52\%.  Use a calculator to evaluate a 90\% confidence interval for the proportion of Toohey's support. \footnote{x needs to be the number of yes's, so x$ = 0.52(500)=260$.  Let n = 500 and C-Level be 0.90.  Choose Calculate.  The interval is $(0.23866, 0.29413)$.}  
\end{exercise}
...
\Comment{rewrite since this preceeds means}
\Cut{
...
Planning a sample size before collecting data is \Cut{equally }important when estimating a proportion. For instance, if we are conducting a university survey to determine whether students support a \$200 per year increase in fees to pay for a new football stadium, how big of a sample is needed to be sure the margin of error is less than 0.04 using a 95\% confidence level?
...
\begin{align*}
ME = z^{\star}SD = 1.96\times \sqrt{\frac{p(1-p)}{n}} \leq 0.04
\end{align*}
...
In our investigations, we first identify a reasonable point estimate of $p_1 - p_2$ based on the sample. You may have already guessed its form: $\hat{p}_1 - \hat{p}_2$\index{point estimate!difference of proportions}. Next, \Cut{in each example we verify that the point estimate follows the normal model by checking certain conditions. Finally, we compute the estimate's standard error and apply our inferential framework.}\Add{we develop a formula for the standard deviation of $\hat{p}_1 - \hat{p}_2$.}


\subsection{Sampling distribution for the difference of two proportions}
...
\Cut{We must check two conditions before applying the normal model to $\hat{p}_1 - \hat{p}_2$.}\Add{In addition to the mean and the standard deviation of $\hat{p}_1 - \hat{p}_2$, we would like to the know the shape of its distribution.}  If the following conditions are met, the distribution of $\hat{p}_1 - \hat{p}_2$ will follow the normal model.  First, the sampling distribution for each sample proportion must be nearly normal, and secondly, the samples must be independent. Under these two conditions, the sampling distribution of $\hat{p}_1 - \hat{p}_2$ may be well approximated using the normal model.

\Comment{added the np rule explicitly}.

\begin{termBox}{\tBoxTitle{Conditions for the sampling distribution of $\hat{p}_1 - \hat{p}_2$ to be normal}
The difference $\hat{p}_1 - \hat{p}_2$ tends to follow a normal model when
\begin{itemize}
\setlength{\itemsep}{0mm}
\item each proportion separately follows a normal model, i.e. \newline $n_1p_1\ge 10$, $n_1(1-p_1)\ge 10$, $n_2p_2\ge 10$, and $n_2(1-p_2)\ge 10$ \newline and
...
\Comment{renamed subsection}
\subsection{Confidence Interval for $p_1 -p_2$}
%\subsection{Confidence intervals for $p_1 -p_2$}

In the setting of confidence intervals, the sample proportions are used \Add{in place of the population proportions }to verify the success-failure condition and also compute standard error, just as was the case with a single proportion.
...
First the conditions must be verified. Because each group is a simple random sample from less than 10\% of the population, the observations are independent, both within the samples and between the samples. The success-failure condition \Cut{also holds for each sample.}is verified as follows:  $771(0.47)\ge 10$, $771(1-0.47)\ge 10$, $732(0.34)\ge 10$, and $732(1-0.34)\ge 10$. Because all conditions are met, the normal model can be used for the point estimate of the difference in support, where $p_1$ corresponds to the original ordering and $p_2$ to the reversed ordering:
$$\hat{p}_{1} - \hat{p}_{2} = 0.47 - 0.34 = 0.13$$
The standard error may be computed from Equation~\eqref{seForDiffOfProp} using the sample proportions \Add{in place of the population proportions}:
...
\Comment{cut this exercise and example, because students only need to do hypothesis with p1=p2.}
\Cut{
...
\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for the difference of two proportions \color{red} ADD THIS BOX}\vspace{-1mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the CI} being used.
\begin{itemize}
...
\end{itemize}
\item Evaluate the CI and \textbf{write in the form ( $\_$ , $\_$ )}.
\item \textbf{Interpret the interval}:  ```We are XX\% confident that the true difference in the proportion of [...] is between [...] and [...]."
\item State your \textbf{conclusion} to the original question.
\end{enumerate}}
\end{termBox}

\begin{example}{A remote control car company is considering a new manufacturer for wheel gears. The new manufacturer would be more expensive but their higher quality gears are more reliable, resulting in happier customers and fewer warranty claims. However, management must be convinced that the more expensive gears are worth the conversion before they approve the switch.  The quality control engineer collects a sample of gears, examining 1000 gears from each company and finds that 899 gears pass inspection from the current supplier and 958 pass inspection from the prospective supplier. Using these data, construct a 95\% confidence interval for the differene in the proportion that pass inspection.}Let current supplier be 1 and prospective supplier be 2.
\begin{itemize}
\item[$\bullet$] We will carry out a 2 proportion Z Interval.
\item[$\bullet$] The samples are independent, but not necessarily random, so to proceed we must assume the gears are all independent; for this sample we will suppose this assumption is reasonable, but the engineer would be more knowledgeable as to whether this assumption is appropriate. 
\newline \newline $1000(\frac{899}{1000})\ge 10$, $1000(1 - \frac{899}{1000})\ge 10$ \newline \newline $1000(\frac{958}{1000})\ge 10$, $1000(1 - \frac{958}{1000})\ge 10$.
\item[$\bullet$] 95\% CI: $(0.899-0.958)\pm 1.96\sqrt{\frac{0.899(1-0.899)}{1000}+\frac{0.958(1-0.958)}{1000}}$
\item[$\bullet$] $(-0.0814,-0.0366)$
\item[$\bullet$] We are 95\% confident that the true difference in the proportion of current and prospective gears that pass inspection is between -0.0814 and -0.0366.
\item[$\bullet$] Because the entire interval is negative, we have evidence that a significantly greater proportion of prospective gears pass inspection.  The remote control car company should go with the new manufacturer.
\end{itemize}
\end{example}
...
\Comment{the second one is the formula on the ap formula sheet}
$$SD = \sqrt{p(1-p)}\sqrt{\frac{1}{n_c} + \frac{1}{n_n}}$$
...
\end{eqnarray*}
Here $x_1$ represents the number of successes in sample 1.  If it is unknown it can be computed as $\hat{p}_1n_1$. Similarly, $x_2$ represents the number of successes in sample 2. If it is unknown it can be computed as $\hat{p}_2n_2$.}
\end{termBox}

\Comment{edited to correspond to formula on ap formula sheet}
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for the difference of two proportions \color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
...
\item Write the \textbf{hypotheses} in plain language, then set them up in mathematical notation.
\begin{itemize}
\item H$_0: p_1 = p_2$ or  $p_1 - p_2 = 0$
\item H$_0: p_1 \ne \text{or} < \text{or} > p_2$
\end{itemize}
\item Identify the \textbf{significance level} $\alpha$.
\item Verify \textbf{conditions}.
\begin{itemize}
\item 2 independent random samples OR 2 randomly allocated treatments
\item calculate the pooled sample proportion $\hat{p}$ and verify \newline $n_1\hat{p}\geq 10$, $n_1(1-\hat{p})\geq 10$ \newline $n_2\hat{p}\geq 10$, $n_2(1-\hat{p})\geq 10$
\end{itemize}
\item Calculate the \textbf{test statistic}.
$$\text{Z} = \frac{\text{point estimate} - \text{null value}}{\text{SE of estimate}}$$
\begin{itemize}
\item point estimate = $\hat{p}_1-\hat{p}_2$
\item SE = $\sqrt{\hat{p}(1-\hat{p})}\sqrt{\frac{1}{n_1} + \frac{1}{n_2}}$
\end{itemize}
\item Find the \textbf{p-value} and compare it to $\alpha$.
\item State the \textbf{conclusion}.  
\begin{itemize}
\item Reject H$_0$ and have evidence for the alternate hypothesis OR
\item Do not reject H$_0$ and do not evidence for the alternate hypothesis.
\end{itemize}
\end{enumerate}}
\end{termBox}
...
\begin{itemize}
\item[$\bullet$]  We will conduct a 2 proportion Z test.
\item[$\bullet$] H$_0$: $p_1=p_2$ \newline H$_A: p_1 \ne p_2$
\item[$\bullet$] $\alpha=0.05$
\item[$\bullet$] There were two independent random samples.
...
\item[$\bullet$] $Z=\frac{(0.47-0.37) - 0}{\sqrt{0.407(1-0.407}\sqrt{\frac{1}{771}+\frac{1}{732}}}=5.1$
\item[$\bullet$] p-value $ =3.34\times 10^{-7} < \alpha$
\item[$\bullet$] Reject H$_0$.  There is evidence of a significant differences in the proportion of yes responses based on the wording of the question.
\end{itemize}
\end{example}

\subsection{Calculator: The 2 proportion Z test and CI}

\begin{termBox}{\tBoxTitle{TI calculator: Carrying out the 2 proportion Z test}Use \textbf{STAT, TESTS, 2-PropZTest}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
...
\text{z} &\quad  \text{Z statistic} \\
\text{p} &\quad  \text{p-value} \\
$\hat{p}_1$ &\quad  \text{the proportion of yes's in sample 1} \\
$\hat{p}_2$ &\quad  \text{the proportion of yes's in sample 2} \\
$\hat{p}$ &\quad  \text{the pooled sample proportion}
\end{tabular}
\end{enumerate}
}
\end{termBox}

\begin{termBox}{\tBoxTitle{TI calculator: Calculating the 2 proportion Z CI}Use \textbf{STAT, TESTS, 2-PropZInt}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
...
$\hat{p}_1$ &\quad  \text{the proportion of yes's in sample 1} \\
$\hat{p}_2$ &\quad  \text{the proportion of yes's in sample 2} \\
\text{n$_1$} &\quad  \text{the size of sample 1} \\
\text{n$_2$} &\quad  \text{the size of sample 2}
\end{tabular}
\end{enumerate}
}
\end{termBox}
...
\begin{exercise}{Use the data above and a calculator to find the Z score and p-value for one sided test with H$_A$: dogs with cancer are more likely to have been exposed to 2,4-D than dogs without cancer; pc - pn $>$ 0.}\footnote{Let x1 be 191 and n1 be $191+304$.  Let x2 be 300 and n2 be $300+641$.  Choose $>$ and Calculate.  $Z=2.55$ and p-value$=0.0055$.  Also the pooled proportion $\hat{p}=0.342$.}
\end{exercise}

\begin{exercise}{Use the data above and a calculator to find a 95\% confidence interval for the difference in proportion of dogs with cancer that have been exposed to 2,4-D versus not exposed to 2,4-D.}\footnote{Let x1 be 191 and n1 be $191+304$.  Let x2 be 300 and n2 be $300+641$. Let C-Level be 0.95 and Calculate.  The interval is $(0.01484, 0.11926)$.  Note that there is no value given for the pooled proportion since we do not pool for confidence intervals.}
\end{exercise}
...
\Add{
In this example we have four categories: white, black, hispanic and other.  The presence of four categories rather than one or two makes it more difficult to find a point estimate.  Our strategy will be to find a test statistic that measures the \emph{overall} deviation between the observed counts and the expected counts.  First we find the difference between the observed and expected counts for the four groups.  We want to find the difference between the observed count and the expected expected for each group.  
\begin{align*}
...
\text{observed - expected }&& 205-198 && 26-19.25 
	...
	&& 19-24.75 \\
\end{align*}
If we add up these differences, the positives will cancel out the negatives and we will not get an appropriate measure of the overall deviation between the observed counts and the expected counts.  For this reason, we square the differences.
\begin{align*}
&&White && Black && Hispanic&&Other \\
\text{(observed - expected)}^2 && (205-198)^2 && (26-19.25)^2 
	...
	&& (19-24.75)^2 \\
\end{align*}
We must divide each term by some number to standardize them.  To know whether the squared difference is large, we must compare it to what was expected.  If the expected was 5, a squared difference of 25 is very large; however, if the expected was 1,000,000, a squared difference of 25 is negligible.  For this reason, we divide each of the squared differences by the corresponding expected count.
\begin{align*}
&&White && Black && Hispanic&&Other \\
...
\end{align*}
Finally, to arrive at the overall measure of deviation between the observed counts and the expected counts, we sum up the terms.
\begin{align*}
\sum{\frac{\text{(observed - expected)}^2}{\text{expected}}}= \frac{(205-198)^2}{198} + \frac{(26-19.25)^2 }{19.25}
	+ \frac{(25-33)^2}{33}
	+ \frac{(19-24.75)^2}{24.75} 
\end{align*}
}
...
$X^2$\vspace{0.5mm}\\\footnotesize chi-square\\test statistic}\Cut{, which is the sum of the $Z^2$ values,} is generally used for these reasons. We can \Cut{also }write an equation for $X^2$ using the observed counts and expected counts:
\index{data!racial make-up of jury|)}
{\begin{align*}
X^2 &=
	\frac
...
\Comment{added statement about becoming normal to footnote, deleted ``We would see this trend continue if we examined distributions with even more larger degrees of freedom."}
...
\begin{example}{We rarely observe the \emph{exact} value in the table. For instance, Figure~\ref{chiSquareAreaAbove4Point3WithDF2} shows the upper tail of a chi-square distribution with 2 degrees of freedom. The \Add{lower }bound for this upper tail is at 4.3, which does not fall in Table~\ref{chiSquareProbabilityTableShort}. Find the approximate tail area.}
...
\Add{Using a calculator allows us to get more precise areas under the chi-square curve than we can get from the table alone.}

\begin{termBox}{\tBoxTitle{TI Calculator: Finding areas under the chi-square curve \color{red} ADD THIS BOX}Use the \textbf{$X^2$cdf} command to find areas under the chi-square curve.
\begin{enumerate}
...
\Comment{just listed solutions since students will use calculator}

\begin{example}{Figure~\ref{chiSquareAreaAbove5Point1WithDF5} shows an upper tail for a chi-square distribution with 5 degrees of freedom and a cutoff of 5.1. Find the tail area \Add{using a calculator}.}
Using $X^2$cdf(5.1, 1000, 5) gives 0.4038.
\end{example}

\begin{exercise}
Figure~\ref{chiSquareAreaAbove11Point7WithDF7} shows a cutoff of 11.7 on a chi-square distribution with 7 degrees of freedom. Find the area of the upper tail.\footnote{area = 0.1109}
\end{exercise}

\begin{exercise}
Figure~\ref{chiSquareAreaAbove10WithDF4} shows a cutoff of 10 on a chi-square distribution with 4 degrees of freedom. Find the area of the upper tail.\footnote{area = 0.4043}
\end{exercise}

\begin{exercise}
Figure~\ref{chiSquareAreaAbove9Point21WithDF3} shows a cutoff of 9.21 with a chi-square distribution with 3 df. Find the area of the upper tail.\footnote{area = 0.0266}
\end{exercise}
...
We determined that a large $X^2$ value would suggest strong evidence favoring the alternative hypothesis: that there was racial bias. However, we could not quantify what the chance was of observing such a large test statistic ($X^2=5.89$) if the null hypothesis actually was true. This is where the chi-square distribution becomes useful. If the null hypothesis was true and there was no racial bias, then $X^2$ would follow a chi-square distribution, with three degrees of freedom in this case. Under certain conditions, the statistic $X^2$ follows a chi-square distribution with $k-1$ degrees of freedom, where $k$ is the number of bins \Add{or categories of the variable}.
...
\begin{example}{If the null hypothesis is true, the test statistic $X^2=5.89$ would be closely associated with a chi-square distribution with three degrees of freedom. Using this distribution and test statistic, identify the p-value \Add{and state whether or not there is evidence of racial bias in the juror selection}.}
...
\Add{The test that we just carried out regarding jury selection is known as the \term{$X^2$ goodness of fit test}.  It is called ``goodness of ift" because we test whether or not the proposed or expected distribution is a good fit for the observed data. 
}
\Comment{changed title of term box}
...
\begin{tipBox}{\tipBoxTitle{Conditions for the chi-square test \color{red} CUT THIS BOX}
...
\begin{tipBox}{\tipBoxTitle{Conditions for the chi-square goodness of fit test \color{red} ADD THIS BOX}
...
\begin{termBox}{\tBoxTitle[]{Goodness of fit test for a one-way table.\color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
...
\end{itemize}
\item Write the \textbf{hypotheses} in plain language.  No mathematical notation is needed for this test. 
\begin{itemize}
...
\end{itemize}
\item Calculate the \textbf{test statistic} and \textbf{degrees of freedom}.
\begin{align*}
...
\Comment{I propose removing this whole section/example in favor of a simpler example}
...
\Comment{I added this calculator subsection}

\begin{termBox}{\tBoxTitle{TI calculator: Carrying out the chi-square goodness of fit test}Use \textbf{STAT, TESTS,$X^2$GOF-Test}.
\begin{enumerate}
...
\begin{example}Use the data above and a calculator to find the $X^2$ statistic, df, and p-value for chi-square goodness of fit test.\footnote{First enter the observed values into L1 and the expected values into L2. Use$X^2$GOF-Test. $X^2=15.08$, $df=6$, p-value$=0.0196$.  }
\end{example}

%__________________
\section{Testing for homogeneity and independence in two-way tables}
\label{twoWayTablesAndChiSquare}
\Comment{renamed section to include test of homogeneity}
...
In this experiment, the explanatory variable is the search algorithm. However, an outcome variable is also needed. This outcome variable should somehow reflect whether the search results align with the user's interests. One possible way to quantify this is to determine whether (1)~\Add{there was ``No new search": }the user clicked one of the links provided and did not try a new search, or (2)~\Add{there was a ``New search": }the user performed a related search. Under scenario~(1), we might think that the user was satisfied with the search results. Under scenario~(2), the search results probably were not relevant, so the user tried a second search.
...
\Comment{cut last sentence of tip box "That is, are they dependent (versus independent?)"}
...
\begin{description}
\setlength{\itemsep}{0mm}
\item[Mutliple random samples or randomly allocated treatments. ]Data collected by multiple independent random samples or multiple randomlly allocated treatments.  Data must be able to be organized into a two-way table.   
...
\Comment{separated what follows into separate subsection}
...
\begin{example}{\index{data!approval ratings|(}Table~\ref{pewResearchPollOnApprovalRatingsForChiSquareSectionExampleAndExercises} summarizes the results of a Pew Research poll.\footnote{See the Pew Research website: {\scriptsize\href{http://www.people-press.org/2012/03/14/romney-leads-gop-contest-trails-in-matchup-with-obama/}{www.people-press.org/2012/03/14/romney-leads-gop-contest-trails-in-matchup-with-obama}}. The counts in Table~\ref{pewResearchPollOnApprovalRatingsForChiSquareSectionExampleAndExercises} are approximate.} We would like to determine if \Cut{there are actually differences in the approval ratings of Barack Obama, Democrats in Congress, and Republicans in Congress. } \Add{three groups and approval ratings are associated.  }What are appropriate hypotheses for such a test?}\label{hypothesisTestSetupForPewResearchPollOnApprovalRatingsForChiSquareSection}
\begin{itemize}
\Cut{
...
\Add{
\item[$H_0$:] The three groups and approval ratings are independent.
\item[$H_A$:] The three groups and approval ratings are dependent.}
\end{itemize}
\end{example}

\Comment{slightly reformatted table}

\begin{table}
\centering
\begin{tabular}{ll ccc ll}
& &  \raisebox{-1.7ex}[0pt]{Obama}  &  \raisebox{-1ex}[0pt]{Congressional}  & \raisebox{-1ex}[0pt]{Congressional}   \\
 & \hspace{1mm} &  & Democrats & Republicans & \hspace{1mm} & Total \\
\hline
...
\begin{termBox}{\tBoxTitle[]{$X^2$ test of homogeneity \color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
\item $X^2$ test of homegeneity
\end{itemize}
\item Write the \textbf{hypotheses} in plain language.  No mathematical notation is needed for this test. 
\begin{itemize}
...
\item all expected counts $\ge 5$ (calculate and recorded expected counts)
\end{itemize}
\item Calculate the \textbf{test statistic} and \textbf{degrees of freedom}.
\begin{align*}
X^2 &= \sum{\frac{\text{(observed counts - expected counts)}^2}{\text{expected counts}}} \\
...
\begin{termBox}{\tBoxTitle[]{$X^2$ test of independence \color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
...
According to the problem, there was one random sample taken.  Two variables were recorded on the respondents: gender and response to the question regarding privacy settings.  Because there was one random sample rather than two independent random samples, we carry out a test of independence.  
\begin{itemize}
\item[$\bullet$]  $X^2$ test of independence
\item[$\bullet$]  H$_0$: gender and knowing how to adjust Facebook privacy settings are independent
\newline H$_A$: gender and knowing how to adjust Facebook privacy settings are dependent
\item[$\bullet$]  $\alpha=0.1$
\item[$\bullet$]  The problem states that there was a random sample
...
\item[$\bullet$] $X^2 = 3.13$
\newline $df = 2$
\item[$\bullet$] p-value$ = 0.209 > \alpha$
\item[$\bullet$] Do not reject H$_0$.  We do not have evidence that gender and knowing how to adjust Facebook privacy settings are dependent.
\end{itemize}
\end{example}


\subsection{Calculator: chi-square test for two-way tables}
\Comment{added this calculator subsection}
...
\begin{termBox}{\tBoxTitle{TI Calculator: The chi-square test of homogeneity and independence}Use \textbf{STAT, TESTS, $X^2$-Test}.
\begin{enumerate}
...
\begin{exercise}Use the table above and a calculator to find the expected values and the $X^2$ statistic, $df$, and p-value for the corresponding test.\footnote{Enter the $2\times 3$ table into MATRIX A.  Select the $X^2$-Test.  $X^2=106.4$, p-value$=8.06\times 10^{-24}\approx 0$, and $df=2$.  Editing MATRIX B, gives the following expected values. 
\begin{tabular}{l ccc}
...
\end{exercise}
\Comment{deleted section ``Small sample hypothesis testing for a proportion" and ``Hypothesis testing for two proportions"}
