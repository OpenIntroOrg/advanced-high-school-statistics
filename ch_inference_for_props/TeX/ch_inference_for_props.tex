...
\Comment{Leah: I'd favor keeping this box, since conditions are very important and they could use the reminder.  The TIP box below the example I would omit, since that same TIP box exists elsewhere.}
...
The data are from a simple random sample, so the independence condition is satisfied.  \Add{To check the success/failure condition we want to check that $np$ and $n(1-p)$ are at least 10.  However, p is unknown.  Therefore, we will use the sample proportion $\hat{p}$ to check this condition. }\Cut{To check that $np$ and $n(1-p)$ are both at least 10, we will use the sample proportion in place of the population proportion:}
\begin{align*}
...
\Comment{I added the formula before the numbers for SE}
...
\item[(b)] \Add{State the alternate hypothesis.  }What value we should use as the null value, $p_{0}$?
...
(c) The calculations in a hypothesis test for a proportion assume the value $p_0$ for the unknown $p$, so we use $p_{0}$ \Add{rather than $\hat{p}$} when verifying the hypothesis test conditions:
\begin{align*}
...
\Comment{I added the formula in addition to the numbers in the example below}
...
\Comment{changed the formatting of equation so students can focus on the important part and what to solve for}
\Comment{did we ever define margin of error?}
\Cut{$ME = 1.96 \times SE = 1.96\times \sqrt{\frac{p(1-p)}{n}} \leq 0.04$}
...
For a 95\% confidence level, the value $z^{\star}$ corresponds to 1.96.\Cut{, and we can write the margin of error expression as follows:}  \Add{The margin of error is the part that follows the +/- in the confidence interval.  It is half the total width of the confidence interval.  We want:}
\begin{align*}
ME \leq 0.04 \\
...
\Comment{This box format was different than all the others, so I made it look like the others.}
...
\item Calculate the test statistic: $\text{Z} = \frac{\text{point estimate} - \text{null value}}{\text{SE of estimate}}$
  \begin{itemize}
  \item  the point estimate is: $\hat{p}_1 - \hat{p}_2$ and the SE = $\sqrt{\hat{p}(1-\hat{p})}\sqrt{\frac{1}{n_1} + \frac{1}{n_2}}$
  \end{itemize}
\item Find the p-value and compare it to $\alpha$ to determine whether to reject or not reject $H_0$.
\item Write the conclusion in the context of the question.
\end{enumerate}}
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for the difference of two proportions\color{red} CUT THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\begin{termBox}{\tBoxTitle{TI Calculator: Chi-square test of homogeneity and independence}
