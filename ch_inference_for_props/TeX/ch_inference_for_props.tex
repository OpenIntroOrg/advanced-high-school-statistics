\begin{chapterpage}{Inference for categorical data:  proportions}
 ...
 \chaptersection{distributionphat}
  \chaptersection{singleProportionCI}
  \chaptersection{singleProportionTest}
\chaptersection{distributionofdifference}
  \chaptersection{differenceOfTwoProportionsCI}
 \chaptersection{differenceOfTwoProportionsTest}
...
\end{nexercise}
\end{exercisewrap}
\footnotetext{Intuitively we would use the sample standard deviation $s$ as a point estimator for $\sigma$.  Based on this sample, the point estimate for $\sigma$ is $s = $13,130.}
...
When these conditions are met, we can use what is called a \termni{one-sample Z-interval for p}, where p is a population proportion.
...
When working with proportions, we can say that the p-value is the probability of getting a sample proportion as far from or farther from the null proportion in the direction of $H_A$ if the null hypothesis is true.  In general, we calculate and interpret a p-value as follows:
...
{0.33}{technologyInferenceProps}{desmos2PropCI}}\hspace{5mm}
...
{0.33}{technologyInferenceProps}{desmos2PropTest}}
\end{center}
\end{enumerate}
...
\begin{figure}[h]
\centering
\Figures[Observed counts]{0.45}{gsearch}{gsearch_mosaic2}
\Figures[]
{0.45}{gsearch}{gsearchSegBar}
