...
%\CommentA{Renamed section and added this subsection header}
...
\Cut{According to a New York Times / CBS News poll in June 2012, only about 44\% of the American public approves of the job the Supreme Court is doing.\footnote{\href{http://www.nytimes.com/2012/06/08/us/politics/44-percent-of-americans-approve-of-supreme-court-in-new-poll.html}{\scriptsize nytimes.com/2012/06/08/us/politics/44-percent-of-americans-approve-of-supreme-court-in-new-poll.html}} This poll included responses of 976 adults.}

\Add{The distribution of a sample proportion was introduced in Section~\ref{distributionphat}.  This theoretical distribution tells us about the values we can expect for a sample proportion from a given population.  The details of the distribution are presented below.}

\Comment{L : added symbols}  
...
\Comment{L: I'm not sure that this box makes sense here, should we omit it here?}

\begin{termBox}{\tBoxTitle{Computing the standard error of $\hat{p}$}
...
\Comment{Added everything before the termbox.}
...
\begin{example}{Construct a 90\% confidence interval for $p$, the proportion of Americans who approve of the job the Supreme Court is doing.  Recall the sample proportion, based on a sample of size 976, was 44\%.}\label{90CIForJobSupremeCourtDoingExample}
...
\Comment{Added everything before the term box.}
...
In the last chapter, we saw that the general form of the test statistic for a test of hypothesis has the form:
\begin{align*}
...
\Add{We can use a calculator to compute the left and right endpoint of a confidence interval or to evaluate the test statistic and the p-value.  Remember to show work and first substitute in all numbers before using the calculator.}
...
\Comment{it seems intro stat books don't use the pooled proportion when checking hypotheses for the two proportion z test.  my understanding was that if we assume p1=p2, that is not that same as assuming that they are equal to some null value.}
...
\Comment{de-italicized white, black, hispanic, other}
\begin{align*}
...
$X^2$\vspace{0.5mm}\\\footnotesize chi-square\\test statistic} is generally used for these reasons. We can write an equation for $X^2$ using the observed counts and expected counts:
\index{data!racial make-up of jury|)}
{\begin{align*}
X^2 &=
	\frac
...
\Comment{David, can you reorder these so that they are in order by degrees of freedom, to further illustrate how the curve changes as degrees of freedom gets larger?  Maybe not use repeats and end with a larger value for degrees of freedom so we can see it stretching to the right.  Maybe use the same left endpoint for each?}
