...
\sectionintro{
\Add{
\noindent In this section, we will apply the inferential procedures introduced in Chapter~\ref{ch_inference_foundations} to the context of a single proportion and we will explore how to do sample~size calculations for data collection purposes.  We will answer questions such as the following:
\begin{itemize}
...
\begin{enumerate}
\item Verify whether or not the conditions for inference on a proportion using a normal distribution are met.
...
\sectionintro{
\Add{
\noindent We often wish to compare to groups to each other.  In this section we will answer the following questions: 
\begin{itemize}
...
%%
\subsection[Confidence Interval for $p_1 -p_2$]{Confidence Interval for \pmb{$p_1 -p_2$}}
...
\item \term{$\chi^2$ tests for a one-way table}.  When there is one sample and we are comparing the distribution of a categorical variable to a specified or population distribution, e.g. using sample values to determine if a machine is producing M\&M's with the specified distribution of color, the hypotheses can often be written as:
\begin{itemize}
\item[] $H_0$: The distribution of [...] matches the specified or population distribution. 
\item[] $H_A$: The distribution of [...] doesn't match the specified or population distribution. 
\end{itemize}
...
\begin{exercisewrap}\begin{nexercise}
Use Table~\ref{pewResearchPollOnApprovalRatingsForChiSquareSectionExampleAndExercises}, reproduced below, and a calculator to find the expected values and the $\chi^2$-statistic, $df$, and p-value for the chi-square test for independence.\footnotemark
