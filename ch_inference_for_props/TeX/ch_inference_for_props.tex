...
\begin{onebox}{Conditions for the sampling distribution of \pmb{$\hat{{p}}$} being nearly normal}
...
\begin{itemize} 
\item[] To estimate a single proportion we use a \term{1-proportion Z-interval}.
\end{itemize}
...
\begin{itemize}
\item[] To test hypotheses about a single proportion we use a \term{1-proportion Z-test}.
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution of $\hat{p}$ to be nearly normal, assuming $H_0$: $p=p_0$ is true.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
\item[] 1. Independence:  Data come from a random sample or random process.  When sampling 
\item[] \quad \ without replacement, check that sample size is less than 10\% of the population size.
...
\item We use the sample proportion $\hat{p}$ as the \emph{point estimate} for the unknown population proportion $p$. The sampling distribution of $\hat{p}$ is approximately normal when the success-failure condition is met and the observations are independent.  When the sampling distribution of $\hat{p}$ is normal, the standardized test statistic also follows a \term{normal} distribution.
...
\begin{itemize}
\item[] To estimate a difference of proportions we choose the \term{2-proportion Z-interval}.
\end{itemize}
...
\end{itemize}
 \inferencestep{Choose} Choose the correct test procedure and identify it by name.\vspace{-1mm}
\begin{itemize}
\item[] To test hypotheses about a difference of proportions we use a \term{2-proportion Z-test}.  
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution of $\hat{p}_1-\hat{p}_2$ to be nearly normal, assuming $H_0$: $p_1=p_2$ is true.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
\item[] 1.  Independence:  Data come from 2 independent random samples or from a randomized 
\item[] \quad \ experiment with two treatments.  When sampling without replacement, check that the 
...
\item Test $H_0$: $p_1-p_2=0$ at the $\alpha$ significance level using a \term{2-proportion Z-test}.
\end{itemize}
...
\inferencestep{Choose} Choose the correct test procedure and identify it by name.\vspace{-1mm}
\begin{itemize}
\item[]  Here we use a \termsub{$\pmb{\chi^2}$ goodness of fit test}{chi-square goodness of fit test@$\chi^2$ goodness of fit test}.
\end{itemize}
...
\item While a normal distribution is defined by its mean and standard deviation, the chi-square distribution is defined by just one parameter called \termsub{degrees of freedom}.  
...
\item When there is a random sample and all of the expected counts are at least 5, the $\chi^2$-statistic follows a \term{chi-square distribution} with degrees of freedom equal to number of \mbox{categories $-$ 1.}
...
\begin{onebox}{$\mathbf{\chi^2}$ test for homogeneity}
...
\inferencestep{Choose} Choose the correct test procedure and identify it by name.\vspace{-1mm}
\begin{itemize}
\item[]  Here we use a \termsub{$\pmb{\chi^2}$ test for homogeneity}{chi-square test for homogeneity@$\chi^2$ test for homogeneity}.
\end{itemize}
\inferencestep{Check} Check that the test statistic follows a chi-square distribution.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\D{\newpage}
\Comment{check placement of conditions box}
...
\inferencestep{Choose} Choose the correct test procedure and identify it by name.\vspace{-1mm}
\begin{itemize}
\item[]  Here we use a \termsub{$\pmb{\chi^2}$ test for independence}{chi-square test for independence@$\chi^2$ test for independence}.
\end{itemize}
\inferencestep{Check} Check that the test statistic follows a chi-square distribution.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\item[\inferencestep{Choose}] Two variables were recorded on the respondents: gender and response to the question regarding privacy settings.  We want to know if these variables are associated / dependent, so we will carry out a chi-square test for independence.  
\item[\inferencestep{Check}] According to the problem, there was one random sample taken.  We note that the population of Facebook users is much larger than 10 times the sample size of 806, so independence is reasonable.  All values in the table of expected counts are $\ge$ 5.
...
\end{nexercise}
\end{exercisewrap}
\footnotetext{The p-value in this test corresponds to the area to the right of $\chi^2 = 3.13$ under the chi-square curve with 2 degrees of freedom.  Assuming the probability model is true and assuming the null hypothesis is true, i.e. that gender and response really are independent, there is a 0.209 probability of getting a $\chi^2$-statistic as large or larger than 3.13.  Equivalently, it is the probability of our observed counts being this different from the expected counts, relative to the expected counts, if the null is true and the model holds.  Because the p-value is large, we do not reject the null.}
...
\item[] $H_A$:  The distribution of [...] is not the same for each population/treatment.
\end{itemize}
\item[] We test these hypotheses at the $\alpha$ significance level using a \termsub{$\pmb{\chi^2}$ test for homogeneity}{chi-square test for homogeneity@$\chi^2$ test for homogeneity}.
...
\item[]  $H_A$:  [variable 1] and [variable 2] are dependent.
\end{itemize}
\item[] We test these hypotheses at the $\alpha$ significance level using a \termsub{$\pmb{\chi^2}$ test for independence}{chi-square test for independence@$\chi^2$ test for independence}.
...
\item \termsub{$\pmb{\chi^2}$ test for homogeneity}{chi-square test for homogeneity@$\chi^2$ test for homogeneity}: 
\begin{itemize}\vspace{-2mm}
\setlength{\itemsep}{0mm}
...
\item \termsub{$\pmb{\chi^2}$ test for independence}{chi-square test for independence@$\chi^2$ test for independence}
\begin{itemize}\vspace{-2mm}
\setlength{\itemsep}{0mm}
