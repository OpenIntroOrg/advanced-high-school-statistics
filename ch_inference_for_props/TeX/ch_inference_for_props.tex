...
\Comment{Verify the use of the $t$ table is consistent with the guidelines given in Ch~5.}
...
If a single-example run-through (not breaking things into pieces) is desired, a second example should be used, since putting the term box ahead of the first example felt a bit awkward.}
...
\Comment{David: They should be using a pooled proportion when $p_1 = p_2$ is the null hypothesis. In the case where $p_1 - p_2 = X$ where $X \neq 0$ is the null hypothesis, then we wouldn't use a pooled proportion (but we don't cover this context in the textbook, which I believe was dropped during the Preliminary Edition editing).}
