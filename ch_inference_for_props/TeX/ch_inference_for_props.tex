...
\begin{example}{A poll randomly selected 1,042 adults residing in the state of New York and asked the question, ``Regardless of whether the person shows symptoms or not, do you support or oppose a 21 day quarantine for anyone who has come in contact with someone with the Ebola virus?"\footnote{\oiRedirect{ebola_quarantine_poll_2014}{NBC 4 New York / The Wall Street Journal / Marist Poll. October 31, 2014.}} Among the sample, 82\% said``support", 15\% said ``oppose", and 3\% said ``unsure". Carry out the appropriate 95\% confidence interval procedure to estimate the true proportion of adults in New York who supported a quarantine.  Is there evidence that the true percent is greater than 75\%?}
We will construct a 1-proportion $Z$ interval.  The poll can be considered a simple random sample of adults form New York.  Also, $1042\times 0.82\ge10$ and $1042\times (1-0.82)\ge10$, so the conditions for the test are satisfied.  The standard error and confidence interval can be calculated as
\begin{align*}
SE=&\sqrt{\frac{0.82(1-0.82)}{1042}}=0.0119 \\
0.82\ \pm&\  1.96 \times 0.0119 \\
(0.796&,\ 0.843)
\end{align*}
We are 95\% confident that the true proportion of adults in New York who supported a 21 quarantine for anyone who has come in contact with someone with the Ebola virus lies between 0.796 and 0.843.  Because the entire interval is above 0.75, the interval does provide evidence that the true percent is greater than 75\%.
\end{example}
...
\Comment{L: added this exercise - TODO(David) fix formatting as needed}

\begin{exercise}According to a Gallup poll\footnote{\urlwofont{http://www.gallup.com/poll/182180/support-nuclear-energy.aspx?utm_source=Politics&utm_medium=newsfeed&utm_campaign=tiles}}  conducted in March of 2015, 51\% of respondents reported that they support nuclear energy.  (This result was similar to the result from a poll conducted two years before, but down from the peak of 62\% found five years before.)  The survey was based on telephone interviews from a random sample of 1,025 adults in the United States.  Is there convincing evidence that the true percent of adults in the United States that support nuclear energy is greater than 50\%?  Carry out an appropriate test at the 0.10 significance level.\footnote{We will perform a 1-proportion $Z$ test.  We will assume that the sample can be treated as a simple random sample of adults from the United States.  Our null value will be $p$=0.5 and $1025\times 0.5=1025\times(1-0.5)\ge10$ so the conditions for the test are satisfied. We will use a one-sided test with the following hypotheses:
\begin{itemize}
\item[$H_0$:] $p = 0.5$. Support for nuclear energy is 50\%.
\item[$H_A$:] $p > 0.5$. Support for nuclear energy is higher than 50\%.
\end{itemize}
We will use a significance level of $\alpha = 0.10$ for the test. We can compute the standard error as $SE = \sqrt{\frac{p_0 (1 - p_0)}{n}} = \sqrt{\frac{0.5 (1 - 0.5)}{1025}} = 0.0156$.  The test statistic can be computed as: $Z = \frac{\hat{p} - p_0}{SE} = \frac{0.51 - 0.50}{0.0156} = 0.656$ and the p-value for this one-sided test is 0.256.  $0.256>0.10$, so we do not reject $H_0$.  We do not have evidence that the true percent of adults in the United States that support nuclear energy is greater than 50\%.  
 }
\end{exercise}
...
\Comment{TODO(David): Find suitable TI calculator videos.}
...
\Comment{David: the ``calculating the'' and ``carrying out the'' text doesn't really add anything to the titles below. Objections to deleting this text in the TI term boxes?}

\begin{termBox}{\tBoxTitle{\videohref{ti84_1_prop_CI} TI-83/84: 1-proportion $Z$ confidence interval}
...
\begin{termBox}{\tBoxTitle[]{\videohref{casio_1_prop_inference} Casio fx-9750GII: 1-proportion $Z$ confidence interval}
...
\begin{termBox}{\tBoxTitle{Margin of error\color{red}ADD THIS BOX}
...
\Add{
\begin{example}{All other things being equal, will the margin of error be bigger for a 90\% confidence interval or a 95\% confidence interval?}Because a 95\% confidence interval must be wider, it will have a larger margin of error.
\end{example}

\begin{example}{All other things being equal, what happens to the margin of error as the sample size increases?}As the sample size $n$ increases, the $SE$ will decrease.  Thus, as $n$ increases, the margin of error will decrease.  This makes sense as we expect less error with a larger sample.\
\end{example}
}
...
\begin{termBox}{\tBoxTitle{\videohref{ti84_2_prop_CI} TI-83/84: 2-proportion $Z$ confidence interval}
...
\begin{termBox}{\tBoxTitle[]{\videohref{casio_2_prop_inference} Casio fx-9750GII: 2-proportion $Z$ confidence interval}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Navigate to \calctext{STAT} (\calcbutton{MENU} button, then hit the \calcbutton{2} button or select \calctext{STAT}).
\item Choose the \calctext{INTR} option (\calcbutton{F4} button).
...
\Comment{edit footnote to be non-specific to TI?}
...
\begin{termBox}{\tBoxTitle{TI-84: Finding upper tail areas under the chi-square curve}
...
\begin{termBox}{\tBoxTitle[]{Casio fx-9750GII: Finding upper tail areas under the chi-square curve}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Navigate to \calctext{STAT} (\calcbutton{MENU} button, then hit the \calcbutton{2} button or select \calctext{STAT}).
...
\Comment{The table below needs to be in a table environment, have a caption, and be referenced using a label.}
