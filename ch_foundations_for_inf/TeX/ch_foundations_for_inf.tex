...
\sectionintro{
\Add{\noindent Companies such as the Gallup and Pew Research frequently conduct
polls as a way to understand the state of public opinion
or knowledge on many topics, including politics,
scientific understanding, brand recognition, and more.  How well do these polls estimate the the opinion or knowledge of the
...
than a smaller sample.  \Add{Remember though, that this is only true for \emph{random} samples.  Additionally, a bigger sample cannot correct for response bias or other types of bias that may be present.}
...
\href{\oiRedirectUrl{desmos-foursamplingdistributions}}{
   ...
   \caption{Four sampling distributions shown, with parameter identified. \qquad  \qquad Explore this and other Desmos activities at \oiRedirect{opeintro-desmos}{openintro.org/desmos}.}
   \label{fourSamplingDistributions}
\end{figure}

\Comment{Added these exercise}
...
\begin{itemize}
\item In this section we laid some groundwork for our study of \term{inference}.  Inference involves using known sample values to estimate or better understand unknown population values.
...
\sectionintro{
\Add{
\noindent The site \url{fivethiryeight.com} regularly forecasts support for each candidate in House and Senate races.  In addition to point estimates, they report confidence intervals.\footnote{See: \url{https://projects.fivethirtyeight.com/2018-midterm-election-forecast/senate}} What are confidence intervals and how do we interpret them?  
...
\Comment{I tried integrating the rosling scenario into this section, but had difficulty, because ahss talks about the hypotheses being about population parameters, and it is awkward here to talk about "population", when the parameter in this case is kind of a probability, not a population parameter per se.  I do like the example though and wish that ahss could use a less complicated example that the consultant example for introducing this topic.  }
...
\Comment{ please review the box below}

\begin{onebox}{Null and alternative hypotheses}
\vspace{-5mm}
\begin{description}
\item[] The \term{null hypothesis} is abbreviated $H_0$. It represents a skeptical perspective and is often a claim of no change or no difference.  
\item[] The \term{alternative hypothesis} is abbreviated $H_A$. It is the claim researchers hope to prove or find evidence for and it often asserts that there has been a change or an effect.  
...
\Comment{This footnote/solution is split onto the next page}
...
\sectionintro{
\Add{
\noindent It is the responsibility of the statistician or the data scientist to know when the use of these inference procedures is appropriate and to correctly interpret the results.  In this section we look at considerations around the misuse or misinterpretation of these procedures.}
%%
\subsection*{Learning objectives}
\begin{enumerate}
...
\Comment{please feel free to edit this and the footnote as you see fit}

\Add{When a result is statistically significant at the $\alpha=0.05$ level, we have evidence that the result is real.  However, when there is no difference or effect, we can expect to commit a Type I~error and incorrectly reject the null hypothesis 5\% of the time.  Therefore we must beware of what is called p-hacking, in which researchers may test many, many hypotheses and then publish the ones that come out significant.  As we noted, we can expect 5\% of the results to be significant when the null hypothesis is true and there really is no difference or effect.\footnote{
The problem is even greater than p-hacking.  In what has been called the "reproducibility crisis", researchers have failed to reproduce a large proportion of results that were found significant and were published in scientific journals.  The probability that a difference will be found significant given that it is not a real difference is not the same as the probability that a difference is not real, given that it was found significant.  Depending upon the veracity of the hypotheses tested, the latter can be upwards of 80\%, leading some to assert that "most published research is false".  \url{https://www.economist.com/briefing/2013/10/18/trouble-at-the-lab}
}
\Comment{can't read full article on economist}
}
