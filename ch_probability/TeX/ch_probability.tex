...
\item \Add{Use Venn diagrams to represent events and their probabilities and to visualize complement, union, and intersection of events.}
...
\item \Add{Apply the Multiplication Rule for finding the joint probability of independent events.}

\end{enumerate}
}

\Comment{added subsection header}
...
\sectionintro{
\Add{
\noindent In this section we will use conditional probabilities to answer the following questions:
\begin{itemize}
\item What is the likelihood that a machine learning algorithm will classify a photo as about fashion given that it is not about fashion?

\item How much more likely to attend college are children whose parents attended college than children whose parents did not attend college?

\item Given O+ result on a test such as an HIV test or a pregnancy test, what is the probability that one is actually positive?
\end{itemize}
...
\end{nexercise}\end{exercisewrap}
\footnotetext{The answer is 0.0382, which can be verified using Table~\ref{smallpoxProbabilityTable}.}
...
%%
\Comment{changed subsection name to better align with using this concept again later for inference}
...
\Comment{turned this into a onebox and slightly reworded}
...
\begin{examplewrap}\begin{nexample}{Are teenager college attendance and parent college degrees independent or dependent? Table~\ref{contTableOfParStCollegeCopy} may be helpful.}\label{teenParentCollegeIndependentExample}
...
We are given sufficient information to quickly compute the probability of testing positive if a woman has breast cancer ($1.00 - 0.11 = 0.89$). However, we seek the inverted probability of cancer given O+ test result:
\begin{align*}
...
\end{align*}
That is, even if a patient has O+ mammogram screening, there is still only a~4\%~chance that she has breast cancer.
\end{nexample}\end{examplewrap}
...
\Comment{check placement of truth/mammogram figure}
...
\sectionintro{
\Add{
\noindent What is the probability of exactly 50 heads in 100 tosses of a coin?  Of exactly 1~left handed person in a random sample of 8 people?  Of at most 3~people exceeding their insurance deductible in a random sample of 20 people?  The binomial formula can help us answer these questions.
}
...
\Comment{changed exercises - doesn't make sense to say the 4 friends are a random sample}
...
\sectionintro{
\Add{\noindent What is the probability of getting a sum greater than 16 in three rolls of a die?  Finding all possible combinations that satisfy this would be tedious, but we could conduct a physical simulation or a computer simulation to estimate this probability.  With modern computing power, simulations have become an important and powerful tool for statisticians.  In this section we will look at the concepts that underlie simulations.}
%%
\subsection*{Learning objectives}
\begin{enumerate}
...
\sectionintro{
\Add{\noindent The chance of landing on single number in roulette is 1/38 and the pay is 35:1.  The chance of landing on Red is 18/38 and the pay is 1:1.  Which game has the higher expected value?  The higher standard deviation?  How do we interpret these quantities in this context?  If you were to play each game 20 times, what would the \emph{distribution} of possible outcomes look like?  In this section, we define and summarize random variables such as this and look at some of their properties.
}
...
\end{nexercise}\end{exercisewrap}
\footnotetext{$150 \times 1 - 30 = 120$;  $150 \times 2 - 30 = 270$; $150 \times 3 - 30 = 420$}
...
\end{align*}
If we plug in the change in the stock value for $X$ and $Y$, this equation gives the change in value of Leonard's stock portfolio for the month. O+ value represents a gain, and a negative value represents a loss.
\end{nexample}\end{examplewrap}
...
\begin{exercisewrap}\begin{nexercise}
You should have found that Leonard expects O+ gain in Guided Practice~\ref{expectedChangeInLeonardsStockPortfolio}. However, would you be surprised if he actually had a loss this month?\footnotemark
\end{nexercise}\end{exercisewrap}
...
If we need the standard deviation of a linear combination of independent variables, such as $aX + bY$, we can consider $aX$ and $bY$ as two new variables. Recall that multiplying all of the values of variable by O+ constant multiplies the standard deviation by that constant. Thus, $SD_{aX}$ =  $a \times SD_X$ and $SD_{bY}$ =  $b \times SD_Y$. It follows that:
\begin{align*}
SD_{aX + bY} = \sqrt{(a \times SD_X)^2 + (b \times SD_Y)^2}
\end{align*}
This equation can be used to compute the standard deviation of Leonard's monthly return. Recall that Leonard has \$6,000 in Google stock and \$2,000 in Exxon Mobil's stock. From Table~\ref{sumStatOfGOOGXOM}, the standard deviation of Google stock is 0.0849 and the standard deviation of Exxon Mobile stock is 0.0520.
\begin{align*}
SD_{6000X + 2000Y}
...
\sectionintro{
\Add{\noindent So far we have looked only at cases where the random variable takes on integer values.  What happens when the we consider a smooth curve and look at intervals of values, such as wait time for a bus?  In this section we introduce the concept of a continuous distribution.  In the next chapter, you will encounter the most famous continuous distribution of all.\footnote{It's the normal distribution!}
}
%%
\subsection*{Learning objectives}
\begin{enumerate}
