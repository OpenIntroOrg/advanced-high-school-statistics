...
P(A\text{ or }B) &= P(A) + P(B)
\end{align*}
If the equation that is checked holds true (the left and right sies are equal), $A$ and $B$ are mutually exclusive.  If hte equation does not hold, then $A$ and $B$ are not mutually exclusive.}
\end{tipBox}
...
Bayes' Theorem is just a generalization of what we have done using tree diagrams. The formula \Add{need not} be memorized; \Add{it can always be derived using a tree diagram:}
\begin{itemize}
\setlength{\itemsep}{0mm}
