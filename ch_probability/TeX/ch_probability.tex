...
{If two events are dependent, we can\underline{not} simply conclude they are mutually exclusive. For example, the drug use of parents and students are dependent, but they are not mutually exclusive.}
\end{caution}
...
\Comment{I made the answers calculator agnostic}
\begin{exercise}
...
In the previous section we saw how to apply the binomial formula to find the probability of exactly $k$ successes in $n$ independent trials of an event that has probability $p$.  Sometimes, though, we have a problem we want to solve but we don't know the appropriate formula, or even worse, a formula may not even exist. In this case, one approach is to estimate the probability via \term{simulation}.

You may already be familiar with simulations.  Want to know the probability of rolling a sum of 7 with a pair of dice?  Roll a pair of dice many, many, many times and see what proportion of times the sum was 7.  The more times you roll the pair of dice, the better the estimate will tend to be.  Of course, such an experiment is time consuming and may not be feasible.

In this section, we consider simulations using \term{random numbers}.  Random numbers (or technically, \emph{psuedo-random numbers}) can be produced using a calculator or computer.  Random digits are produced such that each digit, 0-9, is equally likely to come up in each spot.  You'll find that occasionally we may have the same number in a row -- sometimes multiple times -- but in the long run, each digit should appear with same relative frequency.

\Comment{TODO: David - add random digit table in appendix}.
...
\quad\ Row\quad\ \  & 1-5 && 6-10 && 11-15 && 16-20 \\
  \hline
1 & 43087 & \quad & 11864 & \quad & 51009 & \quad & 39689 \\
2 & 63432 & \quad & 72132 & \quad & 40269 & \quad & 56100 \\
3 & 19025 & \quad & 83056 & \quad & 62511 & \quad & 52598 \\
4 & 85117 & \quad & 46706 & \quad & 31083 & \quad & 24816 \\
5 & 16285 & \quad & 56280 & \quad & 01494 & \quad & 10243 \\
  \hline
6 & 91342 & \quad & 18473 & \quad & 50845 & \quad & 77757 \\
7 & 65099 & \quad & 14136 & \quad & 39052 & \quad & 50235 \\
...
10 & 27217 & \quad & 12151 & \quad & 52645 & \quad & 96218 \\
  \hline
\end{tabular}
\caption{Random number table. A full page of random numbers may be found in Appendix~\ref{TBD}.}
\label{sampleRandomNumberTable}
\end{table}
%set.seed(25)
%rand <- sample(0:9, 200, replace = TRUE)
%tab <- array(rand, dim = c(10,20))
%rownames(tab) <- NULL
%colnames(tab) <- NULL
%tab <- apply(tab, 1, function(x) {
%  c(paste(x[1:5], collapse = ""), paste(x[6:10], collapse = ""),
%     paste(x[11:15], collapse = ""), paste(x[16:20], collapse = ""))
%})
%tab <- t(tab)
%to.print <- paste(1:10, "&", apply(tab, 1, paste, collapse = " & \\quad & "), "\\\\", collapse = "\n")
%cat(to.print)

\begin{example}{Imani's favorite brand of cereal s running a special where 20\% of the cereal boxes contain a special prize.  Imani really wants that prize.  If her mother buys 6 boxes of the cereal over the next few months, what is the probability Imani will get a prize?}
...
\begin{align*}
0,1 &\rightarrow \text{prize}\\
2-9 &\rightarrow \text{no prize}
\end{align*}
\Add{Of the ten possible digits (0, 1, 2, ..., 8, 9), two of them, i.e. 20\% of them, correspond to winning a prize, which exactly matches the odds that a cereal box contains a prize.} \Comment{D - I think reiterating this is helpful. Not sure if the proposed wording I've put here is ideal or not, but could be starter text.}

In Imani's simulation, one trial will consist of 6 boxes of cereal, and therefore a trial will require six digits (each digit will correspond to one of the boxes of cereal). We will repeat the simulation for 20 trials. Therefore we will need 20 sets of 6 digits. Let's begin on the first row of the random digit table, shown in Table~\ref{sampleRandomNumberTable}.

\Comment{TODO(Leah) - Finish up example. (I realize you know this, but just putting comments as precautionary.)}
\end{example}

\begin{example}{We can also use simulations to estimate quantities other than probabilities.  Let's say that instead of buying exactly 6 boxes of cereal, Imani's mother agrees to buy boxes of this cereal \emph{until} she finds one with a prize.  On average, how many boxes of cereal would one have to buy until one gets a prize?}
...
\begin{align*}
00-27 &\rightarrow \text{success}\\
28-99 &\rightarrow \text{failure}
\end{align*}
\end{example}

\Comment{D - Worth having an example where the probability is something like 1-in-7 where they will need to discard some of the random digits?}
