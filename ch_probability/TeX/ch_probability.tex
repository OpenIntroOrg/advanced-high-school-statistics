...
In the previous section we saw how to apply the binomial formula to find the probability of exactly $k$ successes in $n$ independent trials of an event that has probability $p$.  Sometimes, though, we have a problem we wish to solve for which we do not know the appropriate formula.  Worse, a formula may not even exist.  In this case, one approach is to estimate the probability via \term{simulation}.

You may already be familiar with physical or empirical simulations.  Want to know the probability of rolling a sum of 7 with a pair of dice?  Roll a pair of dice many, many, many times and see what proportion of times the sum was 7.  The more times you roll the pair of dice, the better the estimate should tend to be.  Of course such an experiment is time consuming and may not be feasible.

In this section, we consider simulations using \term{random numbers}.  Random numbers (or technically, \emph{psuedo-random numbers}) can be produced using a calculator or computer.  Random digits are produced such that each digit, 0-9, is equally likely to come up in each spot.  This means that it is possible to have runs of digits, but in the long run, each digit should appear with same relative frequency.

\Comment{TODO: David - add random digit table here, and a full page random digit table in appendix}.

\begin{example}{Imani's favorite brand of cereal s running a special.  20\% of the boxes contain a special prize.  Imani really wants that prize.  If her mother buys 6 boxes of the cereal over the next few months, what is the probability Imani will get a prize? }
...
\end{align*}
Here, one trial will consist of 6 boxes of cereal, therefore one trial will require five digits (each digit corresponds to one box of cereal).  We will repeat the simulation for 20 trials.  Therefore we will need 20 sets of 6 digits.  Let's begin on the first row of the random digit table.  
\end{example}
