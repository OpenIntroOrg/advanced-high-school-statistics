\begin{chapterpage}{Probability and probability distributions}
  \chaptertitle{Probability and probability \titlebreak{} distributions}
  \label{probability}
...
If we sample from a small population \termni{without replacement}, we no longer have independence between our observations. In Example~\ref{3woRep}, the probability of not being picked for the second question was conditioned on the event that you were not picked for the first question. In Example~\ref{3wRep}, the professor sampled her students \term{with replacement}: she repeatedly sampled the entire class without regard to who she already picked.
...
\end{align*}
For example, instead of wanting to know $P($lived $|$ inoculated$)$, we might want to know $P($inoculated $|$ lived$)$. This is more challenging because it cannot be read directly from the tree diagram. In these instances we use \termni{Bayes' Theorem}. Let's begin by looking at a new example.
...
\item Sometimes, the conditional probability $P(B|A)$ may be known, but we are interested in the ``inverted" probability $P(A|B)$.  \termni{Bayes' Theorem} helps us solve such conditional probabilities that cannot be easily answered.  However, rather than memorize Bayes' Theorem, one can generally draw a tree diagram and apply the conditional probability rule $P(A | B)=\frac{P(A\text{ and }B)}{P(B)}$. 
...
\begin{itemize}
\item When a probability is difficult to determine via a formula, one can set up a \termni{simulation} to estimate the probability.
...
\begin{figure}[ht]
\centering
\Figure[A dot plot is overlaid on a box plot for a variable ``Monthly Returns (2009 - 2011)". The box portion spans about -300 to positive 550, with the median line at about 200. The whiskers extend to the lower end at about -1000 and at the upper end at about 1200.  For the dot plot, the points are pretty evenly distributed across the locations of the box-and-whiskers portion of the box plot.]
{.55}{changeInLeonardsStockPortfolioFor36Months}
...
\item A probability distribution can be represented with a histogram and, like the distributions of data that we saw in Chapter 2, can be summarized by its \termni{center}, \termni{spread}, and \termni{shape}.
...
\item Find and interpret the mean (expected value) and standard deviation of a geometric distribution.
...
\footnotetext{No. The geometric distribution is always
  right skewed and can never be well-approximated by a
  normal model.}
...
\noindent Get started quickly with this Desmos \oiRedirect{desmos-geometric}{Geometric Calculator} (\small{available at \oiRedirect{openintro-ahss-desmos}{openintro.org/ahss/desmos}}).

\begin{center}
\Figure[A Desmos calculator screen is shown.  On the left is a slider with p set to 0.25.  Below that, the geometric probability function is used and the mean and standard deviation are calculated for the geometric distribution with parameter p.  On the right is a graph of the geometric distribution with parameter p.  x, on the horizontal axis, ranges from 0 to about 20.  The heights, which correspond to probability, start at about 0.32 when x is 0, then drop down for increasing values of x.  By 20, the height is close to 0.]
{0.95}{desmosGeometric}
\end{center}
...
{\input{ch_probability/TeX/geometric_distribution.tex}}
...
100 coin tosses?
Or the probability of randomly sampling 12 people and having
more than 9 of them identify as male?  
 If the probability of a defective part is 1\%, how many defective items would we expect in a random shipment of 200 of those parts?  We can model these scenarios and answer these questions using the binomial distribution.
...
\item Calculate the probabilities of the possible values of a binomial random variable using the binomial formula.
...
\item Determine whether a binomial distribution can be modeled as approximately
normal. If so, use normal approximation to estimate cumulative binomial probabilities.
\end{enumerate}
}
...
deductible.  Each people is thought of as a \term{trial}.
We label a trial a \term{success} if the individual healthcare costs
do not exceed the deductible.
We label a trial a \term{failure} if the individual healthcare costs do exceed the
deductible.
...
\noindent Get started quickly with this Desmos \oiRedirect{desmos-binomial}{Binomial Calculator} (\small{available at \oiRedirect{openintro-ahss-desmos}{openintro.org/ahss/desmos}}).
\begin{center}
\Figure[A Desmos calcualtor screen is shown.  On the left, there are sliders, with n set to 25 and p set to 0.5.  Below that, the binomialdist function is used and the mean and standard deviation are calculated for the binomial distribution with parameters n and p.  On the right is a graph of the binomial distribution with parameters n and p.  x, on the horizontal axis, ranges from 0 to 25.  The distribution is approximately normal, with a mean of 12.5 and a standard deviation of 2.5.  ]
{0.95}{desmosBinomial}
\end{center}
...
\begin{figure}[h]
\centering
 \Figure[A normal distribution centered at 140 with a standard deviation of about 10 is shown. A region of this distribution is shaded from 29 to 31. Additionally, a red outlined area is boxed out between 28.5 and 31.5 that represents the exact binomial distribution.]
{}{normApproxToBinomFail}{normApproxToBinomFail-n400-p35}
...
%%%%%%%%%%Section exercises
{\input{ch_probability/TeX/binomial_distribution.tex}}
...
\begin{itemize}
\item The \textbf{General Multiplication Rule} for \textbf{and} probabilities (intersection), along with the special case when events are \term{independent}.
