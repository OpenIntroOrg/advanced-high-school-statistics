...
\Comment{deleted "special topic" in all sections of this chapter}
...
\Comment{students must know the term relative frequency interpretation/definition}
...
Probability can be illustrated by rolling a die many times.   \Cut{Let $\hat{p}_n$ be the proportion of outcomes that are \resp{1} after the first $n$ rolls. As the number of rolls increases, $\hat{p}_n$}\Add{Consider the event ``roll a 1".  The \term{relative frequency} of an event is the proportion of times the event occurs out of the number of trials.  Let $\hat{p}_n$ be the proportion of outcomes that are \resp{1} after the first $n$ rolls. As the number of rolls increases, $\hat{p}_n$ (the relative frequency of rolls) will converge to the probability of rolling a \resp{1}, $p = 1/6$}. Figure~\ref{dieProp} shows this convergence for 100,000 die rolls. The tendency of $\hat{p}_n$ to stabilize around $p$\Add{, that~is, the tendency of the relative frequency to stabilize around the true probability,} is described by the \term{Law of Large Numbers}. 

\begin{figure}[bt]
\centering
\includegraphics[width=0.85\textwidth]{03/figures/dieProp/dieProp}
...
\Comment{replace phat and p with words in the text box?}
...
Two outcomes are called \term{disjoint} or \term{mutually exclusive} if they cannot both happen \Add{in the same trial}. For instance, if we roll a die, the outcomes \resp{1} and \resp{2} are disjoint since they cannot both occur \Add{on a single roll}. On the other hand, the outcomes \resp{1} and ``rolling an odd number'' are not disjoint since both occur if the outcome of the roll is a \resp{1}. The terms \emph{disjoint} and \emph{mutually exclusive} are equivalent and interchangeable.
...
\begin{figure}[hhh]
\centering
\includegraphics[height=0.7in]{03/figures/disjointSets/disjointSets}
...
\begin{figure}
\centering
\includegraphics[height=1.4in]{03/figures/venn/venn}
...
\Comment{I switched the 12 and 13 to 13 and 12 and made order same in bottom eqn}
\begin{eqnarray*}
...
\begin{tipBox}{\tipBoxTitle{``or'' is inclusive \color{red} CUT THIS BOX}
...
\begin{tipBox}{\tipBoxTitle{``or'' is inclusive \color{red} ADD THIS BOX}
When we write, ``or"  in statistics, we mean ``and/or'' unless we explicitly state otherwise. Thus, $A$ or $B$ occurs means $A$, $B$, or both $A$ and $B$ occur.  This is equivalent to at least one of $A$ or $B$ occurs.}
\end{tipBox}
...
\hfill\includegraphics[height=13mm]{03/figures/emailSpamNumberVenn/emailSpamNumberVenn} \vspace{-13mm}
\end{minipage}
}
\end{exercise}
...
\Comment{subsection on Probability Distribution moved to beginning of section on Random Variables}
...
\begin{figure}[hht]
\centering
\includegraphics[width=0.5\textwidth]{03/figures/complementOfD/complementOfD}
...
\Add{An event $A$ together with its complement $A^c$ comprise the entire sample space.  Because of this we can say that $P(A) + P(A^c) = 1$.  
}
...
\Comment{Because I moved subsection "distribution of random variable" to before expectation, these two exercises must be changed.}

\Add{
\begin{exercise}
A die is rolled 10 times.  What is the complement of getting at least one 6 in 10 rolls of the die?  What is the complement of getting at most three 6's in 10 rolls of the die?\footnote{The complement of getting at least one 6 in ten rolls of a die is getting zero 6's in the 10 rolls.  The complement of getting at most three 6's in 10 rolls is getting 0, 1, or 2 6's in 10 rolls.}
\end{exercise}
}
...
\begin{figure}[hht]
\centering
\includegraphics[width=0.6\textwidth]{03/figures/indepForRollingTwo1s/indepForRollingTwo1s}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.65\textwidth]{03/figures/drugUseVenn/drugUseVenn}
...
&& \quad \quad \quad \quad P(\text{\var{parents} \resp{did not use}, \underline{\color{black}\var{student} \resp{uses}}}) \\
&& \quad = 0.28 + 0.21 = 0.49
\end{eqnarray*}
}
\Add{
...
%\begin{align*}
%P(\text{\underline{\var{parents} \resp{did not use}} and \var{student} \resp{uses }}) + P(\text{\underline{\var{parents} \resp{did not use}} and \var{student} \resp{does not use}})
%\end{align*}
%\end{exercise}
...
Write out, in formal notation, the probability a randomly selected person who was not inoculated died from smallpox, and find this probability.\footnote{$P($\var{result} = \resp{died} $|$ \resp{not inoculated}$) = \frac{P(\text{\var{result} = \resp{died} and \resp{not inoculated}})}{P(\text{\resp{not inoculated}})} = \frac{0.1356}{0.9608} = 0.1411$.}
\end{exercise}

\textB{\pagebreak}

\begin{exercise}
Determine the probability that an inoculated person died from smallpox. How does this result compare with the result of Exercise~\ref{probDiedIfNotInoculated}?\footnote{$P($\resp{died} $|$ \resp{inoculated}$) = \frac{P(\text{\resp{died} and \resp{inoculated}})}{P(\text{\resp{inoculated}})} = \frac{0.0010}{0.0392} = 0.0255$. The death rate for individuals who were inoculated is only about 1~in~40 while the death rate is about 1~in~7 for those who were not inoculated.}
\end{exercise}
...
\Comment{replaced "it is useful to think of A as the outcome of interest..." with "For the term P(A$|$B) it is useful to think of A as the outcome of interest.."}
...
\Comment{delete term box or move to section on tree diagrams?}
...
\Comment{sampling from a small population moved to here}
%_________________
...
\Add{
We are often interested in knowing whether or not two events are independent and whether or not they are mutually exclusive.}

\begin{tipBox}{\tipBoxTitle{Checking the independence of two events}
...
P(A\text{ and }B) &= P(A)\times P(B)
\end{align*}
If asked to determine if events $A$ and $B$ are independent, check one of the two equations above. If the equation holds, the events $A$ and $B$ are independent. If the equation does not hold (the left and right sides are not equal), then $A$ and $B$ are dependent.
}
\end{tipBox}
...
\begin{example}{Are student and parent drug use independent or dependent?}
To check for independence, we can use either equation in the Tip Box above. Let's use the first approach described. If student and parent drug use are independent, it must be true that 
\begin{align*}
P(\text{\var{student} \resp{uses }} | \text{ \var{parent} \resp{used}}) &= P(\text{\var{student} \resp{uses}}) 
\end{align*}
Using the table we will see whether or not the above statement is true.
\begin{align*}
P(\text{\var{student} \resp{uses }} | \text{ \var{parent} \resp{used}}) &\overset{?}{=} P(\text{\var{student} \resp{uses}}) \\
\frac{125}{210}=0.595 &\neq \frac{219}{445}=0.492
\end{align*}
These terms are not equal, therefore student and parent drug use are dependent.  That is, the probability a student uses drugs is higher if we know that the students' parents used drugs.
\end{example}

\begin{exercise}Use the second equation in the Tip Box above to show that student and parent drug usage are dependent.\footnote{If student and parent drug use are independent, it must be true that
\begin{align*}
P(\text{\var{student} \resp{uses} and \var{parent} \resp{used}}) &= P(\text{\var{student} \resp{uses}})\times P(\text{\var{parent} \resp{used}})
\end{align*}
Using the table we will see whether this statement is true.
\begin{align*}
...
\end{align*}
These terms are not equal, therefore student and parent drug use are dependent.  }
\end{exercise}

\Comment{move to a different spot}
\Cut{
\begin{example}{Find the probability that the student and parent used drugs directly from the table and using the General Multiplication Rule.} 
From the table, we can see the there 125 cases where both student and parent used drugs.  There were a total of 445 cases.  Therefore 
\begin{align*}
P(\text{\var{student} \resp{uses } and \var{parent} \resp{used}}) =\frac{125}{445}
\end{align*}
Using the General Multiplication Rule we get
\begin{align*}
P(\text{\var{student} \resp{uses} and \var{parent} \resp{used}}) &=
P(\text{\var{student} \resp{uses} }| \text{ \var{parent} \resp{used}})\times P(\text{\var{parent} \resp{used}})\\
&=\frac{125}{210}\times \frac{210}{445} \\
&=\frac{125}{445}
\end{align*}
which matches the result we arrived at using the table.
\end{example}
}
}

\begin{tipBox}{\tipBoxTitle{Checking whether two events are mutually exclusive}
If $A$ and $B$ are mutually exclusive (disjoint) events, then the probability of $A$ and $B$ occuring at the same time is zero.  Mathematically, this is written as
\begin{align*} 
P(A \text{ and }B) &= 0
\end{align*}
...
\end{align*}
If asked to determine if events $A$ and $B$ are mutually excusive, check one of the two equations above. If the equation holds, the events $A$ and $B$ are mutually exclusive. If the equation does not hold (the left and right sides are not equal), then $A$ and $B$ are not mutually exclusive.}
\end{tipBox}

\Add{
\begin{example}{Are student and parent drug use mutually exclusive?}
Looking in the table, we see that there are 125 instances where both the parents used drugs and the student uses drugs, indicating the probability of both events occurring is greater than 0. Since we have found an example where both of these events happen together, these two events are not mutually exclusive.  Formally, if student and parent drug use are mutually exclusive, it must be true that 
\begin{align*}
P(\text{\var{student} \resp{uses} and \var{parent} \resp{used}}) = 0
\end{align*}
Using the table we see that
\begin{align*}
P(\text{\var{student} \resp{uses} and \var{parent} \resp{used}}) = \frac{125}{445}\neq 0
\end{align*}
Therefore student and parent drug use are not mutually exclusive.  
\end{example}

What is the relationship between the two concepts ``dependent" and ``mutually exclusive"?  Does one imply the other or are they unrelated?  To understand this, we go back to the definitions.  If $A$ and $B$ are mutually exclusive, they cannot both happen at the same time.  This means that if $B$ happens, $A$ cannot happen.  In other words, if two events are mutually exclusive, knowing that the one occurred \emph{changes} the likelihood of the other to zero.\footnote{Formally, if $A$ and $B$ are mutually exclusive, $P(B | A) = 0$.  Also, if $A$ and $B$ are independent, $P(B | A)=P(B)$. Therefore, assuming $P(B)\ne 0$, if $A$ and $B$ are mutually exclusive, they cannot be independent.}  This is a strict form of dependence.  
}

\begin{tipBox}{\tipBoxTitle{If two events are mutually exclusive, they are dependent}
If two events are mutually exclusive, then if one is true, the other cannot be true. This implies the two events are in some way connected, so they are dependent.  By a similar logic, if two events are independent, they cannot be mutually exclusive.}
\end{tipBox}

\begin{caution}{Dependent events need not be mutually exclusive. }{If two events are dependent, that does \textbf{not} ensure that they are mutually exclusive, as we have seen in this section’s examples.}
\end{caution}
...
\Comment{I've changed this section to look only at simple tree diagram questions, not inverted probability.  And I changed the Bayes' Theorem section to focus less on the formula and more on the process.  For ap course, students should always make a tree diagram.  They do not need to understand the more complicated formula.  }
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.93\textwidth]{03/figures/smallpoxTreeDiagram/smallpoxTreeDiagram}
...
\Cut{
\begin{eqnarray*}
&& P(\text{\resp{inoculated} and \resp{lived}}) \\
	&&\quad = P(\text{\resp{inoculated}})\times P(\text{\resp{lived}}|\text{\resp{inoculated}}) \\
...
\begin{example}{What is the probability that a randomly selected person lived?}There are two ways that a person could have lived:  be inoculated \emph{and} live OR not be inoculated \emph{and} live.  To find this probability, we sum the two joint probabilities.  P(\text{\resp{lived}}) = $0.0392(0.9745) + 0.9608(0.8589) = 0.03824 + 0.82523 = 0.86347$.  
\end{example}

\Comment{I moved all Bayes theorem stuff to next section to separate out these two sections.
}
\Cut{
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{03/figures/testTree/testTree}
...
\includegraphics[width=\textwidth]{03/figures/treeDiagramAndPass/treeDiagramAndPass} \vspace{-25mm}
\end{minipage}}
\end{exercise}
}
...
(c)~$P($able to construct tree diagrams and passed$) = P($able to construct tree diagrams$)P($passed $|$ able to construct tree diagrams$) = 0.78(0.97) = 0.7566$.
...
\end{align*}
\Add{For example, instead of wanting to know $P($lived $|$ inoculated$)$, we might want to know $P($inoculated $|$ lived$)$.  This is more challenging because it cannot be read directly from the tree diagram. In these instances we use \term{Bayes' Theorem}.  Let's begin by looking at a new example.}\Cut{Tree diagrams can be used to find the second conditional probability when given the first. However, sometimes it is not possible to draw the scenario in a tree diagram. In these cases, we can apply a very useful and general formula: Bayes' Theorem.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{03/figures/BreastCancerTreeDiagram/BreastCancerTreeDiagram}
...
Notice that we are given sufficient information to quickly compute the probability of testing positive if a woman has breast cancer ($1.00-0.11=0.89$). However, we seek the inverted probability of cancer given a positive test result. (Watch out for the non-intuitive medical language: a~\emph{positive} test result suggests the possible presence of cancer in a mammogram screening.) \Add{The inverted probability that we desire is.}

\begin{align*}
P(\text{has BC } | \text{ mammogram$^+$}) 
\end{align*}

where ``has BC'' is an abbreviation for the patient actually having breast cancer and ``mammogram$^+$'' means the mammogram screening was positive. We can use the conditional probability formula from the previous section.
...
\begin{align*}
P(\text{has BC $|$ mammogram$^+$}) &=  \frac{P(\text{has BC and mammogram$^+$})}{P(\text{has BC and mammogram$^+$}) +  P(\text{no BC and mammogram$^+$})} \\
...
\Comment{added to here}

\Cut{This inverted probability may be broken into two pieces:
\begin{align*}
...
Bayes' Theorem is just a generalization of what we have done using tree diagrams.  \Add{It need not be memorized.}  The numerator identifies the probability of getting both $A_1$ and $B$. The denominator is the \Add{ overall probability of getting $B$.} This bottom component of the fraction appears long and complicated\Add{; however, it is equivalent to what we did numerically using tree diagrams.  Each piece being added in the denominator represents traversing a branch of the tree diagram that ends in $B$. }  \Cut{since we have to add up probabilities from all of the different ways to get $B$. We always completed this step using tree diagrams so it did not seem as complex. }
...
\begin{tipBox}{\tipBoxTitle{Only use Bayes' Theorem when tree diagrams are difficult \color{red} CUT THIS BOX}
...
\begin{align*}
P(\text{sporting event} | \text{garage full}) &= \frac{P(\text{sporting event and garage full})}{P(\text{garage full})} \\
...
\end{minipage}
\begin{minipage}[c]{0.5\linewidth}
\includegraphics[width=\textwidth]{03/figures/treeDiagramGarage/treeDiagramGarage}\vspace{-30mm}
\end{minipage}}
\end{exercise}
}
...
\begin{figure}
\centering
\includegraphics[width=0.69\textwidth]{03/figures/bookCostDist/bookCostDist}
...
Chapter~\ref{introductionToData} emphasized the importance of plotting data to provide quick summaries. Probability distributions can also be summarized in a \Add{ histogram or }bar plot.  The probability distribution for the sum of two dice is shown in Table~\ref{diceProb} and its histogram is plotted in Figure~\ref{diceSumDist}.  The distribution of US household incomes is shown in Figure~\ref{usHouseholdIncomeDistBar} as a bar plot.  \Add{The presence of the 100+ category makes it difficult to represent it with a regular histogram.\footnote{It is also possible to construct a distribution plot when income is not artificially binned into four groups.  Density histograms for \emph{continuous} distributions are considered in Section~\ref{contDist}.}} 

\begin{figure}
\centering
\includegraphics[width=0.73\textwidth]{03/figures/diceSumDist/diceSumDist}
...
\begin{figure}
\centering
\includegraphics[width=0.68\textwidth]{03/figures/usHouseholdIncomeDistBar/usHouseholdIncomeDistBar}
...
\Comment{I made second one a regular histogram with no gaps and put it first.  Then in the caption of the second explain that a bar graph was chosen for simplicity but that it fails to show the right-skew of the distribution.}

In these bar plots, the bar heights represent the probabilities of outcomes. If the outcomes are numerical and discrete, it is usually (visually) convenient to make a \Add{ histogram}\Cut{ bar plot that resembles a histogram}, as in the case of the sum of two dice. Another example of plotting the bars at their respective locations is shown in Figure~\ref{bookCostDist} on page~\pageref{bookCostDist}.
...
\Comment{added "$p_i$ is the probabiity of $x_i$" to caption}
\begin{table}
\centering
...
\hline
\end{tabular}
\caption{The probability distribution for the random variable $X$, representing the bookstore's revenue from a single student.  $p_i$ is the probabiity of $x_i$}
\label{statSpendDist}
\end{table}
...
\begin{termBox}{\tBoxTitle{Expected value of a Discrete Random Variable \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle{Expected value of a Discrete Random Variable \color{red} ADD THIS BOX}
...
\begin{figure}
\centering
\includegraphics[width=0.72\textwidth]{03/figures/bookWts/bookWts}
...
\begin{figure}
\centering
\includegraphics[width=0.68\textwidth]{03/figures/contBalance/contBalance}
...
\begin{termBox}{\tBoxTitle{General variance formula \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle{Variance and Standard Deviation of a discrete random variable \color{red} ADD THIS BOX}
If $X$ takes outcomes $x_1$, $x_2$, ..., $x_n$ with probabilities $p_1$,  $p_2$, ..., $p_n$ and expected value $\mu_x=E(X)$, then to find the standard deviation of $X$, we first find the variance and then square root it as follows:
\begin{align}
...
\Add{Just as it is possible to compute the mean of a continuous random variable using calculus, so too can we compute the variance; however, we need not worry about this here.\footnote{$\sigma^2_x = \int (x - \mu_x)^2f(x)dx$ where $f(x)$ represents a function for the density curve.}
}

\Comment{reformatted table to take emphasis of top row.}
...
\begin{exercise}Verify that the distribution of $Y$ is given by the table below.\footnote{$150(1)-30 = 120$;  $150(2)-30 = 270$; $150(3)-30 = 420$}
...
&= 150\times 1.5 - 30\\
& = 195 \\
\tag*{\makebox[0pt][l]{\hspace*{-\textwidth}and}}
\sigma_{150X-30}&=SD(150X-30) \\
&=150\times SD(X) = 150\times 0.67 \\
&= 100.5
\end{align*}

The textbook company can expect to make \$195 dollars per student give or take \$100.50.
...
\Comment{slightly reformatted box}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.65\textwidth]{03/figures/changeInLeonardsStockPortfolioFor36Months/changeInLeonardsStockPortfolioFor36Months}
...
Just as we have done in many previous cases, we use the variance and standard deviation to describe the uncertainty associated with Leonard's monthly returns. To do so, the \Add{standard deviations and }variances of each stock's monthly return will be useful, and these are shown in Table~\ref{sumStatOfGOOGXOM}. The stocks' returns are nearly independent.
...
\Cut{
Here we use an equation from probability theory to describe the uncertainty of Leonard's monthly returns; we leave the proof of this method to a dedicated probability course.  The variance of a linear combination of independent random variables can be computed by plugging in the variances of the individual random variables and squaring the coefficients of the random variables:
\begin{align*}
...
\Comment{students will be given SD rather than variance, so they tend to use this formula}

Because the standard deviation is the square root of the variance, we can rewrite this equation using SD as follows:
\begin{align*}
...
\Comment{I wrote this because they sometimes have to do it for three variables}
...
This equation can be used to compute the variance of Leonard's monthly return.  Recall that Leonard has \$6000 in Google stock and \$200 in Exxon Mobil's stock.  From Table~\ref{sumStatOfGOOGXOM}, the standard deviation of Google stock is 0.0846 and the standard deviation of Exxon Mobile stock is 0.0519.
\begin{align*}
SD_{6000X + 2000Y}
	...
	&= 520
\end{align*}
The standard deviation is \$520. While an average monthly return of \$134 on an \$8000 investment is nothing to scoff at, the monthly returns are so volatile that Leonard should not expect this income to be very stable.
}
...
\begin{termBox}{\tBoxTitle{Variability of linear combinations of random variables \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle{Standard Deviation of linear combinations of random variables \color{red} ADD THIS BOX}
To find the standard deviation of a linear combination of random variables we first consider the two new variables $aX$ and $bY$, find the standard deviation of each, and then apply the equation for the standard deviation of the sum of two variables.
\begin{align*}
SD_{aX + bY} = \sqrt{(a\times SD_X)^2 + (b\times SD_Y)^2}
\end{align*}
...
\Add{Each coefficient is 1.  Thus, the standard deviation of the total weekly commute time is
\begin{align*}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{03/figures/fdicHistograms/fdicHistograms}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{03/figures/usHeightsHist180185/usHeightsHist180185}
...
\begin{figure}[tbh]
\centering
\includegraphics[width=0.9\textwidth]{03/figures/fdicHeightContDist/fdicHeightContDist}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{03/figures/fdicHeightContDistFilled/fdicHeightContDistFilled}
