...
The variable that is suspected to be the response variable is plotted on the vertical axis \Add{(y-axis)}and the variable that is suspected to be the explanatory variable is plotted on the horizontal axis \Add{(x-axis)}. In this example, the variables could be switched since either variable could reasonably serve as the explanatory variable or the response variable.

\begin{tipBox}{\tipBoxTitle{Drawing scatterplots} {\color{red} ADD THIS BOX}
(1) Decide which variable should go on each axis.  Draw and label the two axes.
\newline(2)  Take note of the largest value in each data set.  Add tick marks and scales to each.  
\newline(3) Plot the dots as you would on an (\textit{x},\textit{y}) coordinate plane.}
\end{tipBox}

\begin{tipBox}{\tipBoxTitle{Drawing scatterplots} {\color{red} CUT THIS BOX}
...
\Comment{moved this exercise here since it didn't make logical sense where it was}
...
\begin{tipBox}{\tipBoxTitle{Drawing histograms}{\color{red}ADD THIS BOX}
...
\newline 2.  Draw bars such that the height of the bar corresponds to the frequency and the width of the bar corresponds to the bin width.}
\end{tipBox}

\begin{tipBox}{\tipBoxTitle{Drawing histograms}{\color{red}CUT THIS BOX}
...
\begin{example}{What \emph{percent} of the emails had 10 thousand or more characters?}\Add{First find the number of emails that had 10 thousand or more characters.  }From the first example, we know that 31 emails had fewer than 10 thousand characters. Since there are 50 emails in total, there must be 19 emails that have 10 thousand or more characters. \Add{To find the percent, do: }$19/50 = 0.38 = 38\%$.
\end{example}
...
By tracing the height of the 15-20 thousand bin over to the vertical axis, we can see that it has a height just under 40 on the cumulative frequency scale. Therefore, we estimate that $\approx$39 of the emails had fewer than 30 thousand characters. \Cut{Note that answering this question using the original frequency histogram would require additional work.}\Add{Note that, unlike with a regular frequency histogram, we do not add up the height of the bars in a cumulative frequency histogram because each bar already represents a cumulative sum.}
\end{example}
...
\Add{
\begin{exercise}
Would you expect the distribution of number of pets per household to be right skewed, left skewed, or approximately symmetric?  Explain.\footnote{Because most households have 0 or 1 or 2 pets, there is greater density over the small numbers.  A small number of households, though, might have 10 or more pets, thus creating a long right hand tail.  Therefore, the distribution is likely right skewed. }
\end{exercise}
}

In addition to looking at whether a distribution is skewed or symmetric, histograms\Add{, stem-and-leaf plots, and dot plots} can be used to identify modes. A \term{mode} is represented by a prominent peak in the distribution.\footnote{Another definition of mode, which is not typically used in statistics, is the value with the most occurrences. It is common to have \emph{no} observations with the same value in a data set, which makes this other definition useless for many real data sets.} There is only one prominent peak in the histogram of \var{num\_\hspace{0.3mm}char}.
...
\Add{
\begin{example}{At the start of this section, we reported that the mean family income in the U.S. in 2012 was \$82,743.  Estimating the standard deviation of income as approximately \$50,000, is a family income of \$60,000 unusually far from the mean or relatively close to the mean?}
...
\Comment{TODO(David), can you put the Original on on top rather than on the bottom to be consistent with table and verbal description?}
...
\Add{Since there are no large gaps between observations around the three quartiles, adding, deleting, or changing one value, no matter how extreme that value, will have little effect on the quartiles.}
\end{example}
...
To compare two distributions, we require two data sets that we can compare with respect to center and spread.  The number of elements in each data set need not be the same (e.g. height of men versus height of women).  We must compare two univariate graphs, such as two histograms, two dot plots, parallel box plots, or a back-to-back stem-and-leaf.  To look at assocation, we require two data sets of equal length that are essentially paired (e.g. height and weight of individuals).  Such association can be seen in a scatterplot.}
\end{tipBox}
