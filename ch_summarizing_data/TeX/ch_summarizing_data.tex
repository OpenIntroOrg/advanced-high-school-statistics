...
\end{nexample}
\end{examplewrap}
\footnotetext{\url{https://data.census.gov/cedsci/table?hidePreview=true&tid=ACSST1Y2017.S1901}} 
...
\Cut{Cumulative}\Add{Combined} SAT scores are approximated well by a normal model with mean \satmean{} and standard deviation \satsd{}.
...
Finally, we should observe that it is possible for a normal random variable to fall 4,~5, or~even more standard deviations from the mean. The probability of being further than 4 standard deviations from the mean is about 1-in-15,000. For 5 and 6 standard deviations, it is about 1-in-2 million and 1-in-500 million, respectively.  \Add{However, while the tails of the normal distribution extend infinitely in either direction, our data sets are finite and normal approximation in the extreme tails is unlikely to be very accurate, even for bell-shaped data sets.}
...
\noindent Get started quickly with this \oiRedirect{desmos-normal}{Desmos calculator}.
...
\begin{examplewrap}
\begin{nexample}{Use a calculator to determine what percentile corresponds to a Z-score of 1.5.\footnotemark}
...
\item \Add{When a distribution is nearly normal, we can use the \textbf{empirical rule} (68-95-99.7 rule) and we can use \textbf{normal approximation} to approximate area/percent under a histogram using area/percent under the normal curve.}
