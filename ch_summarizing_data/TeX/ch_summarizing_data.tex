...
\sectionintro{
\noindent \Add{How do we visualize and describe the distribution of household income for counties within the United States?  What shape would the distribution have?  What other features might be important to notice? In this section we will explore techniques for
summarizing numerical variables. We will apply these techniques using the American Community Survey \data{county} data set, which was introduced in Section~\ref{dataBasics}, and a new data set \data{email50}, that comprises information on a random sample of 50 emails.  }
%%
...
We say observations are \emph{paired} when the two observations correspond to each other. In unpaired data, there is no such correspondence. In our example the two observations correspond to a particular email.
\end{nexample}\end{examplewrap}
...
\begin{figure}[h]
  \centering
\href{\oiRedirectUrl{tableau-scatterplotschoose}}{
...
\begin{figure}[bth]
   \centering
    \href{\oiRedirectUrl{tableau-histogramschoose}}{\includegraphics[width=0.82\textwidth]{ch_summarizing_data/figures/email50NumCharHist/email50NumCharHist}}
...
\Comment{don't know how to make graph not cut off the 50 label on vertical axis on Fig 2.10=}

\begin{figure}[bth]
   \centering
\href{\oiRedirectUrl{tableau-histograms}}{
...
\sectionintro{
\noindent \Add{Why are there different measures of center?  How do you know if a value is ``far" from the center?  What does it mean to an outlier?  We will continue with the \data{email50} data set and investigate multiple quantitative summarizes for numerical data.}
...
\begin{examplewrap}\begin{nexample}{In the data's context (the number of characters in emails), describe the distribution of the \var{num\_\hspace{0.3mm}char} variable shown in the histogram below.
  \begin{center}
   \includegraphics[width=0.65\textwidth]{ch_summarizing_data/figures/email50NumCharHist/email50NumCharHist}
\end{center}}
...
\begin{figure}[h]
   \centering
     \href{\oiRedirectUrl{tableau-boxplotschoose}}{ \includegraphics[width=0.9\mycaptionwidth]{ch_summarizing_data/figures/boxPlotLayoutNumVar/boxPlotLayoutNumVar}}
...
\Comment{This section updated with Desmos link}

\Add{One can use a handheld calculator or online software such as Desmos to calculate summary statistics.  More advanced statistical software packages include R (in which most of the graphs in this text were made), Python, SAS, and STATA.}

\begin{figure}[h]
   \centering
     \href{\oiRedirectUrl{textbook-desmos-1varstats}}{ \includegraphics[width=0.8\textwidth]{ch_summarizing_data/figures/email50Desmos/email50Desmos}}
\caption{Use \oiRedirect{textbook-desmos-1varstats}{this activity} to graph and find summary statistics for a single variable in Desmos, as shown in the figure.}
...
\Comment{updated this to include Desmos graph.  Link doesn't work yet, because I have made it private for now, but will work once I make it public.}

\begin{examplewrap}\begin{nexample}{Enter the following 10 data points into a calculator or into this \oiRedirect{textbook-desmos-1varstatsdiscrete}{Desmos calculator}: {5, 8, 1, 19, 3, 1, 11, 18, 20, 5}. Find the summary statistics and make a box plot of the data.}
...
\Comment{check formatting of stem-leaf, also needs to be directly with data table and with other graphs}
...
\begin{figure}
  \centering
\href{\oiRedirectUrl{tableau-medianincome-gainnogain-all}}{
...
\href{\oiRedirectUrl{tableau-intensitymapsall}}{
  ...
  \caption{\subref{countyPovertyMap} Intensity map of
      ...
      unemployment rate (percent).  Explore more dozens of intensity maps using American Community Survey data on Tableau Public\tableauhref{tableau-intensitymapsall}.}
...
%%%%%%%%%%%Section Exercises
{\input{ch_summarizing_data/TeX/numerical_summaries_and_boxplots.tex}}
...
\Add{\noindent How do we visualize and summarize categorical data?  Like numerical data, categorical data can also be organized and analyzed. In this section, we will introduce tables and other basic tools for categorical data that are used throughout this book and will answer the following questions:
...
\item Using the \data{email50} data, does email type provide any useful value in calssifying email as spam or not spam?  
...
\item Compare distributions of a categorical variable using a two-way table or a side-by-side or segmented bar~chart. 
...
A bar~chart is a common way to display a single
categorical variable.
The left panel of Figure~\ref{loan_homeownership_bar_plot}
shows a \term{bar~chart} for the \var{homeownership} variable.
...
A \termsub{stacked bar~chart}{bar chart!stacked bar chart}
\index{bar chart!segmented bar chart}
is a graphical display of contingency table information.
For example, a~stacked bar~chart representing
...
One related visualization to the stacked bar~chart is the
...
have been translated into a standardized stacked bar~chart
...
\Comment{TODO(David):  add a fourth graph to the bottom right panel that is a side-by-side bar graph using proportion.  This will make the set complete and parallel.}

\newcommand{\loanapptypehomesegbarplotwidth}{0.46\textwidth}
\begin{figure}[h]
  \centering
\href{\oiRedirectUrl{tableau-bargraphs2V}}{
  \subfigure[]{
...
%  }
  \caption{\subref{loan_app_type_home_seg_bar} Stacked
      ...
      version of the stacked bar~chart.  See these bar charts on Tableau Public\tableauhref{tableau-bargraphs2V}.}
  \label{loan_app_type_home_seg_bar_plot}
\end{figure}

\begin{examplewrap}
\begin{nexample}{Examine the three bar~charts in
    Figure~\ref{loan_app_type_home_seg_bar_plot}.
    When is the stacked, side-by-side, or standardized
    stacked bar~chart the most useful?}
  The stacked bar~chart is most useful when it's reasonable
  ...
  The standardized stacked bar chart is helpful if the primary
  variable in the stacked bar chart is relatively imbalanced,
  e.g. the \resp{own} category has only a third of the
  observations in the \resp{mortgage} category,
  making the simple stacked bar chart less useful for
...
\Comment{can you make the pie chart section start on next page to be with the pie charts?}
...
\begin{figure}[h]
  \centering
\href{\oiRedirectUrl{tableau-bargraphs1V}}{
...
\item To graphically summarize a single categorical variable, use a \term{bar~chart}.  To summarize and compare two categorical variables, use \term{side-by-side} or \term{segmented} (stacked) bar~charts.  
...
\Comment{redid the next two examples}
...
strong \Add{enough} evidence against the independence model and we assert that the vaccine had an effect in this
clinical setting.
(2)~We conclude the evidence is sufficiently strong
to reject $H_0$ and we assert that the vaccine was useful.  
\Comment{cutting this sentence and footnote as it is pretty subtle to be introduced at this moment}
...
event.\footnote{This reasoning does not generally extend
    ...
    anecdotal evidence.}}
\Add{Is 2\% small enough to make us reject the independence model?  That depends on how much evidence we require.  The smaller that probability is, the more evidence it provides against $H_0$.  Later, we will see that researchers often use a cutoff of 5\%, though it can depend upon the situation.  Using the 5\% cutoff,} we would reject the independence model in favor
of the alternative.
That is, we are concluding the data provide strong evidence
that the vaccine provides some protection against malaria
in this clinical setting. 

\Add{When there is strong enough evidence that the result points to a real difference and is not simply due to random variation, we call the result \term{significant}.  }
