...
\Comment{D - added a definition for outlier for the AP Stat exam below. Probably needs to be tweaked. I've also added some additional text and tweaked the box plot figure to say ``outlier'' rather than ``suspected outlier''. $Q_2$ has also been added to the median label in the figure.}

\begin{termBox}{\tBoxTitle{Outlier definition for the AP Statistics exam}
The AP Statistics exam defines an outlier using the box plot criteria: if an \mbox{observation} is more than $1.5 \times IQR$ above $Q_3$ or $1.5 \times IQR$ below $Q_1$, then it is an \term{outlier}, and it is marked using a dot or asterisk in a box plot.}
\end{termBox}
