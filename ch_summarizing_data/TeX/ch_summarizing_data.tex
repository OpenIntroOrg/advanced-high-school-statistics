...
\Comment{D: what was changed? I cannot see the changes referred to here via GitHub. L - subsection "Describing shape" was added.  Previously the subsection was "Histograms and shape"}
...
\Comment{D: We had gone through similar attempts early in OS on the changes being proposed in the tip box. On the 2nd and third sentences, which are new:
\begin{itemize}
\item The second sentence doesn't really say anything helpful that the students haven't already seen.
\item The third sentence is not true: the observations are not 1 SD away from the mean on average.
\end{itemize}
I'm planning to revert this change, but bouncing back if a second attempt is desired. L - in the third sentence, I didn't mean in an exact arithmetic average sense, but that is the way we ask students to think about SD - average deviation from the mean.  I'd prefer a conceptual description of SD that is separate from the empirical rule.  When I ask students to interpret the SD, I don't want them to tell me 68\% of the data falls between ave +/- 1 SD because that is not a true statement.  Even though it says in the box that this is not always true students tend to think it is.}
...
Because \$60,000 is less that one standard deviation from the mean, it is relatively close to the mean.  \Add{If the value were more than 2 standard deviations away from the mean, we would consider it far from the mean. }\Comment{I think this example can create confusion: it suggests that 1 SD is our rule for what is ``unusual'', when generally the rule of thumb is 2 SD. Planning to revert, but bouncing back in case wanting to try to revise. L- added sentence saying what you said.}
\end{example}
}
...
\Comment{D: Adding a tip box below on the proposed topic sounds good, but this box is way too long. Move some of the content out of the box, or eliminate some of the content.}

Looking at association is different than comparing distributions.  When comparing distributions we are interested in questions such as, "Which distribution has a greater average?" and "How do the shapes of the distribution differ?".  The number of elements in each data set need not be the same (e.g. height of women and height of men).  When we look at association, we are interested in whether there is a positive, negative, or no association between the variables.  This requires two data sets of equal length that are essentially paired (e.g. height and weight of individuals). 

\begin{tipBox}{\tipBoxTitle{Comparing distributions versus looking at association}{\color{red}ADD THIS BOX}
We compare two distributions with respect to center, spread, and shape.  To compare the distributions visually, we require two univariate graphs, such as two histograms, two dot plots, parallel box plots, or a back-to-back stem-and-leaf. When looking at association, we look for a positive, negative, or no relationship between the variables.  To see assocation visually, we require a scatterplot.}
\end{tipBox}
