...
\Add{Sometimes researchers wish to see the \emph{relationship} between two variables.  When we talk of a relationship or an association between variables, we are interested in how one variable behaves as the other variable increases or decreases.}

A \term{scatterplot} provides a case-by-case view of data \Add{that illustrates the relation between} two numerical variables.  In Figure~\vref{county_fed_spendVsPoverty}, a scatterplot was used to examine how federal spending and poverty were related in the \data{county} data set. Another scatterplot is shown in Figure~\ref{email50LinesCharacters}, comparing the number of line breaks (\var{line\_\hspace{0.3mm}breaks}) and number of characters (\var{num\_\hspace{0.3mm}char}) in emails for the \data{email50} data set. In any scatterplot, each point represents a single case. Since there are 50 cases in \data{email50}, there are 50 points in Figure~\ref{email50LinesCharacters}. 
...
\Add{
\begin{example}{A scatterplot requires paired data.  What does the term ``paired data" mean?}  This means that the two observations correspond to each other.  In unpaired data, there is no such correspondence.  Here the two observations correspond to a particular email.
\end{example}

The variable that is suspected to be the response variable is plotted on the vertical axis and the variable that is suspected to be the explanatory variable is plotted on the horizontal axis.  In this example, the variables could be switched since either variable could reasonably serve as the explanatory variable or the response variable.}

\begin{tipBox}{\tipBoxTitle{Drawing scatterplots}
First label both axes and draw a uniform scale on each.  Then, plot the dots.}
\end{tipBox}

\Comment{moved to next section.}
\Cut{To put the number of characters in perspective, this paragraph has 363 characters. Looking at Figure~\ref{email50LinesCharacters}, it seems that some emails are incredibly verbose! Upon further investigation, we would actually find that most of the long emails use the HTML format, which means most of the characters in those emails are used to format the email rather than provide text.}

\Add{
The association between two variables can be \termsub{positive}{positive association} or \termsub{negative}{negative association}, or there can be no association.  Positive association means that larger values of the first variable are associated with larger values of the second variable.  Additionally, the association can follow a linear trend or a curved (non-linear trend). 
...
\begin{exercise}
What does this scatterplot reveal about the email data?\footnote{The association between the number of characters in an email and the number of lines in an email is positive (when one is larger, the other tends to be larger as well).  As number of characters increases, number of lines increases is an approximately linear fashion. }
\end{exercise}
}
\Cut{
\begin{exercise}
...
\begin{exercise}
Describe two variables that would have a horseshoe shaped (i.e. ``U"-shaped) association in a scatterplot.\footnote{Consider the case where your vertical axis represents something ``good'' and your horizontal axis represents something that is only good in moderation. Health and water consumption fit this description since water becomes toxic when consumed in excessive quantities.}
\end{exercise}
...
\Add{
Sometimes two variables is one too many: only one variable may be of interest. In these cases we want to focus not on the association between two variables, but on the \term{distribution} of a single variable.  The term distribution refers to the values that a variable takes on and the frequency of these values.  Let's take a closer look at the \data{email50} data set and focus on the number of characters in each email.  To simplify the data, we will round the numbers and record the values in thousands.  Thus, 22105 is recorded as 22.   }
...
Rather than look at the data as a list of numbers, which makes the distribution difficult to discern, we will organize it into a table as follows.
...
\Add{
\begin{exercise}What is the smallest number in this data set?  What is the largest?\footnote{The smallest number is less than 1 thousand, and the largest is 64 thousand.  That is a big range!}
\end{exercise}

Many emails are short, but some are very long. An investigation of the data would reveal that most of the long emails use the HTML format, which means most of the characters in those emails are used to format the email rather than provide text.
...
\begin{figure}[h]
   \centering
   \includegraphics[width=0.7\textwidth]{02/figures/emailCharactersDotPlot/emailCharactersDotPlotStackedRounded}
   \caption{A dot plot of \var{num\_\hspace{0.3mm}char} for the \data{email50} data set.}
   \label{emailCharactersDotPlotStacked}
\end{figure}

\Add{
\begin{exercise} Imagine rotating the dot plot 90 degrees clockwise.  What do you notice?\footnote{It has a similar shape as the stem-and-leaf plot!  The values on the horizontal axis correspond the the stems and the number of dots in each interval correspond the number of leaves needed for each stem. }
\end{exercise}
...
\begin{example}{Based on both the stem-and-leaf and dot plot, where are the values clustered and where are the gaps for the \data{email50} data set? } There is a large cluster in the 0 to less than 20 thousand range, with a peak around 1 thousand.   There are gaps between 30 and 40 thousand and between the two values in the 40 thousands and the largest value of approximately 64 thousand.
\end{example}
}

\Comment{introduction of outliers presented here}
...
\Cut{Dot plots show the exact value for each observation. This is useful for small data sets, but they can become hard to read with larger samples.}\Add{stem-and-leaf plots and dot plots are ideal for displaying data from small samples because they show the exact values of the observations and how frequently they ocur.  However, they are impractical for larger samples. } Rather than showing the \Cut{value of each observation}\Add{frequency of \emph{every} value}, we prefer to think of the value as belonging to a \emph{bin}. For example, in the \data{email50} data set, we create a table of counts for the number of cases with character counts between 0 and 5,000, then the number of cases between 5,000 and 10,000, and so on. \Add{This table is called a \term{frequency table}}.  Observations that fall on the boundary of a bin (e.g. 5,000) are allocated to the lower bin.\Add{\footnote{This is called \emph{left inclusive.}}  A frequency table } \Cut{This tabulation} is shown in Table~\ref{binnedNumCharTable}. These binned counts are plotted as bars in Figure~\ref{email50NumCharHist} into what is called a \term{histogram} or \term{frequency histogram}, which resembles the stacked dot plot shown in Figure~\ref{emailCharactersDotPlotStackedRounded}.
...
\begin{figure}[bth]
   \centering
   \includegraphics[width=0.82\textwidth]{02/figures/emailCharactersDotPlot/emailCharactersDotPlotStackedRoundedHist}
...
\Comment{I cut the part of the caption that says - This distribution is very strongly skewed to the right.\index{skew!example: very strong}, since this term hasn't been introduced yet}
...
\Comment{This may seem like a lot of examples.  I feel like students actually need to practice reading histograms.  They think it is obvious, but then they make mistakes.}
\Add{
...
\begin{example}{What \emph{percent} of the emails had 10 thousand or more characters?}From the first example, we know that 31 emails had fewer than 10 thousand characters.  Since there are 50 emails in total, there must be 19 emails that  have 10 thousand or more characters.  19/50 = 0.38 = 38\%.
\end{example}

Sometimes questions such as the ones above can be answered more easily with a \term{cumulative frequency histogram}.  This type of histograms shows cumulative, or total, frequency achieved by each bin, rather than the frequency in that particular bin.  
...
\begin{figure}[bth]
   \centering
   \includegraphics[width=0.82\textwidth]{02/figures/email50NumCharHist/email50NumCharCumulativeFreqHist}
...
\end{figure}
\Comment{check numbers are correct in exercise}
...
\begin{tipBox}{\tipBoxTitle{Pay close attention to the vertical axis of a histogram}
...
\Comment{Exercise slightly reworded}
...
\begin{exercise}
What can you see in the dot plot \Add{and stem-and-leaf} that you cannot see in the frequency histogram?\footnote{Character counts for individual emails.}
\end{exercise}
...
\Comment{should omit this first exercise? or replace the caption with an exercise?}
...
\Add{
In the previous section, we saw that modes can occur anywhere in a data set.  Therefore, mode is not a measure of \term{center}.  We understand the term ``center" intuitively, but quantifying what is the center can be a little more challenging.  This is because there are distinct definitions of center.  Here we will focus on the two most common:  the mean and median. 
}
...
\end{eqnarray}
The sample mean is often labeled $\bar{x}$\marginpar[\raggedright$\bar{x}$\\\footnotesize sample\\ mean]{\raggedright$\bar{x}$\\\footnotesize sample\\ mean}. The letter $x$ is being used as a generic placeholder for the variable of interest, \var{num\_\hspace{0.3mm}char}, and the bar says it is the average number of characters in the 50 emails was 11,600. \Cut{It is useful to think of the mean as the balancing point of the distribution. The sample mean is shown as a triangle in Figures~\ref{emailCharactersDotPlot} and~\ref{emailCharactersDotPlotStacked}.}

\Comment{added sigma notation, not that it is important here, but that is how it is listed on the ap formula sheet}
...
\Comment{exercise slightly reworded because x sub i already included in term box}
\begin{exercise}
...
The sample mean, 11,600, may provide a reasonable estimate of $\mu_x$. While this number will not be perfect, it provides a \emph{point estimate} of the population mean. In Chapter~\ref{foundationsForInference} and beyond, we will develop tools to characterize the \Cut{accuracy} \Add{reliability} of point estimates, and we will find that point estimates based on larger samples tend to be more \Add{reliable} \Cut{accurate} than those based on smaller samples.
\end{example}

\Comment{We will later define accurate as  unbiased and precise as low variability}.
...
The median provides another measure of center.  The \term{median} splits \Add{an ordered data set} in half. \Cut{Figure~\ref{boxPlotLayoutNumVar} shows 50\% of the data falling below the median (dashes) and other 50\% falling above the median (open circles). }There are 50 character counts in the \data{email50} data set (an even number) so the data are perfectly split into two groups of~25. We take the median in this case to be the average of the two \Add{middle} observations \Cut{closest to the $50^{th}$ percentile}: $(\text{6,768} + \text{7,012}) / 2 = \text{6,890}$. When there are an odd number of observations, there will be exactly one observation that splits the data into two halves, and in this case that observation is the median (no average needed).

\begin{termBox}{\tBoxTitle{Median: the number in the middle \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle{Median: the number in the middle \color{red} ADD THIS BOX}
...
\Comment{want students to understand median graphically.  If they don't understand the words "half of the area is to the left/ half to the right " then can we add something to the graph to indicate this?  My sense is that, for the most part, they should be able to understand what this means.}
...
\Comment{retitle subsection - remove equal focus on variance, which, for us, is only a means to calculate sd}
\Add{\subsection{Standard deviation as a measure of spread}}
%\subsection{Variance and standard deviation}}
\label{variability}

\Add{
The U.S. Census Bureau reported that in 2012, the median family income was \$62,241 and the mean family income was \$82,743.\footnote{\urlwofont{http://www.census.gov/hhes/www/income/ }} Is a family income of \$40,000 unusually low or near the average?  

In order to answer this question, it is not enought to know the center of the data set and its \term{range} (maximum value - minimum value).  We must know about the \indexthis{variability}{variability} of the data set within that range.  Low variability or small spread means that the values tend to be more clustered together.  High variability or large spread means that the values tend to be far apart.}
\Cut{The mean \Add{and median} were introduced as methods to describe the center of a data set, but the \indexthis{variability}{variability} in the data is also important.} 

\begin{example}{Is it possible for two data sets to have the same range but different spread?  If so, give an example.  If not, explain why not.}Yes.  An example is:  {1, 1, 1, 1, 1, 9, 9, 9, 9, 9} and {1, 5, 5, 5, 5, 5, 5, 5, 5, 5, 9}.  \newline The first data set has a larger spread because values tend to be farther away from each other while in the second data set values are clustered together at the mean.  
\end{example}  

Here, we introduce \Cut{two measures of variability: the variance and the standard deviation. Both of these are very useful in data analysis, even though their formulas are a bit tedious to calculate by hand. The standard deviation is the easier of the two to understand, and it roughly describes how far away the typical observation is from the mean.} \Add{the standard deviation as a measure of spread.  Though its formula is a bit tedious to calculate by hand, the standard deviation is very useful in data analysis and roughly describes how far away, on average, the observations are from the mean.}
...
s_{}^2 &= \frac{10.1_{}^2 + (-4.6)_{}^2 + (-11.0)_{}^2 + \cdots + 4.2_{}^2}{50-1} \\
	...
	&= 172.44
\end{align*}
We divide by $n-1$, rather than dividing by $n$, when computing the variance; you need not worry about this mathematical nuance for the material in this textbook.\Add{\footnote{For a formal proof that dividing by $n-1$ makes $s^2$ an unbiased estimator of $\sigma^2$, see: \urlwofont{http://tinyurl.com/http-isites-harvard-edu}.  For an informal explanation by way of example see: \urlwofont{http://nebula.deanza.edu/~bloom/math10/m10divideby_nminus1.pdf}.}} Notice that squaring the deviations does two things. First, it makes large values much larger, seen by comparing $10.1^2$, $(-4.6)^2$, $(-11.0)^2$, and $4.2^2$. Second, it gets rid of any negative signs.

\Comment{I added the footnote url because students don't stop asking about why n-1.  They may not read/understand this proof, but at least you can tell them that it is there.}
...
\begin{termBox}{\tBoxTitle{Variance and standard deviation \color{red} CUTTING THIS TERM BOX}
...
\begin{termBox}{\tBoxTitle{Calculating the Standard Deviation \color{red} ADDING THIS TERM BOX}
...
\Add{The variance is useful for mathematical reasons, but the standard deviation is easier to interpret because it has the same units as the data set.  Variance will have units$^2$ (units-squared).}
...
\Comment{replaced instances of 70\% with 68\% (or approximately 2/3)}
\begin{figure}
\centering
\includegraphics[width=\mycaptionwidth]{02/figures/sdAsRuleForEmailNumChar/sdAsRuleForEmailHist}
...
\Comment{TODO(David) replace histogram with stacked, rounded dot plot to show 68, 95 rule?  Also, minor thing - maybe it is just me - but I found the shading color for $\pm$ 1 SD in the figure 2.14 three very different population distributions, difficult to distinguish from the color ofthe $\pm$ 2 sds.  Can inner color be made a tad darker?}
...
\Add{
\begin{example}{When describing any distribution, comment on the three important characteristics of center, spread, and shape.  Also note any especially unusual cases.  In the data's context (the number of characters in emails), describe the distribution of the \var{num\_\hspace{0.3mm}char} variable using the histogram in Figure~\vref{email50NumCharHist}. }
...
\Comment{subsection changed from Box plots, quartiles, and the median}
\subsection{Box plots and quartiles}

A \term{box plot} summarizes a data set using five summary statistics while also plotting unusual observations. Figure~\ref{boxPlotLayoutNumVar} provides a \Cut{vertical dot plot alongside a} box plot of the \var{num\_\hspace{0.3mm}char} variable from the \data{email50} data set.
...
\Add{The five summary statistics, more commonly known in statistics as the \term{five-number summary}, consist of the minimum, the maximum, and the three quartiles ($Q_1$, $Q_2$, $Q_3$) of the data set being studied.

$Q_2$\index{Q$_2$} represents the \term{second quartile}, which is equivalent to the 50th percentile (i.e. the median).  Previously, we saw that Q$_2$ or the median for the \data{email50} data set was the average of the two middle values, or ($\text{6,768} + \text{7,012}) / 2 = \text{6,890}$

$Q_1$\index{Q$_1$} represents the \term{first quartile}, or 25th percentile, and is the median of the smaller half of the data set.  There are 25 values in the lower half of the data set, so $Q_1$ is the middle value: 2,454 characters.  $Q_3$\index{$Q_3$} represents the \term{third quartile}, or 75th percentile, and is the median of the larger half of the data set: 15,829 characters.

\Comment{it would be cool to make the box plot of characters a horizontal box plot and put it above or below the stacked dot plot (or histogram).}

To determine if there are any unusually distant observations (i.e. outliers), we first calculate the spread of the middle 50\% of the data by subtracting $Q_1$ from $Q_3$.  This quantity is called the \term{interquartile range} (\hiddenterm{IQR}, for short). It, like the standard deviation, is a measure of \indexthis{variability}{variability} or \term{spread} in data. The more variable the data, the larger the standard deviation and~IQR.   }
...
\Add{
We will call a value an outlier if it is more than $1.5\times IQR$ below $Q_1$ or above $Q_3$.\footnote{While the choice of exactly 1.5 is arbitrary, it is the most commonly used value for box plots.} 
...
\begin{example}
{Compare the box plot to the graphs discussed at the beginning of this chapter: stem-and-leaf, dot plot, frequency and relative frequency histogram.  What can we learn more easily from a box plot?  What can we learn more easily from the other graphs? }It is easier to immediately identify the quartiles from a box plot.  However, a box plot, unlike the other graphs, does no show the \emph{distribution} of the data.  It merely indicates where the quartiles fall.  
\end{example}

\begin{example}
{Is it possible to identify skew from the box plot?} Yes.  Looking at the lower and upper whiskers of this box plot, we see that the first 25\% of the data is squished into a shorter distance than the last 25\% of the data, implying that there is greater density in the low values and a tail trailing to the upper values.  This box plot is right skewed.  
\end{example}

\begin{exercise}
What percent of the data fall between $Q_1$ and the median? What percent is between the median and $Q_3$?\footnote{ Since $Q_1$ is the 25th percentile and $Q_2$ is the 50th percentile, 25\% of the data falls between $Q_1$ and $Q_2$.  Similarly, 25\% of the data fall between $Q_2$ and $Q_3$.}
\end{exercise}


\begin{exercise}
Given a histogram, can one sketch a box plot of the data?  Given a box plot, can one sketch a histogram of the data?\footnote{Given a histogram, one can visually estimate the quartiles, making it possible to sketch a box plot.  However, given a box plot, there is no way to know the \emph{distribution} of the data within the quartiles, making it impossible to sketch a histogram.}
\end{exercise}

}
\Cut{The first step in building a box plot is drawing a dark line denoting the \term{second quartile}.  The second quartile is labeled $Q_2$\index{Q$_2$} and is equivalent to the median, which splits the data in half. Figure~\ref{boxPlotLayoutNumVar} shows 50\% of the data falling below the median (dashes) and other 50\% falling above the median (open circles). There are 50 character counts in the data set (an even number) so the data are perfectly split into two groups of~25. We take the median in this case to be the average of the two observations closest to the $50^{th}$ percentile: $(\text{6,768} + \text{7,012}) / 2 = \text{6,890}$. When there are an odd number of observations, there will be exactly one observation that splits the data into two halves, and in this case that observation is the median (no average needed).
}
\Comment{term box moved to previous section}
...
\begin{termBox}{\tBoxTitle{Outliers are extreme \color{red} CUT THIS BOX}
An \term{outlier} is an observation that appears extreme relative to the rest of the data.}
\end{termBox}


\begin{tipBox}{\tipBoxTitle{Why it is important to look for outliers \color{red} CUT THIS BOX}
Examination of data for possible outliers serves many useful purposes, including\vspace{-2mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\Comment{TODO(David)  create a horizontal box plot using different data set }
...
\Cut{Knowing how to calculate summary statistics such as standard deviation and quartiles by hand helps us better understand what these quantities \emph{represent} and how to interpret them.  However, in practice, researchers utilize statistical software to perform these calculations.\footnote{Common statistical applications include R, SAS, and Minitab}.  Because most students have access to a TI-83/84 graphing calculator, we will provide instruction for how to perform common calculations using a calculator.  }

\Comment{TODO(David) add icon for video tutorial}
...
\Add{Calculating the summary statistics will return the following information. It will be necessary to hit the down arrow to see all of the summary statistics.
...
n & (sample size or number of data points)\\
minX & (minimum) \\
Q$_1$ & (first quartile) \\
Med & (median) \\
Q$_3$ & (third quartile) \\
...
\begin{tipBox}{\tipBoxTitle{What to do if L1 or other list does not appear on TI calculator\color{red}  ADD THIS BOX}
\begin{enumerate}
\item Press STAT.
...
\Comment{TODODavid(redo horizontal box plot in R)}
...
\begin{figure}[ht]
\centering
\includegraphics[width=50mm]{02/figures/TI/exampleBoxPlot}
\label{exampleBoxPlot}
\end{figure}
\end{example}
}

\subsection{Outliers and Robust statistics}
\Comment{make this section Outliers and robust statistics?}

While it is best to identify outliers visually from a graph, there are two common rules of thumb for identifying outliers numerically.

\Comment{unfortunately, they must memorize these and use them for ap exam}
...
\Comment{let's replace with three stacked dot plots or rename graph to not call it it a dot plot}
...
Suppose that a researcher is looking at a list of 50 temperatures recorded in Celsius (C).  The mean is given as 27\degree C with a standard deviation of 3\degree C.  Because she is not familiar with the Celsius scale, she would like to convert these summary statistics into Fahrenheit (F).  To convert from Celsius to Fahrenheit we use this conversion:
\begin{align*}
x_{\degree F} = \frac{9}{5}x_{\degree C} + 32
\end{align*}
Fortunately, she does not need to convert each of the 50 temperatures to Fahrenheit and then recalculate the mean and the standard deviation.  The unit conversion above is a linear transformation of the form 
\begin{align*}
aX + b
\end{align*}
where $a=9/5$ and $b=32$.  

\Comment{TODO(David) add picture}

\begin{example}{Let us take a simpler example.  Begin with the list:  {1, 1, 5, 5}.  Multiply all of the numbers by 10.  What happens to the mean?  What happens to the standard deviation?  How do these compare to the mean and the standard deviation of the original list?} The original list has a mean of 3 and a standard deviation of 2. The new list: {10, 10, 50, 50} has a mean of 30 with a standard deviation of 20.  Because all of the values were multiplied by 10, both the mean and the standard deviation were multiplied by 10.~\footnote{Here, the population standard deviation was used in the calculation. These properties can be proven mathematically using properties of sigma (summation).}
\end{example}
...
\begin{termBox}{\tBoxTitle{Adding shifts the values, multiplying stretches or contracts them \color{red} ADD THIS BOX}
...
\Add{
\begin{example}{Consider the temperature example.  How would converting from Celsuis to Fahrenheit affect the median?  The IQR?}The median is affected in the same way as the mean and the IQR is affected in the same way as the standard deviation.  To get the new median, multiply the old median by $9/5$ and add 32.  To get the new IQR, note that $Q_1$ and $Q_3$ will be affected in the same way as the median, but when you subtract $Q_1$ from $Q_3$ the 32 will cancel out.  Therefore the IQR will be multiplied by $9/5$ but will be unaffected by the addition of 32.\footnote{new IQR = $(9/5Q_3 + 32) - (9/5Q_1 + 32) = 9/5(Q_3 - Q_1)= 9/5$(old IQR)} 
\end{example}
...
\Comment{TODO(David) add back-to-back stem-and-leaf}

Some of the more interesting investigations can be considered by examining numerical data across groups. The methods required here aren't really new. All that is required is to make a numerical plot for each group.  \Add{To make a direct comparison between two groups, one could make a pair of dot plots or a pair of histograms drawn using the same scales.  Also common are the back-to-back stem-and-leaf plot, parallel box plots, and hollow histograms, the three of which are explored here.}
...
\begin{tipBox}{\tipBoxTitle{Comparing distributions \color{red} ADD THIS BOX}
...
\Add{
\begin{exercise}
Do these graphs tell us about any association between income for the two groups?\footnote{No, to see assocation we require a scatterplot.  Moreover, this data is not paired so the discussion of assocation does not make sense here.}
\end{exercise}
...
\Comment{these terms have now been defined previously, so should we change this?}
...
\Add{The numbers in these tables correspond to counts, not to data points, therefore they cannot be graphed using methods from the previous section.  Instead, we need graphs suitable for categorical data.} 
...
\Comment{Removed mosaic plots}
\subsection{Segmented bar plots}
%\subsection{Segmented bar and mosaic plots}
\label{segmentedBarPlotsAndIndependence}

Contingency tables using row or column proportions are especially useful for examining how two categorical variables are related. Segmented bar\Cut{and mosaic} plots provide a way to visualize the information in these tables.
