...
\begin{termBox}{\tBoxTitle[]{Steps for using Confidence Intervals}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\subsection{Calculating Power and the Type 2 Error rate (special topic)}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{04/figures/power132And141/power132And141}
...
The hypotheses were set up and the conditions were checked in Guided Practice~\ref{ebayAmazonOneSidedTestExercise} and~\ref{exerciseFor52EbayAuctionsToExamineMarioKartLessExpensiveThanAmazonConditions}. The next step is to find the standard error of the sample mean and produce a sketch to help find the p-value.
\begin{eqnarray*}
...
\begin{example}{The second college randomly samples 72 students and finds a mean of $\bar{x} = 6.83$ hours and a standard deviation of $s=1.8$ hours. Does this provide strong evidence against $H_0$ in Guided Practice~\ref{2ndSchSleepHypSetupExercise}? Use a significance level of $\alpha=0.05$.}
...
\begin{example}{The sample mean for student housing is \$611.63 and the sample standard deviation is \$132.85. Construct a 95\% confidence interval for the population mean and evaluate the hypotheses of Guided Practice~\ref{htForHousingExpenseForCommunityCollege650}.}
...
%__________________
\Comment{section ``examing central limit theorem" moved to chapter dealing with means"}
\Comment{cut section ``inference for other estimators" and its subsections}
%__________________
...
\begin{example}{In Guided Practice~\vref{pointEstimateOfDifferentNetTimesBetweenGender}, we computed a point estimate for the average difference in run times between men and women: $\bar{x}_{women}-\bar{x}_{men}=14.48$ minutes. This point estimate is associated with a nearly normal distribution with standard error $SE=2.78$ minutes. What is a reasonable 95\% confidence interval for the difference in average run times?}
