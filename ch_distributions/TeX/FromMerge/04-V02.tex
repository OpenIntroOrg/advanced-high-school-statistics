...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{04/figures/simpleNormal/simpleNormal}
...
\begin{figure}[hht]
\centering
\includegraphics[width=0.9\textwidth]{04/figures/twoSampleNormals/twoSampleNormals}
...
\begin{figure}[hht]
\centering
\includegraphics[width=0.62\textwidth]{04/figures/twoSampleNormalsStacked/twoSampleNormalsStacked}
...
\begin{figure}
\centering
\includegraphics[width=65mm]{04/figures/satActNormals/satActNormals}
...
\begin{figure}[hht]
\centering
\includegraphics[height=1.9in]{04/figures/6895997/6895997}
...
\begin{figure}[htb]
   \centering
   \includegraphics[height=1.5in]{04/figures/satBelow1800/satBelow1800}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{04/figures/normalTails/normalTails}
...
\begin{center}
\includegraphics[height=0.9in]{04/figures/satAbove1630/satAbove1630}
\end{center}
...
\begin{center}
\includegraphics[height=0.8in]{04/figures/subtractingArea/subtractingArea}
\end{center}
...
\begin{center}
\includegraphics[height=22mm]{04/figures/satBelow1400/satBelow1400}
\end{center}
...
\includegraphics[height=12mm]{04/figures/satBelow1400/satAbove1400}}
\end{exercise}
...
\begin{exercise}
Mike is 5'7'' and Jim is 6'4''. (a) What is Mike's height percentile? (b) What is Jim's height percentile? Also draw one picture for each part.\footnote{First put the heights into inches: 67 and 76 inches. Figures are shown below. (a) $Z_{Mike} = \frac{67 - 70}{3.3} = -0.91\ \to\ 0.1814$. (b) $Z_{Jim} = \frac{76 - 70}{3.3} = 1.82\ \to\ 0.9656$. \\\includegraphics[height=12mm]{04/figures/mikeAndJimPercentiles/mikeAndJimPercentiles}}
\end{exercise}
...
\begin{center}
\includegraphics[height=22mm]{04/figures/height40Perc/height40Perc}\vspace{-1mm}
\end{center}
...
\begin{center}
\includegraphics[height=22mm]{04/figures/height82Perc/height82Perc}\vspace{-1mm}
\end{center}
...
\begin{center}
\includegraphics[height=0.85in]{04/figures/between59And62/between59And62}
\end{center}
...
\begin{center}
\includegraphics[height=0.65in]{04/figures/subtracting2Areas/subtracting2Areas}
\end{center}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{04/figures/fcidMHeights/fcidMHeights}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{04/figures/normalExamples/normalExamples}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{04/figures/nbaNormal/nbaNormal}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{04/figures/pokerNormal/pokerNormal}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{04/figures/normalQuantileExer/normalQuantileExer}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{04/figures/normalQuantileExer/normalQuantileExerAdditional}
...
\Comment{deleted age histogram, just use time}

These data are special because they include the results for the entire population of runners who finished the 2012 Cherry Blossom Run. We took a simple random sample of this population, which is represented in Table~\ref{run10SampDF}. \Cut{We will use this sample, which we refer to as the \data{run10Samp} data set, to draw conclusions about the entire population. This is the practice of statistical inference in the broadest sense. } A histogram summarizing the time variable in the \data{run10Samp} data set is shown in Figure~\ref{run10SampHistograms}.
...
\Comment{added estimated sd}

% WARNING: This figure is referenced in Section 4.2
\begin{figure}
\centering
\includegraphics[width=\textwidth]{05/figures/run10SampHistograms/run10SampHistograms} 
...
\Comment{added to caption ``This histogram approximates the true sampling distribution of the sample mean, with mean $\mu_{\bar{x}}$ and standard deviation $\sigma_{\bar{x}}$."}

\begin{figure}
   \centering
   \includegraphics[width=0.85\textwidth]{05/figures/netTime1000SamplingDistribution/netTime1000SamplingDistribution}
...
\begin{termBox}{\tBoxTitle{Standard deviation of an estimate \color{red} ADD THIS BOX}
...
\Cut{
When considering the case of the estimate $\bar{x}$, there is one problem: there is no obvious way to estimate its standard deviation from a single sample. However, statistical theory provides a helpful tool to address this issue. 
}
\Add{
...
\Cut{
\begin{exercise}
In the sample of 100 runners, the standard deviation of the runners' ages is $s_y = 8.97$. Because the sample is simple random and consists of less than 10\% of the population, the observations are independent. (a)~What is the standard deviation of the sample mean, $\bar{y}=35.05$ years? (b)~Would you be surprised if someone told you the average age of all the runners was actually 36~years?\footnote{(a) Use Equation~(\ref{seOfXBar}) with the population standard deviation to compute the standard deviation of the sample mean: $SD_{\bar{y}} = 8.97/\sqrt{100} = 0.90$ years. (b) It would not be surprising. Our sample is about 1 standard error from 36 years. In other words, 36 years old does not seem to be implausible given that our sample was relatively close to it. (We use the standard deviation to identify what is close.)}
\end{exercise}
}
%library(openintro); library(xtable); data(run10); data(run10Samp); mean(run10Samp$age); sd(run10Samp$age); sd(run10$age, na.rm=TRUE)
\Add{
\begin{exercise}
The average of the runners' ages is 35.05 years with a standard deviation of $\sigma = 8.97$. A simple random sample of 100 runners is taken.  (a)~What is the standard deviation of the sample mean? (b)~Would you be surprised to get a sample of size 100 with an average of 36~years?\footnote{(a) Use Equation~(\ref{seOfXBar}) with the population standard deviation to compute the standard deviation of the sample mean: $SD_{\bar{y}} = 8.97/\sqrt{100} = 0.90$ years. (b) It would not be surprising. 36 years is about 1 standard deviation from the true mean of 35.05. Based on the 68, 95 rule, we would get a sample mean at least this far away from the true mean approximately 32\% of the time.}
\end{exercise}
}
...
\Comment{TODO(David), could you replace the log-normal example with a normal population so that students can see that if population is normal, sampling distribution is normal for any n.  There's probably not room, but if there is - a distribution with peaks at the min and max would be a neat example.}
...
$$P(Z < -0.504) = 0.3072$$\\
There is a 30.72\% probability that a randomly selected runner will complete the run is less than 90 minutes.
\end{example}

\begin{example}{
Find the probabiliy that the average of 20 runners is less than 90 minutes.}Here, $n=20<30$, but the distribution of the population, that~is, the distribution of run times is stated to be approximately normal.  Because of this, the sampling distribution will be normal for any sample size.  
$$SD_{\bar{x}}=\frac{\sigma}{\sqrt{n}}=\frac{8.97}{\sqrt{20}}=2.01$$ \\
$$Z = \frac{x - \mu}{\sigma}=\frac{90-94.52}{2.01}=-2.25$$\\
$$P(Z < -0.504) = 0.0123$$\\
There is a 1.23\% probability that a randomly selected runner will complete the run is less than 90 minutes.
\end{example}
...
\begin{exercise}
What is the probability that the average of 50 randomly selected runners is greater than 37 years?\footnote{Because $n=50\ge 30$, the sampling distribution of the mean is approximately normal, so we can use normal approximation for this problem.  The mean is given as 35.05 years.  The standard deviation of the mean is $SD_{\bar{x}}=\frac{\sigma}{\sqrt{n}}=\frac{8.97}{\sqrt{50}}=1.27$.\\
$z=\frac{\bar{x}-\mu}{SD_{\bar{x}}} = \frac{37-35.05}{1.27}=1.535$\\
$P(Z > ) = 0.062$
\newline
...
When finding the probability that an \emph{average} or mean is greater or less than a particular value, remember to divide by $\sqrt{n}$ when calculating the SD.  }
\end{tipBox}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{04/figures/geometricDist35/geometricDist35}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{04/figures/fourBinomialModelsShowingApproxToNormal/fourBinomialModelsShowingApproxToNormal}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{04/figures/normApproxToBinomFail/normApproxToBinomFail}
...
\Comment{Add a ``Central iimit theorem revisited" subsection?  This would explain that any sum of independent r.v. because normal for large n, which implies that a mean and a proportion become normal for large n as well.  At any rate, should connect the fact with we can do normal approximation on $\hat{p}$ with the central limit theorem and the normal approximation we did on $\bar{x}$.}

\subsection{Normal approximation for the sampling distribution of $\hat{p}$}
