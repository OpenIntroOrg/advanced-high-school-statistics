...
\Comment{changed: while not perfect for any single problem, is very useful for a variety of problems}
...
$Z$\vspace{1mm}\\\footnotesize Z score, the\\standardized\\observation}\index{Z@$Z$} of an observation is defined as the number of standard deviations it falls above or below the mean. If the observation is one standard deviation above the mean, its Z score is 1. If it is 1.5 standard deviations \emph{below} the mean, then its Z score is -1.5. If $x$ is an observation from a \Cut{normal }distribution with mean $\mu$ and standard deviation $\sigma$, we define the Z score mathematically as
\begin{eqnarray*}
Z = \frac{x-\mu}{\sigma}
\end{eqnarray*}
...
\begin{exercise}
Let $X$ represent a random variable from a \Cut{normal }distribution with $\mu = 3$ and $\sigma = 2$, and suppose we observe $x=5.19$. (a) Find the Z score of $x$. (b) Use the Z score to determine how many standard deviations above or below the mean $x$ falls.\footnote{(a) Its Z score is given by $Z = \frac{x-\mu}{\sigma} = \frac{5.19 - 3}{2} = 2.19/2 = 1.095$. (b) The observation $x$ is 1.095 standard deviations \emph{above} the mean. We know it must be above the mean since $Z$ is positive.}
\end{exercise}
...
\begin{termBox}{\tBoxTitle{TI calculator:  1. Finding areas under normal curve\vspace{0.5mm}}
...
\Comment{TODO: DAVID, please add title: Distribution of Population of Run Times}
...
\Comment{TODO: DAVID, please add title: Sampling Distribution of Sample Means for n = 100.  Also, can you shrink the horizontal axis, to emphasize that the variability in this distribution is must smaller than the variability of the original population?}
...
\begin{termBox}{\tBoxTitle{Sampling distribution \color{red}ADD THIS BOX}
A sampling distribution represents the distribution of a point estimate based on samples of a fixed size from a certain population. The distribution of a point estimate has a certain mean and standardard deviation, which depend upon, though are not always identical to, the mean and standard deviation of the population from which the samples were taken.  Understanding the concept of a sampling distribution is central to understanding statistical inference.}
\end{termBox}

\begin{termBox}{\tBoxTitle{Sampling distribution \color{red}CUT THIS BOX}
...
The sampling distribution shown in Figure~\ref{netTime1000SamplingDistribution} \Add{shows the various values of the sample mean obtained from repeated samples of size 100 from the population.}  \Cut{is unimodal and approximately symmetric.} \Add{We denote the mean of the sampling distribution of a sample mean as $\mu_{\bar{x}}$. The sampling distribution of a sample mean is centered exactly at the true population mean: $\mu=94.52$. Intuitively, this makes sense. The sample means should average out to the population mean.  In other words, the sample mean is an unbiased estimator of the population mean.  Mathematically, $\mu_{\bar{x}}=\mu$.}

We can see that the sample mean has some variability around the population mean, which can be quantified using the standard deviation of this distribution of sample means.  \Cut{$\sigma_{\bar{x}} = 1.59$. }The standard deviation of the sample mean tells us how far the typical estimate is away from the actual population mean, 94.52 minutes.  It also describes the typical \term{error} of a single estimate, and is denoted by the symbol $\sigma_{\bar{x}}$. 
...
\Add{In Figure~\ref{netTime1000SamplingDistribution}, we note that the sampling distribution of the sample mean looks approximately normally distributed.  Will the sampling distribution of a mean always be approximately normal?  Will it depend on the shape of the population?  To address these questions, }we will investigate three cases to see roughly when the approximation is reasonable.
...
\Comment{TODO(David) Could you put the normal first, followed by uniform, followed by exponential?  Also, is there any way you could keep the scales more constant so that we can see the shrinking sd's??}
...
-If the population is normal, the sampling distribution of $\bar{x}$ will be normal for any sample size (including a sample size of 1!).  
\\-The less normal the population, the larger $n$ needs to be for the sampling distribution of $\bar{x}$ to be normal.  However, a good rule of thumb is that for almost all populations, the sampling distribution of $\bar{x}$ will be approximately normal if $n\ge 30$.}
\end{termBox}

\Add{This brings us to the \term{Central Limit Theorem}, the most fundamental theorem in Statistics.  }

\begin{termBox}{\tBoxTitle{Central Limit Theorem}
When taking a random sample of independent observations from a population with fixed mean and standard deviation, the distribution of $\bar{x}$ approaches the normal distribution as $n$ increases.  }
\end{termBox}
...
\Comment{I had to change this from 50 observations to 20 observations, otherwise it is inconsistent with the rule of the thumb just presented about n being at least 30}
\Comment{TODO{DAVID}:  please change histogram below to correspond to 20 observations, rather than to 50 observations}
...
\end{itemize}
Since we should be skeptical of the independence of observations and the very extreme upper outlier poses a challenge, we should not use the normal model for the sample mean of these 20 observations. If we can obtain a much larger sample, \Cut{perhaps several hundred observations,} then the concerns about skew and outliers would no longer apply.
\end{example}
...
\begin{caution}
{Watch out for strong skew and outliers\color{red}ADD THIS BOX}
{Strong skew in the population is often identified by the presence of clear outliers in the data. If a data set has prominent outliers, then a larger sample size will be needed for the sampling distribution $\bar{x}$ to be normal.  There are no simple guidelines for what sample size is big enough for each situation.  However, we can use the rule of thumb that, in general, an $n$ of at least 30 is sufficient for most cases.}
\end{caution}
\index{skew!strongly skewed guideline}

\begin{caution}
{Watch out for strong skew and outliers\color{red}CUT THIS BOX}
...
\subsection{Normal approximation for the sampling distribution of a sample mean $\bar{x}$}

\Cut{We have seen that when the sample size is at least 30 and the observations are independent, we can apply the normal model for the sampling distribution of $\bar{x}$.}
\Add{At the beginning of this chapter, we used normal approximation for populations or for data that had an approxmately normal distribution.  When appropriate conditions are met, we can also use normal approximation to estimate probabilities about a sample average.  We must remember to verify that the conditions are met and use the mean and standard deviation for the sampling distribution of the sample average.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{ch_distributions/figures/smokingFriends4/smokingFriends4}
...
% The following two exercises were omitted since the distribution isn't really skewed in the example above, so these questions didn't really make sense
%
%\Add{
%\begin{exercise}Describe the graph of the binomial distribution if $p$ had been 0.5. Let $n$ remain 40. \footnote{It would be symmetric. The mean would be $40\times 0.5 = 20$ and the standard deviation would be $ \sqrt{40\times 0.5\times 0.5} = 3.16$.}
%\end{exercise}
%}
%
%\Add{
%\begin{exercise}Describe the graph of the binomial distribution if $p$ had been 0.7. Let $n$ remain 40?\footnote{The mean would be $40\times 0.7 = 28$ and the standard deviation would be $ \sqrt{40\times 0.7\times 0.3} = 2.9$. The graph would be left-skewed. In fact, is is the same as the graph with $n=4$ and $p=0.3$ but flipped horizontally.}
%\end{exercise}
%}

% The following paragraph was basically an intro to the next section.
%It is useful to consider what happens to the binomial distribution as $n$ and $p$ change. We will soon see that when $p=0.5$, as $n$ increases, the distribution becomes more normal. Surprisingly, for any fixed value of $p$, as $n$ becomes very large, the distribution will become more normal! The next section will introduce a rule of thumb for when the binomial distribution can be well approximated by a normal distribution.
...
The binomial formula is cumbersome when the sample size ($n$) is large, particularly when we consider a range of observations. \Add{Image finding the probability that at least 25 of 40 somoking friends will develop a sever lung condition.  We would need to use the binomial formula with k=25, k=26, k=27,...,k=40.  That's a lot of work!  }In some cases we may use the normal distribution as an easier and faster way to estimate binomial probabilities.  \Add{While a normal approximation for the distribution in Figure~\ref{smokingFriends4} would not be appropriate, it would not be too bad for the distribution in Figure~\ref{smokingFriends40}.}
...
In analyses, we think of the formula for the standard deviation of a sample proportion, $\sigma_{\hat{p}}$, as describing the uncertainty associated with the estimate $\hat{p}$. That is, $\sigma_{\hat{p}}$ can be thought of as a way to quantify the typical \hiddenterm{error} in our sample estimate $\hat{p}$ when used as an estimate of the true proportion $p$. Understanding the variability of statistics such as $\hat{p}$ is a central component in the study of statistics.
...
\sigma_{\hat{p}}
	...
	= .02
\end{align*}
On average, the sample proportion will be about 0.02 or 2\% away from the true proportion of $p = 0.20$. We'll become more rigorous about quantifying how close $\hat{p}$ will tend to be to $p$ in Chapter~\ref{foundationsForInference}.
\end{example}
