...
\includegraphics[height=12mm]{ch_distributions/figures/satBelow1030/satAbove1030}}
...
\item Even though the geometric distribution has an infinite number of values, it has a well-defined \term{mean}, $\mu=\frac{1}{p}$.  If the probability of success is $\frac{1}{10}$, then \emph{on average}, it takes 10 trials until we see the first success.
...
\hline
\end{tabular}
\includegraphics[width=0.7\textwidth]{ch_distributions/figures/oPositive4/oPositive4}
\label{oPositive4}
...
\begin{examplewrap}
\begin{nexample}{If the probability that a person has blood type O+ is $0.35$ and you have 40 randomly selected people, about how many would you expect to have blood type O+? What is the standard deviation of the number of people who would have blood type O+ among the 40 people?}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{ch_distributions/figures/oPositive40/oPositive40}
\caption{Distribution for the number of people with blood type O+ in a random sample of size 40, where $p=0.35$.  The distribution is binomial and is centered on~14 with a standard deviation of 3.  }
\label{oPositive40}
\end{figure}
...
This is equivalent to asking, what is the probability of observing $k=0, 1, 2, ..., \text{ or } 11$ with blood type O+ in a sample of size 40 when $p=0.35$?  We previously verified that this scenario is binomial.  We can compute each of the 12 probabilities using the binomial formula and add them together to find the answer:
 \begin{align*}
  &P(k=0\text{ or }k=1\text{ or }\cdots\text{ or } k=11) \\
	&\qquad = P(k=0) + P(k=1) + \cdots + P(k=11) \\
	&\qquad = {40\choose 0}(0.35)^0(0.65)^{40} + {40\choose 1}(0.35)^1(0.65)^{39} + \cdots + {40\choose 11}(0.35)^{11}(0.65)^{29}\\
	&\qquad = 0.21
  \end{align*}
...
The computations in Example~\ref{exactbin} are tedious and long.  In general, we should avoid such work if an alternative method exists that is faster, easier, and still accurate. Recall that calculating probabilities of a range of values is much easier in the normal model. In some cases we may use the normal distribution to estimate binomial probabilities. While a normal approximation for the distribution in Figure~\ref{oPositive4} would not be appropriate, it might not be too bad for the distribution in Figure~\ref{oPositive40}.  We might wonder, when is it reasonable to use the normal model to approximate a binomial distribution?
...
\Comment{guide practice 4.51 needs to be on same page as figure}
...
\begin{examplewrap}
\begin{nexample}{Use the normal approximation to estimate the probability of observing fewer than 12 with blood type O+ in a random sample of 40, if the true proportion with blood type O+ in the population is $p=0.35$.}\label{approxBinomialForN40P35OPosExample}
First we verify that $np$ and $n(1-p)$ are at least 10 so that we can apply the normal approximation to the binomial model:
\begin{align*}
np&=40(0.35)=14\ge 10
&
n(1-p)&=40(0.65)=26\ge 10
\end{align*}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{ch_distributions/figures/bloodtypeOpos/bloodtypeOpos40}
\end{center}
...
\begin{examplewrap}
\begin{nexample}{Use the normal approximation to estimate the probability of observing fewer than 120 people with blood type O+ in a random sample of 400, if the true proportion with blood type O+ in the population is $p=0.35$.}\label{approxBinomialForN400P20SmokerExample}
We have previously verified that the binomial model is reasonable for this context. Now will verify that both $np$ and $n(1-p)$ are at least 10 so we can apply the normal approximation to the binomial model:
\begin{align*}
np&=400(0.35)=140\ge 10
&
n(1-p)&=400(0.65)=260\ge 10
\end{align*}
...
\begin{align*}
\mu &= np = 400(0.35)=140\\
\sigma &= \sqrt{np(1-p)} = \sqrt{400(0.35)(0.65)}= 9.5
\end{align*}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{ch_distributions/figures/bloodtypeOpos/bloodtypeOpos400}
\end{center}
...
\end{nexercise}
\end{exercisewrap}
\footnotetext{$np=100\times 1/6=16.7\ge 10$ and 
...
\Comment{Is there any chance you could redo this graph n=40, p=.35 and again with n=400, p=.35?  It seems unnecessary it introduce another example scenario here.  Also, I think the important thing is using the continuous to estimate the discrete, not the small interval.  If the interval is small, but n is very large, the approximation will be fine.  If n is on the small side, the continuity correction will be a help, whether the interval is narrow or wide.
}

Suppose 20\% of a population identify as Independent.  We wanted to compute the probability of observing 69, 70, or 71 people who identify as Independent in a random sample of 400 people when $p=0.20$. With such a large sample, we might be tempted to apply the normal approximation and use the range 69 to 71. However, we would find that the binomial solution and the normal approximation notably differ:
\begin{align*}
\text{Binomial:}&\ 0.0703
&\text{Normal:}&\ 0.0476
\end{align*}
...
\begin{examplewrap}
\begin{nexample}{Use the method described to find a more accurate estimate for the probability of observing 69, 70, or 71 people who identify as Independent in 400 randomly selected people when $p=0.20$.}
...
Here, $n=40$ and $p=0.35$, $\sigma_{\hat{p}} = \sqrt{\frac{0.35(1-0.35)}{40}}=0.075$. In Figure~\ref{oPositive40prop} we see that the distribution of number of people in sample with blood type O+ out of 40 is equivalent to the distribution of proportion of people in a sample of size 40 with blood type 0+, but with a change of scale.  Instead of number along the horizontal axis, we have proportion.  

\begin{figure}[ht]
\centering
\subfigure[]{
\includegraphics[width=0.45\textwidth]{ch_distributions/figures/oPositive40/oPositive40}
}
\subfigure[]{
\includegraphics[width=0.45\textwidth]{ch_distributions/figures/oPositive40/oPositive40prop}
}
