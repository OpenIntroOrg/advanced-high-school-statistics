...
\chapterintro{In this chapter, we consider sampling distributions for four common statistics:  sample proportion, difference of sample proportions, sample mean, and difference of sample means.  In each case, the goal is the same - to describe the center, spread, and shape of the sampling distribution and determine when normal approximation to the sampling distribution will be reasonable.   }
...
\begin{itemize}
\item Given a population that is 50\% male, what is the probability that a random sample of 200 people would consist of greater than 55\% males?
...
\item The symbol $\hat{p}$ denotes the sample proportion.  $\hat{p}$ for any particular sample is a number.  However, $\hat{p}$ can vary from sample to sample.  The distribution of all possible values of $\hat{p}$ for repeated samples of a fixed size from a certain population is called the \term{sampling distribution} of $\hat{p}$.
...
\noindent This chapter began by introducing the idea of sampling distribution.  As with any distribution, we can summarize a sampling distribution with regard to its center, spread, and shape.  A common thread that ran through this chapter is the application of \term{normal approximation} (introduced in Section~\ref{normalDist}) to various sampling distributions.
\\
...
\item Normal approximation for the \term{sum or difference of two independent random variables}:
...
\item Normal approximation for the \term{difference of sample proportions}:
\\- Verify that observations can be treated as independent, within and between groups, and that $n_1p_1\ge 10$, $n_1(1-p_1)\ge 10$,  $n_2p_2\ge 10$, and $n_2(1-p_2)\ge 10$\\- Use $\mu_{\hat{p}_1-\hat{p}_2}=p_1-p_2$ and $\sigma_{\hat{p}_1-\hat{p}_2}=\sqrt{\frac{p_1(1-p_1)}{n_1}+\frac{p_2(1-p_2)}{n_2}}$ to find the Z-score for the given difference of sample proportions.

\item Normal approximation for the \term{difference of sample means}:\\
- Verify that observations can be treated as independent, within and between groups, and that both populations are approximately normal or both sample sizes are at least 30.\\
- Use $\mu_{\bar{x}_1-\bar{x}_2}=\mu_1-\mu_2$ and $\sigma_{\bar{x}_1-\bar{x}_2}=\sqrt{\frac{{\sigma_1}^2}{n_1}+\frac{{\sigma_2}^2}{n_2}}$ to find the Z-score for the given difference of sample means.
\end{enumerate}

\noindent Cases 3, 4, and 7 apply to \term{numerical} variables, while cases 1, 2 and 6 are for \term{categorical} yes/no variables.  Case 5 applies to both numerical and categorical variables.
\\
