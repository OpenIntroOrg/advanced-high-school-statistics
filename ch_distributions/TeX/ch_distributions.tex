...
$Z$\vspace{1mm}\\\footnotesize Z score, the\\standardized\\observation}\index{Z@$Z$} of an observation is defined as the number of standard deviations it falls above or below the mean. If the observation is one standard deviation above the mean, its Z score is 1. If it is 1.5 standard deviations \emph{below} the mean, then its Z score is -1.5. If $x$ is an observation from a normal distribution with mean $\mu$ and standard deviation $\sigma$, we define the Z score mathematically as
\begin{eqnarray*}
Z = \frac{x-\mu}{\sigma}
\end{eqnarray*}
...
\begin{exercise}
Let $X$ represent a random variable from a normal distribution with $\mu = 3$ and $\sigma = 2$, and suppose we observe $x=5.19$. (a) Find the Z score of $x$. (b) Use the Z score to determine how many standard deviations above or below the mean $x$ falls.\footnote{(a) Its Z score is given by $Z = \frac{x-\mu}{\sigma} = \frac{5.19 - 3}{2} = 2.19/2 = 1.095$. (b) The observation $x$ is 1.095 standard deviations \emph{above} the mean. We know it must be above the mean since $Z$ is positive.}
\end{exercise}
...
\begin{example}{Larry believes he can get into his preferred college if he scores at least in the 80th percentile on the SAT. What score should he aim for?}
...
\begin{exercise}Mary scored at the 72nd percentile on the SAT. What was her SAT score?\footnote{First, draw a picture! The closest percentile in the table to 0.72 is 0.7190, which corresponds to $Z = 0.58$. Next, set up the Z score formula and solve for $x$: $0.58 = \frac{x-1500}{300} \rightarrow x = 1674$. Mary scored 1674.}
\end{exercise}
...
%\CommentA{TODO(David), WOULD BE NICE Could you put the normal first, followed by uniform, followed by exponential?  Also, is there any way you could keep the scales more constant so that we can see the shrinking sd's??}
...
However, the more non-normal a population distribution, the larger a sample size seems is necessary for the sampling distribution to look nearly normal.
\end{example}
...
\item[(2)] The sample size is 50, which is pretty large.
...
When finding the probability that an \emph{average} or mean is greater or less than a particular value, remember to divide the standard deviation of the data by $\sqrt{n}$ to calculate the correct~SD.}
\end{tipBox}
...
\hline
\end{tabular}
\caption{Probability distribution for the number of 4 smoking friends who will develop a severe lung condition in their lifetime. This is a binomial distribution. After correcting rounding error, the probabilities add up to~1.}
...
\end{align*}
As one might expect, the sample proportion $\hat{p}$ is expected to be in the neighborhood of the true proportion $p$. Likewise, the standard deviation of $\hat{p}$ is equal to the standard deviation of the binomial distribution divided by $n$:
\begin{align*}
\sigma_{\hat{p}}
...
The mean of the sample proportion is the population proportion: 0.20. That is, $\hat{p}$ will tend to be near $p = 0.20$.
...
\begin{tipBox}{\tipBoxTitle{Three important facts about the distribution of a $\hat{p}$}
