...
myPDF("figureShowingBlocking.pdf", 4, 7,
      mar = rep(0, 4))
plot(c(0, 2.9),
     type = "n",
     axes = FALSE,
...
rect(0, 1.2, 0.45, 1.9)
rect(0.55, 1.2, 1, 1.9)
arrows(0.56, 2.17, 0.75, 2.02,
       length = 0.1,
       lwd = 1.37)
arrows(0.44, 2.17, 0.25, 2.02,
       length = 0.1,
       lwd = 1.37)
text(0.5, 2.07, "create\nblocks", cex = 0.8)
text(0.2, 1.89, "Low-risk patients", pos = 3, cex = 0.7)
text(0.2 + 0.55, 1.89, "High-risk patients",
     pos = 3,
     cex = 0.7)
...
for(i in 1:n){
  ...
  k[j] <- k[j]+1
  points(x[j], y[j], pch = pch[i], cex = cex[i],
         col = COL[1 + 3 * (pch[i] == 1)])
  if(y[j] < 0.12 + 0.51 * (j %in% c(2, 4)) - 0.015){
    x[j] <- x[j] + 0.11 - 0.025 * (j > 2)
    y[j] <- 0.35 + 0.48 * (j %in% c(2, 4)) - 0.015
  } else {
    y[j] <- y[j] - 0.085
  }
}
