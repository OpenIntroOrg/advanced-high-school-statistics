...
\Comment{D: I find the example below is confusing since sun exposure is related to applying sunblock. Also, I think the newly added sentence is incorrect: it should be discussing sun exposure, not sunblock usage (otherwise being a woman would be a confounding variable since women should get lower rates of cancer and they apply more sunblock). I'm favorable to scrapping this example since I think it is adding another variable / complexity to something that has not been historically reported as a problematic part.}

\Comment{L: You're right, this is not a good example.  I think we should include an example, though, of something that is NOT a confounding factor, or students think that any variable under the sun in a confounding variable, whether or not it is relevant to the the variables at hand.}
...
Sampling randomly resolves the problem of selection bias. The most basic random sample is called a \term{simple random sample}, and it is the equivalent of using a raffle to select cases. This means that each case in the population has an equal chance of being included and there is no implied connection between the cases in the sample.
...
\end{enumerate}
\Comment{D: Property 1 is a corollary of Property 2. L - are you saying we should say this? D: actually, my mistake, it's not redundant. Property 2 is difficult to conceptualize IMO. How about ``Observations are independently selected for the sample.''?}
...
Answers can vary. If we take a sample of size 3, then it is possible that we could sample players numbered 1, 2, and 3 in a simple random sample. Such a sample would be impossible from a systematic sample. \Add{Property 2 of simple random samples does not hold for other types of random samples.} \Comment{D: This new sentence is a tautology since Property 2 defines a simple random sample. L - Property 1 does hold for other types of samples though, and we didn't present Property 2 as an if and only if above. D: You're right, Property 2 is needed. I'm still very hesitant about this example since I don't think it makes the concepts easier to grasp, and I think it could lead to confusion and students second-guessing themselves.}
\end{example}
