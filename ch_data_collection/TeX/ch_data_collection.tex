...
\item How different do the results in the groups need to be before there is sufficient evidence of a real difference and not just random variation?
\end{itemize}}
...
\item Researchers compare the relevant \term{summary statistics} to get a sense of whether the treatment group did better, on average, than the control group.
...
\label{dataBasics}
\sectionintro{
\noindent \Add{You collect data on dozens of questions from all of the students at your school. How would you organize this?  Effective presentation and description of data is a first step in most analyses. This section introduces one structure for organizing data as well as some terminology that will be used throughout this book. We use loan data from the Lending Club and county  data from the American Community Survey to motivate and illustrate this section's learning objectives.}
...
\item Understand what it means for two variable to be associated. 
...
in Figure~\ref{countyDF}.  \Add{These data come from the American Community Survey, which is administered by the US Census Bureau.  Unlike the Decennial Census, which takes place every 10 years and attempts to collect basic demographic data from every residents of the US, the American Community Survey (ACS) is an ongoing survey that is sent to approximately 3.5 million households per year.  As stated by the ACS website, these data help communities ``plan for hospitals and schools, support school lunch programs, improve emergency services, build bridges, and inform businesses looking to add jobs and expand to new markets, and more."\footnote{\url{https://www.census.gov/programs-surveys/acs/about.html}}
...
\href{\oiRedirectUrl{tableau-scatterplotschoose}}{
  ...
  \caption{A scatterplot of homeownership versus the percent
      ...
      of 31.3\%. Explore this data set on Tableau Public\tableauhref{tableau-scatterplotschoose}.}
...
\href{\oiRedirectUrl{tableau-scatterplotschoose}}{
  ...
  \caption{A scatterplot showing
      ...
      and had median household income of \$22,736.  Explore dozens of scatterplots using American Community Survey data on Tableau Public\tableauhref{tableau-scatterplotschoose}.}
...
\Comment{check placement of figures}

\textA{\newpage}
%%
\subsection*{Section summary}
\begin{itemize}
\item Researchers often summarize data in a table, where the rows correspond to individuals or \term{cases} and the columns correspond to the \term{variables}, the values of which are recorded for each individual.
...
\sectionintro{
\noindent \Add{How do researchers collect data?  Why are the results of some studies more reliable than others?  How a researcher collects data depends upon what the research goals are.  In this section we look at different methods of collecting data and consider the types of conclusions that can be drawn from those methods.}
%%
\subsection*{Learning objectives}
...
%%%section exercises
{\input{ch_data_collection/TeX/overview_of_data_collection_principles.tex}}
...
\sectionintro{
\noindent \Add{You have probably read or heard of many studies and polls that make claims.  A background in statistical reasoning will help you assess the validity of such claims.  Some of the big questions we address in this section include:

\begin{itemize}
\item When a study finds a relationship between two variables, such as eating chocolate and positive health outcomes, can we draw a causal conclusion?  

\item How do opinion polls work?  How do research organizations collect the data and what types of bias should we look out for?  
\end{itemize}}
...
\item Identify and describe the effects of different types of bias in sample surveys, including selection bias, non-response, and response bias. 
 
\item Identify and describe how to implement different random sampling methods, including simple, stratified, and cluster.  
 
\item Recognize the benefits and drawbacks of choosing one sampling method over another.
 
\item Understand when it valid to draw an inference and to what population that inference can be drawn.  
\end{enumerate}
...
\begin{itemize}
\item In an \term{observational study}, one must always consider the existence of \term{confounding factors}.  A confounding factor is a ``spoiler variable" that could explain an observed relationship between the explanatory variable and the response.  Remember:  For a variable to be confounding it must be associated with both the explanatory variable \textit{and} the response variable.

\item When taking a sample from a population, avoid \term{convenience samples} and \term{volunteer samples}.  Instead, use a \term{random} sampling method.

\item Random sampling avoids the problem of \term{selection bias}.  However, \term{response bias} and \term{non-response} bias can be present in any type of sample, random or not.

\item In a \term{simple random sample}, each individual of the population is numbered from 1 to N.  Using a random digit table or a random number generator, numbers are randomly selected and the corresponding individuals become part of the sample.
...
\item A \term{stratified random sample} involves randomly sampling from \textit{every}  \term{strata}, where the strata should correspond to a variable thought to be associated with the variable of interest.  This ensures that the sample will have appropriate representation from each of the the different strata and reduces variability in the sample estimates.

\item A \term{cluster random sample} involves selecting a set of \term{clusters}, or groups, and then collecting data on all individuals in the selected clusters. This can be useful when sampling clusters is more convenient and less expensive than sampling individuals, and it is an effective strategy when each cluster is approximately representative of the population.  
...
%%%section exercises
{\input{ch_data_collection/TeX/sampling_principles_and_strategies.tex}}
...
\sectionintro{
\noindent \Add{You would like to determine if drinking a cup of tea each morning will cause students to perform better on tests.  What are different ways you could design an experiment to address this question?  What are possible sources of bias and how would you try to minimize them?   Ultimately, the goal of an experiment is to be able to draw a causal conclusion about the effect of a treatment - in this case, drinking tea.  If the design is poor, a causal conclusion cannot be drawn, even if you do observe an association between drinking tea and performing better on tests. This is why it is crucial to start with a well-designed experiment. }
...
\item \term{Replication}, or imposing the treatments on many subjects, gives more data and decreases the likelihood that the treatment groups differ on some characteristic due to chance alone (i.e. in spite of the randomization).  

\item An ideal experiment is \term{randomized}, \term{controlled}, and \term{double-blind}.  
...
\begin{itemize}
\item \textbf{\term{Stratifying} vs \term{Blocking}}.  Stratifying is used when sampling, where the purpose is to \emph{sample} a subgroup from each stratum in order to arrive at a better \textit{estimate} for the parameter of interest.  Blocking is used in an experiment to \emph{separate} subjects into blocks and then \emph{compare} responses within those blocks.  All subjects in a block are used in the experiment, not just a sample of them.
\item \textbf{\term{Random sampling} vs \term{Random assignment}}.  Random sampling refers to sampling a subset of a population for the purpose of inference to that population.  Random assignment is used in an experiment to separate subjects into groups for the purpose of comparison between those groups.  \end{itemize}  When randomization is not employed, as in an \term{observational study}, neither inferences nor causal conclusions can be drawn.  Always be mindful of possible \term{confounding factors} when interpreting the results of observation studies.
