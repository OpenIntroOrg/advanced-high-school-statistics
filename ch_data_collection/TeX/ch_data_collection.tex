...
\Comment{I find the example below is confusing since sun exposure is related to applying sunblock. Could another behavior be chosen, such as healthier eating, exercise, or something else? Also, I think the newly added sentence is incorrect: it should be discussing sun exposure, not sunblock usage (otherwise being a woman would be a confounding variable if sunblock since women should get lower rates of cancer under the scenario and they apply more sunblock). I'm also favorable to scrapping this example since I think it is adding another layer to this example when one is not needed.}
...
\end{enumerate}
\Comment{Property 1 is a corollary of Property 2.}
...
Answers can vary. If we take a sample of size 3, then it is possible that we could sample players numbered 1, 2, and 3 in a simple random sample. Such a sample would be impossible from a systematic sample. \Add{Property 2 of simple random samples does not hold for other types of random samples.} \Comment{This new sentence doesn't really need to be added (see other comment above), and it also seems out of place since it is just a general comment that is not relevant to this example.}
\end{example}
...
Because grade level affects the likelihood of having a part-time job, we should take a stratified random sample. To do this, we can take a simple random sample of 15 students from each grade. This will give us equal representation from each grade. Note: in a simple random sample, just by random chance we might get too many \Add{students who are older,} \Cut{, say, Juniors and Seniors,} which \Cut{would} could make \Cut{our} \Add{the} estimate too high. Also, there are no well-defined clusters in this example. We~wouldn't want to use the grades as clusters and sample everyone from a couple of the grades. This would create too large a sample and would not give us the nice representation from each grade afforded by the stratified random sample.
\end{example}
