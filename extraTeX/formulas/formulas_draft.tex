...
\noindent%
\textbf{Probability and Distributions}
\begin{align*}
&P(A \cup B) = P(A) + P(B) - P(A\cap B) \\[2.5mm]
&P(A | B) = \frac{P(A \cap B)}{P(B)} \\[2.5mm]
&\text{For a discrete random variable X:}\\[2.5mm]
...
\noindent%
\textbf{Sampling Distributions and Inferential Statistics} %\vspace{2mm}
...
\noindent%
\begin{tabular}{lcccl}
random variable &mean & standard deviation &$SE$  of estimate& \\
one population &$\mu_{\hat{p}}=p$ &$\sqrt{\frac{{p}(1-p)}{n}}$ &$\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}$ &
