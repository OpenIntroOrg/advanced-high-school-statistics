...
estimate:  ~4 / 5 = 0.80
(c)~P(at least one car fails in a fleet of seven) = 1 - P(no cars fail) = $1 - (0.84)^7 = 0.705$.}
...
% 43 - eye_color_geometric

\eocesol{(a)~$0.875^2\times 0.125 = 0.096$.
(b)~$\mu=8$, $\sigma=7.48$.}

% 45 - bernoulli_mean_derivation
...
% 57 - tree_hiv_eswatini

\eocesol{0.8247. \\
\includegraphics[width=60mm]{ch_probability/figures/eoce/tree_hiv_eswatini/tree_hiv_eswatini.pdf}}

% 59 - cost_of_breakfast
...
\eocesol{As $n$ increases the standard deviation of the sampling distribution decreases but the mean stays the same.
\begin{center}
$n = 5 \ \rightarrow \ \mu_{\bar{x}} = 10.44, \quad \sigma_{\bar{x}} = \frac{9.2}{\sqrt{5}} = 4.11$ \\
$n = 30 \ \rightarrow \ \mu_{\bar{x}} = 10.44, \quad \sigma_{\bar{x}} = \frac{9.2}{\sqrt{30}} = 1.68$ \\
$n = 100 \ \rightarrow \ \mu_{\bar{x}} = 10.44, \quad \sigma_{\bar{x}} = \frac{9.2}{\sqrt{100}} = 0.92$ \\
\end{center}
...
\eocesol{(a)~The distribution of scores on this final exam is likely left skewed since 
the median is greater than the mean.
(b)~Since the distribution is probably left skewed, the median would be more 
than the mean, and a majority of observations would be be above the mean.
(c)~The shape of the precise distribution is unknown, so we cannot.
(d)~Even though the population distribution is not normal, if the conditions 
for inference are satisfied we may be able to use the Central Limit Theorem to 
estimate this probability. Independence: Students are sampled randomly and 40 
students are likely less than 10\% of all students who take this introductory 
statistics course, so the observations can be assumed to be independent. 
Normality: The data are skewed but a sample size of 40 might be large enough.
$P(\bar{X} > 75) = 0.0008$.
(e)~Halving the sample size would increase the standard of error by a factor 
of $\sqrt{2}$, i.e. the new standard error will be $\sqrt{2}$ times the old 
standard error.}

% 15 - identify_dist_ls_pop

\eocesol{The centers are the same in each plot. Plots A and C are nearly normal while 
plot B is left skewed. Plot B is the distribution of a single random sample of 
500 values from this population since it mimics the population distribution The 
only way to tell which plot corresponds to which scenario is to examine the 
variability of each distribution. Plot C is more variable, hence corresponds to 
the smaller sample size.}

% 17 - cflbs

\eocesol{(a)~0.0668.
(b)~Independence: The light bulbs are selected randomly and 15 light bulbs are 
less than 10\% of all CFLBs, therefore we can assume the that the lifespan of 
one light bulb is independent of another. Normality: The population distribution 
is nearly normal.
(c)~$\approx$ 0.
(d)~$\:$\\
\includegraphics[width=40mm]{ch_distributions/figures/eoce/cflbs/cflbs_sketch.pdf}
...
\eocesol{(a)~0.2514.
(b)~27 square feet per can.
(c)~We are looking for the probability that the total of 20 cans cover more 
than 540 square feet. This means that one can on average should cover 
$\frac{540}{20} = 27$ square feet. To find this we need to find the distribution 
of $\bar{X}$ using CLT, but first we must check that the conditions hold.
Independence: We are not told that this is a random sample but it may be a 
safe assumption that the amount of paint in a can of spray paint is determined 
by a random process. 20 cans certainly make up less than 10\% of all cans of 
spray paint. Therefore, the area the sampled cans of spray paint cover are 
independent.
Normality: The population distribution is nearly normal.
$P(\bar{X} > 27) = 0.0014$
(d)~We could not calculate (a) without a nearly normal population 
distribution. Without this and a small sample size of $n = 20$, the sampling 
distribution will not be nearly normal. Therefore we could not calculate (c) 
either.}

% 21 - betting_on_dinner_2

\eocesol{\begin{align*}
&E(X_1 + \cdots + X_{5})  = \$6 \times 5 = \$30 \\
&SD(X_1 + \cdots + X_{5}) = \sqrt{2^2 \times 5} = \$4.47\\
Z = \frac{28 - 30}{4.47} \approx -0.45
P(X < \$28) \approx P(Z< -0.45) = 0.3264.
\end{align*}}

% 23 - difference_of_props_prob_2

\eocesol{(a)~The success-failure conditions are satisfied for both populations
  and the samples are also random, so the distribution of the difference
  of sample proportions will closely resemble a normal distribution.
(b)~Given parts~(a) and~(b), this is now a normal probability problem
  with $X \sim N(0.05, 0.0081)$.
  $Z = \frac{0.055 - 0.050}{0.0081} = 0.62 \to 0.2676$.
(c)~$Z = \frac{0.04 - 0.05}{0.0081} = -1.23 \to 0.1094$.}

% 25 - difference_of_means_prob_2

\eocesol{(a)~Because the sample sizes are at least 30 and the samples are random
  and independent, the distribution of the difference of sample means
  should closely resemble a normal distribution.
(b)~$Z = \frac{7 - 5}{3.37} = 0.59 \to 0.2776$.
(c)~$Z = \frac{3 - 5}{3.37} = -0.59 \to 0.2776$.
(d)~$Z = \frac{0 - 5}{3.37} = -1.49 \to 0.0681$.
  (The value above is computed without rounding 3.3665 to 3.37.
  Answers that used rounding of 3.37 would get $Z = -1.48$
  and a tail area of 0.0694.)}

% 27 - sat_scores

\eocesol{This can be calculated as
$P(X > 1500~|~X > 1350)
  = P(X > 1500) / P(X > 1350)$,
which yields $0.0228 / 0.1056$,
or 21.6\% of students.}

% 29 - survey_response_rate

\eocesol{Approximately 0.}

% 31 - chocolate_chip_cookies

\eocesol{(a)~ $SD\left(\bar{x}_{sample\ of\ 22}\right) = \frac{SD_{obs}}{\sqrt{n}} = \frac{4.37}{\sqrt{22}} \approx 0.93$ chocolate chips.
(b)~ $Z = \frac{14.77 - 20}{0.93} = -5.62$, and the lower tail for this Z-score is about 0.
(c)~ It would be wise to be skeptical here, since observing an average of 14.77 chocolate chips per cookie would be extremely unusual if the true average was 20.}

% 33 - poverty_us_2019

\eocesol{First, we note that the expected number of successes and failures (192 and 808)
are both well over 10, so normal approximation to the binomial and to the distribution
of the sample proportion is reasonable.
(a)~ The mean and standard deviation of the binomial distribution:
  \begin{align*}
  \mu_{X} &= 0.192 \times 1000 = 192 \\
  \sigma_{X} &= \sqrt{1000 \times 0.192 (1 - 0.192)} = 12.46
  \end{align*}
  The Z-score of $x = 200$ is
  \begin{align*}
  Z = \frac{200 - 192}{12.46} = 0.642
  \end{align*}
  We want the upper tail, which is about 0.2604
(b)~The mean and standard deviation of the distribution of the sample proportion:
  \begin{align*}
  \mu_{\hat{p}} &= 0.192 \\
  \sigma_{\hat{p}} &= \sqrt{\frac{0.192 (1 - 0.192)}{1000}} = 0.01246
  \end{align*}
  The value $x = 200$ corresponds to a sample proportion of $200 / 1000 = 0.2$,
  so the Z-score is
  \begin{align*}
  Z = \frac{0.2 - 0.192}{0.01246} = 0.642
  \end{align*}
  We want the upper tail, which is about 0.2604.
(c)~They are the same, and they should be,
  because we're just rescaling the data by the sample size.}
...
% 19 - prop_which_higher_found_inf
...
(c)~They are equal.
    The sample size does not affect the calculation
    of the p-value for a given Z-score.
(d)~Scenario I is higher.
    If the null hypothesis is harder to reject
    (lower $\alpha$), then we are more likely to make
    a Type~2 Error when the alternative hypothesis is true.}

% 21 - twitter_users_tf

\eocesol{(a)~False. 50\% is included in the 99\% confidence interval, hence a null
hypothesis of $p = 0.50$ would not be rejected at this level.
(b)~False. The standard error measures the variability of the sample proportion,
and is unrelated to the proportion of the population included in the study.
(c)~False. We need to increase the sample size to decrease the standard error.
(d)~False. As the confidence level decreases so does the margin of error,
and hence the width of the confidence interval.}
...
\eocesol{(a)~Identify: $H_0$: $p = 0.5$. $H_A$: $p > 0.5$.  $\alpha = 0.05$.  Choose: 1-proportion Z-test. Check: Independence (random sample, $<10\%$ of 
population) is satisfied, as is the success-failure conditions (using 
$p_0 = 0.5$, we expect 40 successes and 40 failures). Calculate:  $Z = 2.91$ $\to$ p-
value $= 0.0018$. Conclude: Since the p-value $< 0.05$, we reject the null hypothesis. 
...
(b)~The p-value represents the following conditional probability:  $P(\hat{p} > 0.6625~|~p = 0.5)$. If in fact people cannot tell the difference between diet and regular 
soda and they randomly guess, the probability of getting a random sample of 
80 people where 66.25\% (53/80) or higher identify a soda correctly would be 0.0018.}
...
\eocesol{Identify: Subscript $_C$ means control group. Subscript $_T$ means truck drivers. 
$H_0: p_C = p_T$. $H_A: p _C \ne p_T$. $\alpha=0.05$.  Choose: 2-proportion Z-test.  Check: Independence is satisfied (random 
...
\eocesol{Identify:  $H_0$: Opinions regarding offshore drilling for oil and having a college degree are independent.
 $H_A$: Opinions regarding offshore drilling for oil and having a college degree are dependent. $\alpha=0.05$. 
 Choose: chi-square test for independence
 Check:
\begin{align*}
...
% 35 - ship_gifts_chisq_indep_conditions
...
% 37 - college_smokers_CI_sample_size
...
% 45 - chisq_choose_test_from_description

\eocesol{(a)~Since there are 3 independent random samples here,
  we do a test for homogeneity. $df = 2$.
...
\eocesol{(a)~Identify: $H_0$: $\mu = 5$. $H_A$: $\mu < 5$. 
We'll use $\alpha = 0.05$. Choose: 1-sample $t$-test. Check: This is a random sample, so the observations are 
...
(b)~Identify: estimate average hours a child takes piano lessons in this city with 95\% confidence. Choose:  1-sample $t$-interval. Check:  same as in part (a).  Calculate: Using $SE = 0.4919$ and $t_{df = 19}^{\star} = 2.093$, the confidence 
interval is (3.57, 5.63). Conclude: We are 95\% confident that the average number of 
years a child takes piano lessons in this city is 3.57 to 5.63 years. Do not have evidence that average is not 5 because 5 is in the interval.\\
...
% 21 - global_warming_v2_2_ahss

\eocesol{Identify: we want to estimate the average difference in number of days exceeding
90\textdegree{}F for (2018~-~1948) with 90\% confidence.  Choose: 1-sample $t$-interval with paired data.  Check:  $n_{diff}=197 \ge 30$ and the locations are randomly sampled.  Calculate: average~$SE_{diff} = 1.23$ and $df=196$. $t^{\star} \approx 1.65$.
    $2.9 \pm 1.65 \times 1.23 \to (0.87, 4.93)$. Conclude:  We are 90\% confident that there was an increase of 0.87 to 4.93 in the average difference of days that hit 90\textdegree{}F in 2018
    relative to 1948 for NOAA stations. We have evidence that the average difference of days that hit 90\textdegree{}F increased, because the interval is entirely above~0.}

% 23 - diamonds_1_ahss

\eocesol{Identify:$H_0$: $\mu_{0.99} = \mu_{1}$ and $H_A$: $\mu_{0.99} \ne \mu_{1}$; Let $\alpha=0.05$.  Choose: 2-sample $t$-test.  Check:  Independence: 
...
Normal populations: The sample distributions are not very skewed, hence we find it reasonable that the underlying population distributions are nearly normal.  Calculate:  $T = -2.82$, $df = 42.5$, p-value = 0.007.  Conclude:  Since p-value $<$ 0.05, 
...
\eocesol{Let $\mu_{diff} = \mu_{pre - post}$.
$H_0: \mu_{diff} = 0$:
...
$b = r\times s_y/s_x = 0.636 \times 113 / 99 = 0.726$. 
Next, make use of the fact that the regression line passes through 
the point $(\bar{x},\bar{y})$: $\bar{y} = a + b \times \bar{x}$. 
Plug in $\bar{x}$, $\bar{y}$, and $b$, and solve for $a$: 51. 
...
(b)~Neither are a particularly: For the logged model, the scatterplot and residual plot show more constant variance in the residuals. However, the scatterplot with the logged model looks to have a bit of curvature.
