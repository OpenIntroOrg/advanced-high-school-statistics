...
% 47 - eye_color_geometric

\eocesol{(a)~$0.875^2\times 0.125 = 0.096$.
(b)~$\mu=8$, $\sigma=7.48$.}

% 49 - bernoulli_mean_derivation
...
% 29 - heights_10_yrs_prob

\eocesol{(a)~Normal probability: 0.4325.
(b)~Binomial: 0.7814 (if we're okay assuming independence).
(c)~Sequence of events: 0.1062.
(d)~Negative binomial: 0.0550.}
...
\eocesol{First, we note that the number of successes and failures (225 and 775) are both well over 10 so normal approximation to the binomial and to the distribution of the sample proportion is reasonable.
(a)~ The mean and standard deviation of the binomial distribution:
  \begin{align*}
  \mu_{X} &= 0.225 \times 1000 = 225 \\
  \sigma_{X} &= \sqrt{1000 \times 0.225 (1 - 0.225)} = 13.21
  \end{align*}
The Z-score of $x = 250$ is
  \begin{align*}
  Z = \frac{250 - 225}{13.21} = 1.89
  \end{align*}
We want the upper tail, which is about 0.0294.
(b)~The mean and standard deviation of the distribution of the sample proportion:
  \begin{align*}
  \mu_{\hat{p}} &= 0.225 \\
  \sigma_{\hat{p}} &= \sqrt{\frac{0.225 (1 - 0.225)}{1000}} = 0.0132
  \end{align*}
The Z-score of $x = 250$ is
  \begin{align*}
  Z = \frac{\frac{250}{1000} - 0.225}{0.0132} = 1.89
  \end{align*}
We want the upper tail, which is about 0.0294.
(c)~They are the same, and they should be, since we're just rescaling the data by the sample size.}
...
% 37 - offshore_drilling_chisq_indep
