...
\Cut{Roughly speaking, the}
\Add{The} $R^2$ of a linear model describes how much smaller the variance (in the $y$ direction) about the regression line is than the variance about the horizontal line $\bar{y}$.
...
$$\frac{s_{aid}^2 - s_{_{RES}}^2}{s_{aid}^2}
	...
	= 0.25$$
\Add{If we used the simple standard deviation of the
...
\Cut{If we use $s_{_{RES}}^2$ as computed by the linear regression
model, this is close to the $R^2$ value.
More technically, we calculate $R^2$ as the reduction in the sum of the squared error about the line.}
