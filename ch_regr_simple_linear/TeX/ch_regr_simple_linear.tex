...
\begin{termBox}{\tBoxTitle{Interpreting parameters in a linear model\vspace{-4mm}}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\begin{example}{The regression summary offers the degrees of freedom for the test statistic $T$: $df=25$. Identify the p-value for the hypothesis test.}
...
\begin{termBox}{\tBoxTitle{Linear Regression t test\vspace{-1.5mm}}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the name of the test being used: Linear regression t test.
...
\item Verify conditions: the residual plot has no pattern.
\item Calculate the test statistic and degrees of freedom.
$$\text{t} = \frac{\text{point estimate} - \text{null value}}{\text{SE of estimate}}$$
The point estimate is $b_1$, $SE$ can be located on regression summary table next to value of $b_1$, and $df = n-2$.
...
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the name of the CI being used: t CI for slope of regression line.
\item Verify conditions: the residual plot has no pattern.
...
The point estimate is $b_1$, $df = n-2$, the critical value $t^*$ can be found with a t-table at row $df = n-2$, and $SE$ can be located on regression summary table next to value of $b_1$.
\item Evaluate the CI and write in the form ( $\_$ , $\_$ ).
\item Interpret the interval:  ``We are [X]\% confident that this interval contains the true average increase in [y] for each additional [unit] of [x].
...
\begin{termBox}{\tBoxTitle{TI calculator: The Linear Regression t-Test on $\beta_1$}
...
\begin{termBox}{\tBoxTitle{TI calculator: Calculating a Confidence Interval for $\beta_1$}
