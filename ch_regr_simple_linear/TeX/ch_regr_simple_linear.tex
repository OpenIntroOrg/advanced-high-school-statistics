...
\item In Chapter 2 we introduced \termsub{scatterplots}{scatterplot}, which show the relationship between two numerical variables.  When we use $x$ to predict $y$, we call $x$ the \term{explanatory variable} or predictor variable, and we call $y$ the \term{response variable}.
...
\begin{itemize}
\item[] Here we use choose the \termsub{Linear regression $\pmb{t}$-interval for the slope}{linear regression t-interval for the slope@linear regression $t$-interval for the slope}.
\end{itemize}
...
\begin{itemize}
\item[] Here we choose the \termsub{linear regression $\pmb{t}$-test for the slope}{linear regression t-test for the slope@linear regression $t$-test for the slope}. 
\end{itemize}
...
\item Under certain assumptions, the sampling distribution of $b_1$ is \emph{normal} and the distribution of the standardized test statistic using the standard error of the slope follows a \term{t-distribution@$t$-distribution} with $n-2$ degrees of freedom.
...
\begin{itemize}
\item Estimate $\beta_1$ at the C\% confidence level using a \termsub{Linear regression $\pmb{t}$-interval}{linear regression t-interval@linear regression $t$-interval}.
\item Test $H_0$: $\beta_1=0$ at the $\alpha$ significance level using a \termsub{Linear regression $\pmb{t}$-test}{linear regression t-test@linear regression $t$-test}.
\end{itemize}
...
\item $\hat{y} = b_0+b_1x$ provides the best fit line for the \emph{observed data}.  To estimate or hypothesize about the slope of the population regression line, first confirm that the residual plot has no pattern and that a linear model is reasonable, then use a \termsub{linear regression $\pmb{t}$-interval for the slope}{linear regression t-interval for the slope@linear regression $t$-interval for the slope} or a \termsub{linear regression $\pmb{t}$-test for the slope}{linear regression t-test for the slope@linear regression $t$-test for the slope} with $n-2$ degrees of freedom.
\end{itemize}
