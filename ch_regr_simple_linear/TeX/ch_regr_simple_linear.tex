...
\noindent Get started quickly with this \oiRedirect{desmos-linreg}{Desmos Calculator}.
\begin{center}
...
\begin{exercisewrap}
\begin{nexercise}
Use the full \data{loan50} \oiRedirect{data-loan50}{data set} and one of these \oiRedirect{desmos-linreg2}{Desmos Calculators}  to draw the scatterplot and find the equation of the least squares regression line for prediction loan amount ($y$) from total income ($x$).  Also draw the residual plot and find the standard error of the residuals.\footnotemark
\end{nexercise}
\end{exercisewrap}
\label{loan50Desmos}
\footnotetext{
This will require a minor data cleaning step.  After downloading the loan50 data, notice the oddly formatted numbers under total$\_$income column.  This will confuse Desmos!  Highight the column, do Format, Cells, Number, and set decimal places to 0.  Finally, copy and paste the loan$\_$amount column to the right of total$\_$income.  Then you can copy the two columns into Desmos.\\
...
\begin{itemize}
\item[] To estimate the slope of a regression model we use a \termsub{$\pmb{t}$-interval for the slope}{t-interval for the slope@$t$-interval for the slope}.
\end{itemize}
...
%%
\subsection[Technology: the $t$-interval for the slope]{Technology: the linear regression \pmb{$t$}-interval for the slope}

\noindent We usually rely on regression output from statistical software when constructing confidence intervals for the slope of a regression line.  However, it is also possible to use Desmos or a calculator.\\

\noindent Get started quickly with this \oiRedirect{desmos-linregCI}{Desmos Calculator}.
\begin{center}
\oiRedirect{desmos-linregCI}{\includegraphics[width=0.90\textwidth]{ch_regr_simple_linear/figures/desmosLinReg/desmosLinRegCI}}
\end{center}


\noindent Entering a lot of data into a calculator is not practical;  we include calculator instructions here simply for completion.
...
\begin{itemize}
\item[] To test hypotheses about the slope of a regression model we use a \termsub{$\pmb{t}$-test for the slope}{t-test for the slope@$t$-test for the slope}. 
\end{itemize}
...
%%
\subsection[Technology: the $t$-test for the slope]{Technology: the \pmb{$t$}-test for the slope}

\noindent We generally rely on regression output from statistical software programs to provide us with the necessary quantities: $b$ and $SE \text{ of } {b}$.  However we can also find the test statistic and p-value using Desmos or a calculator.\\

\noindent Get started quickly with this \oiRedirect{desmos-linregtest}{Desmos Calculator}.
\begin{center}
\oiRedirect{desmos-linregtest}{\includegraphics[width=0.95\textwidth]{ch_regr_simple_linear/figures/desmosLinReg/desmosLinRegTest}}
\end{center}


\noindent \\ 
\\
Entering a lot of data into a calculator is not practical; we include calculator instructions here simply for completion. 

\begin{onebox}{\videohref{ti84_regression_t_test} TI-83/84: Linear regression T-test for \pmb{$\beta$}}
\label{LinRegtest}
...
\begin{onebox}{\videohref{casio_regression_t_test} Casio fx-9750GII: Linear regression T-test for \pmb{$\beta$}}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Navigate to \calctext{STAT} (\calcbutton{MENU} button, then hit the \calcbutton{2} button or select \calctext{STAT}).
