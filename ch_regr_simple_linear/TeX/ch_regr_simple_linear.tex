...
\oiRedirect{desmos-anscombe}{
   \Figures [Four very different looking scatter plots are shown.  All are graphed on the same scale, with the y axis ranging from 0 to 15 and the x axis ranging from 0 to 20.  The first scatter plot shows points scattered in a football or oval shape, with a moderate, positive, linear association.  In the second scatter plot the points are curved.  In the third scatter plot, the points are collinear with positive slope, except for one outlier.  In the fourth scatter plot, the points are all arranged in a vertical line, except for one outlier point in the upper, right.  All four graphs have the least squares regression line drawn in, and though the scatter plots are very different, the least squares regression line is exactly the same for all four graphs.] {}{anscombe}{anscombeDesmos}}
...
\Figure [A scatter plot is shown with just three points (1, 1), (3,5) and (5,3).  The least squares regression line is drawn in.  For each of the three poinst, a vertical line from the point to the regression line is drawn, indicating the residual.  To show the squared residuals, an actual square is drawn with each vertical residual line being a side of the square. ] {0.75}{leastSquares}}
...
\begin{figure}[ht]
  \centering
  \subfigure[]{
    \Figures[A scatter plot with just three points is shown, where the regression line is the horizontal line y = y average, corresponding to an R^2 of 0.] {0.4}
        {rSquared}
        {rsq1}
    \label{rsq1}
  }\hspace{5mm}
  \subfigure[]{
    \Figures[ A scatter plot with just three points shown that are collinear.  The regression line passes through the three points and R^2 = 1.] {0.4}
        {rSquared}
        {rsq2}
  \label{rsq2}
  }
...
\begin{center}
\Figure [A Desmos calculator screen is shown.  On the left, three x, y coordinates are in a table and a linear model is calculated.  On the right is the graph of the points with the regression line drawn in.] {0.95}{desmosLinReg}
\end{center}
...
\begin{figure}[ht]
   \begin{center}
  \Figures [A scatter plot of y versus x is shown.  There is a positive association that appears curved.  Below is a residual plot, with the residuals for the model plotted versus x.  There is a U-pattern in the residual plot. ] {0.75}{NeedsTransform}{NeedsTransform-PreTransform}
\label{pretransform}
...
\begin{figure}[h]
   \centering
  \Figures [A scatter plot of log(y) versus x is shown.  The assocation is fairly strong, positive and linear.  Below this graph is a residual plot, which plots the residuals for the model versus x.  There are no apparent patterns in the residual plot. ] {0.75}{NeedsTransform}{NeedsTransform-PostTransform}
...
%%%%%%%%%%%Section Exercises
{\input{ch_regr_simple_linear/TeX/transformations_for_skewed_data.tex}}
...
\item Distinguish between when to use the $t$-test for the slope of a regression line and when to use the 1-sample $t$-test for a mean of differences.
\end{enumerate}
}
...
\begin{figure}%
   \centering
  \begin{tabular}{cc}
   \Figures [A scatterplot of the Elmhurst data is shown for gift aid and family income with the least squares regression line overlaid against the data, which has a slight downward trend.  ] {0.5}{elmhurstInference}{elmhurstScatter}
   \Figures [A residual plot is shown graphing the residuals versus family income.  The residuals range from -12 to positive 12 and show no pattern. ] {0.5}{elmhurstInference}{elmhurstResidual}
\end{tabular}
...
\begin{figure}%
   \centering
  \begin{tabular}{cc}
   \Figures [A scatterplot showing head length against total length for 104 brushtail possums is shown again. A linear trend line has been added with an equation of y-hat equals 41 plus 0.59 times x, which shows the clear upward trajectory of the data.] {0.45}{possumInference}{possumScatter}
    \Figures [A residual plot for the trend line fit to the brushtail possum data is shown again. Here, the horizontal axis is the same -- representing "total length", it spans 75 to 97 -- while the vertical axis represents "Residuals" and spans from about -7 to positive 8.] {0.45}{possumInference}{possumResidual}
\end{tabular}
...
\begin{figure}[h]
\centering
\oiRedirect{tableau-residuals-changeinsets-unemployment}{
\Figures[ The residuals for the linear regression model predicting the percent change in House seats for the President's party in each midterm election from 1898 to 2018 based on Percent Unemployment are plotted versus Percent Unemployment.  The residuals range from about -15\% to positive 15\%.  The residual plot shows no pattern. ] {0.9}{unemploymentAndChangeInHouse}{unemploymentAndChangeInHouseResiduals}
}
...
\begin{figure}
\centering
\Figure [A normal distribution is shown, with mean 0 and standard deviation 0.87.  The area to the left of -0.8897 is shaded and appears to be about 15\% of the total area.] {0.82}{oneSidedTailForMidtermUnemploymentHT}
...
\centering
  \subfigure[]{
\Figures [A histogram of UCLA Bookstore price minus Amazon Price is shown. The differences range from -20 dollars to positive 80 dollars.  The distribution is right skewed.] {0.55}{textbooksF18}{diffInTextbookPricesF18}
}
  \subfigure[]{
\Figure [A scatter plot of Amazon Price versus UCLA Bookstore price is shown. The prices for both Amazon and UCLA Bookstore range from 0 to about 250 dollars.  The association is linear, positive, and strong.] {0.45}{textbooks_scatter}
}
...
Likewise, a 1-sample $t$-interval for a mean of differences would provide an interval of reasonable values for the mean of differences in textbook price between UCLA Bookstore and Amazon (for all UCLA textbooks), while a $t$-interval for the slope would provide an interval of reasonable values for the slope of the regression line for predicting Amazon price from UCLA Bookstore price (for all UCLA textbooks). 
...
%%%%%%%%%%%Section Exercises
{\input{ch_regr_simple_linear/TeX/inference_for_the_slope_of_a_regression_line.tex}}
