...
\Comment{TODO: David: Please put the scatter and residual plots here for the untransformed and transformed data.  For the scatterplots, please don't include the best fit line.  Also, for the residual plots, can you change the label on the vertical axis to say residual.  Also, include the two regression outputs.  Thanks.}
...
First, we can note the $R^2$ value is fairly large.  However, this alone does not mean that the model is good.  Another model might be much better.  When assessing the appropriateness of a linear model, we should look at the residual plot.  The U pattern in the residual plot tells us the the original data has a curve. If we inspect the two plots, we can see that for small and large values of $x$ we tend to underestimate $y$, whereas for middle values of $x$, we tend to overestimate $y$.  Because of the this, the model is not appropriate, and we should not carry out a linear regression $t$-test because the conditions for inference are not met.  However, the data might be better modeled using a transformation.
\end{example}

\begin{figure}
   \centering
   \includegraphics[width=0.7\textwidth]{ch_regr_simple_linear/figures/NeedsTransform/NeedsTransform-PreTransform}
   \caption{Variables $y$ is plotted against $x$. A nonlinear relationship is evident by the ``U'' shown in the residual plot. The curvature is also visible in the original plot.}
   \label{NeedsTransform-PreTransform}
\end{figure}

\begin{tipBox}{\texttt{\color{red}The regression equation is} \\
...
\item The model provided by Regression I yields a better fit.
\item The model provided by Regression II yields a better fit.
\end{enumerate}
\end{exercise}
