...
\begin{termBox}{\tBoxTitle{TI-84: finding $b_0$, $b_1$, $R^2$, and $r$ for a linear model}
Use \textbf{STAT, CALC, LinReg(a + bx)}.
\begin{enumerate}
\item Choose STAT.
...
If $r^2$ and $r$ do now show up when doing STAT, CALC, LinReg, the \emph{diagnostics} must be turned on.  This only needs to be once and the diagnostics will remain on.
\begin{enumerate}
...
D - Should it be added, or is the $df = n - 2$ something they need to know?}
