...
\Comment{L: I thought we changed all the R to lower case r for correlation coefficient.  Can we change these for Figure 8.10?}
...
\begin{termBox}{\tBoxTitle{Interpreting parameters in a linear model\color{red} CUT THIS BOX}
\vspace{-4mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
\item The slope, $b_1$, describes the estimated difference in the $y$ variable if the explanatory variable $x$ for a case happened to be one unit larger. 
...
\begin{termBox}{\tBoxTitle{Interpreting parameters in a linear model\color{red} ADD THIS BOX}
\vspace{-4mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\begin{termBox}{\tBoxTitle{TI-84: finding $b_0$, $b_1$, $R^2$, and $r$ for a linear model}Use \textbf{STAT, CALC, LinReg(a + bx)}.
\begin{enumerate}
\item Choose STAT.
...
\begin{tipBox}{\tipBoxTitle{What to do if $r^2$ and $r$ do not show up on a TI-83/84}If $r^2$ and $r$ do now show up when doing STAT, CALC, LinReg, the \emph{diagnostics} must be turned on.  This only needs to be once and the diagnostics will remain on.
\begin{enumerate}
...
\begin{tipBox}{\tipBoxTitle{What to do if a TI-83/84 returns: ERR: DIM MISMATCH }This error means that the lists, generally L1 and L2, do not have the same length.
\begin{enumerate}
\item Choose 1: Quit
...
We previously used a $T$ test statistic for hypothesis testing in the context of numerical data. Regression is very similar. In the hypotheses we consider, the null value for the slope is 0, so we can compute the test statistic using the $T$\Cut{(or Z)} score formula:
\begin{align*}
...
\begin{termBox}{\tBoxTitle{Hypothesis test for the slope of regression line\vspace{-1.5mm}}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for the slope of regression line \vspace{-1mm}}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\begin{termBox}{\tBoxTitle{TI-83/84: Linear Regression $T$ test on $\beta_1$}
Use \textbf{STAT, TESTS, LinRegTTest}.
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Choose STAT.
...
\begin{termBox}{\tBoxTitle{TI-84: $T$ Confidence Interval for $\beta_1$}
Use \textbf{STAT, TESTS, LinRegTInt}.
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Choose STAT.
\item Right arrow to TESTS.
...
\item Leave RegEQ blank.
\item Choose Caculate and hit ENTER, which returns: \\
\begin{tabular}{l l}
