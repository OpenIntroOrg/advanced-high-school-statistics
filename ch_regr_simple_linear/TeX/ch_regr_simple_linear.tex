...
\sectionintro{
\noindent%
In this section, we examine criteria for identifying a linear model and introduce a new statistic called \emph{correlation}.  We answer questions such as the following:
\begin{itemize}
...
We are interested in how well a model accounts for or explains the location of the $y$ values.  Roughly speaking, the $R^2$ of a linear model describes how much smaller the variance (in the $y$ direction) about the regression line is than the variance about the horizontal line $\bar{y}$.
...
$$\frac{s_{aid}^2 - s_{_{RES}}^2}{s_{aid}^2}
	...
	= 0.25$$
If we use $s_{_{RES}}^2$ as computed by the linear regression
model, this is close to the $R^2$ value.
More technically, we calculate $R^2$ as the reduction in the sum of the squared error about the line.  If we call the sum of the squared errors about the regression line $SSRes$ and the sum of the squared errors about the mean $SSM$, we can define $R^2$ as follows:
\begin{align*}
