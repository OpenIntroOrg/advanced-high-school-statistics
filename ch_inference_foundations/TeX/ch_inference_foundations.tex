...
\begin{figure}[hht]
   \centering
   \includegraphics[width=\textwidth]{ch_inference_foundations/figures/95PercentConfidenceInterval/95PercentConfidenceInterval}
...
\Comment{Changed "z* corresponds to the confidence level" to "z* depends on the confidence level", since corresponds sounds to me like it is equivalent}
...
\Comment{TODO(David) Can you change the t table in the appendix to look like this t table???  This is more like how the t table looks like on the formula sheet they have to use for AP exam}
...
\begin{tipBox}{\tipBoxTitle{Finding $z^{\star}$ for a particular confidence level}We select $z^{\star}$ so that the area between -$z^{\star}$ and $z^{\star}$ in the normal model corresponds to the confidence level.  Use the t table at row $\infty$ to find the critical value $z^{\star}$.}
\end{tipBox}
...
\label{marginOfErrorTermBox}A confidence interval can be written as point estimate $\pm$ margin of error.  
...
The null claim is always framed as an equality: it tells us what quantity we should use for the parameter when carrying out calculations for the test of hypothesis.  There are three choices for the alternative hypothesis, depending upon whether the researcher is trying to prove that the value of the parameter is greater than, less than, or not equal to the null value.
...
\begin{exercise}
According to US census data, in 2012 the percent of male residents in the state of Alaska was 52.1\%.\footnote{\urlwofont{http://www.census.gov/newsroom/releases/archives/population/cb13-112.html}}  A researcher plans to take a random sample of residents from Alaska to test whether or not this is still the case.  Write out the hypotheses that the researcher should test in both plain and statistical language.~\footnote{$H_0$: $p=0.521$; The proportion of male residents in Alaska is \emph{unchanged} from 2012.   $H_A$: $p \neq 0.521$; The proportion of male residents in Alaska has changed from 2012.  Note that it could have increased or decreased.  }
\end{exercise}
...
%\Comment{add a couple more examples of 1, 2 sided}.
...
The complication rate in the consultant's sample of size 62 was 0.048 ($0.048 \times 62 \approx 3$). What is the probability that a sample would produce a number of complications this far from the expected value of 6.2, \emph{if her true complication rate were} 0.10, that~is, if $H_0$ were true.  The probability, which is estimated in the section that follows, turns out the be 0.2444.  We call his quantity the \term{p-value}.

\Comment{I think the first sentence alone does not make any sense to the students.  When asked to interpret the pvalue on a test, they will not be able to unless they parrot this sentence, but they won't know what it means.}

\Comment{TODO(DAVID), please add graph, showing how p-value corresponds to both tails for a not equal to HA, the right tail for a > HA, and the left tail for a < HA}

\begin{termBox}{\tBoxTitle{Interpreting the p-value\color{red}FIX}
The \term{p-value}\index{hypothesis testing!p-value|textbf} is the probability of observing data at least as favorable to the alternative hypothesis as our current data set, if the null hypothesis were true. 
\newline More specifically, when examining a a proportion we can interpret the p-value as follows, depending upon the nature of the alternative hypothesis.  
\begin{description}
...
%\CommentA{I added this section to try to clarify the logic of a test, since it is so important .}
%
%\Add{
%The p-value is actually a conditional probability.  It is, for example: 
%\begin{align*}
%P\text{(getting data at least as far from the null value as we observed $|$ $H_0$ is true)}
%\end{align*}
%
%It is NOT: $P(H_0$ is true $|$ we got data this far from the null value)
%
%\noindent In a test of hypothesis, we always begin by assuming the null hypothesis is true, and then we calculate the likelihood of getting sample data such as we observed.  If this probability is very small, that causes us to doubt the original hypothesis.  This type of argument is known as an \emph{argument by contradiction}.
%
%\begin{example}{Consider the following scenario.  Mika walks into a restaurant.  She buys and eats a hamburger.  Is she a vegetarian?  How do we know?}She cannot be a vegetarian.  The argument by contradiction is as follows:  if she were a vegetarian, she would not eat a hamburger.  But she \emph{did} eat a hamburger, so she is not a vegetarian.
%\end{example}
%
%\begin{example}{Consider the following scenario.  James walks into a restaurant.  He buys and eats a grilled cheese sandwich.  Is he a vegetarian?  How do we know?}In this case, the argument by contradiction does not work because there is no contradiction.  We have do not have evidence that he is not a vegetarian.  However, this does not imply that he must be a vegetarian.  Even carnivores can enjoy a grilled cheese sandwich!  
%\end{example}
%
%\begin{example}{When the p-value is very small, we reject the null hypothesis.  When the p-value is not very small, we do not reject the null hypothesis.  Using the examples above as a guide, explain why this is the case.}We assume that the null hypothesis is true.  A small p-value means that our sample observation is unlikely if the null hypothesis is true.  But we did get our sample observation, so the null hypothesis seems faulty.  A large p-value means that our sample observation is not unlikely if the null hypothesis is true.  There is no inconsistency here so we do not have reason to reject the null hypothesis.
%\end{example}
%
%\begin{tipBox}{\tipBoxTitle{Smaller p-values provide greater evidence  \emph{against} null hypothesis.}
%The smaller the p-value, the less likely it is that the sample result can be explained by random variation and the greater the evidence there is against the null hypothesis.  
%
...
%
%\Add{
%We typically use a summary statistic of the data, such as a Z score, to help compute the p-value and evaluate the~hypotheses.  When the alternative hypothesis is a $>$, we use the area upper tail as the p-value.  When the alternative hypothesis is a $<$ we use the area in the lower tail as the p-value.  When the alternative is a $\ne$ we add up the two tails to get the p-value.
%}
%
%\CommentA{TODO(David) IMPORTANT add figure to show right, left, and both tails shaded.}.
%
...
%\end{termBox}
%
%Recall that the null claim is the claim of no difference.  If we reject $H_0$, we are asserting that there is a real difference.  If we do not reject $H_0$, we are saying that the null claim is \emph{reasonable}. That is, we have not disproved it.
%
...
%No. We did not prove that the consultant's complication rate is \emph{exactly} equal to 10\%.  Recall that the test of hypothesis starts by \emph{assuming the null claim is true}.  \emph{If the null claim is true}, there is a 0.2444 chance of seeing a sample proportion as far from 10\% as we saw in our sample.  Because 0.2444 is large, it is within the realm of chance error and we cannot say the null hypothesis is unreasonable.
%\end{example}
%
%
...
%\end{tipBox}
%
%\begin{example}{Does the conclusion in Guided Practice~\ref{plainLanguageExplanationOfHTConclusionForLiverDonorSurgicalConsultant} imply for certain there is no real association between the surgical consultant's work and the risk of complications? Explain.}
...
$\alpha$\\\footnotesize significance\\level of a\\hypothesis test}, is typically set to $\alpha = 0.05$, but can vary depending on the field or the application. Using a  significance level of $\alpha = 0.05$ in the discrimination study, we can say that the data \Add{did not }provide statistically significant evidence against the null hypothesis, because the p-value of 0.2444 was larger than the $\alpha$ of 0.05.

\begin{termBox}{\tBoxTitle{Statistical significance\color{red}ADD THIS BOX}
-If the p-value is less than the significance level $\alpha$ (usually 0.05), we say that the result is statistically significant.  We reject $H_0$ and we have evidence for $H_A$.
\\
\newline -If the p-value is greater than the significance level $\alpha$ (usually 0.05), we say that the result is not statistically significant.  We do not reject $H_0$ and we do not have evidence for $H_A$.}
\end{termBox}

\begin{termBox}{\tBoxTitle{Statistical significance\color{red}CUT THIS BOX}
We say that the data provide \term{statistically significant}\index{hypothesis testing!statistically significant|textbf} evidence against the null hypothesis if the p-value is less than some reference value, usually~$\alpha=0.05$.}
\end{termBox}

Recall that the null claim is the claim of no difference.  If we reject $H_0$, we are asserting that there is a real difference.  If we do not reject $H_0$, we are saying that the null claim is \emph{reasonable}\Add{, not have \emph{proven} it.}  
...
\begin{example}{Does the conclusion in Guided Practice~\ref{plainLanguageExplanationOfHTConclusionForLiverDonorSurgicalConsultant} imply for certain there is no real association between the surgical consultant's work and the risk of complications? Explain.}
No. \Cut{It might be}\Add{It is not impossible} that the consultant's work is associated with a lower or higher risk of complications. However, the data did not provide enough information to reject the null hypothesis. %However, we currently don't have enough data to say whether the corresponding complication rate is any different than 0.10.
\index{data!medical consultant|)}
\end{example}
...
Statistical inference is the practice of making decisions and conclusions from data in the context of uncertainty. \Add{Just as a confidence interval may occasionally fail to capture the true parameter, a test of hypothesis may occasionally lead us to an incorrect conclusion.}\Cut{Errors do occur, just like rare events, and the data set at hand might lead us to the wrong conclusion.} While a given data set may not always lead us to a correct conclusion, statistical inference gives us tools to control and evaluate how often these errors occur.
...
\Comment{moved this section to before Decision errors}
...
\item Verify conditions to ensure the standard error estimate is reasonable and the point estimate follows appropriate distribution and is unbiased.
...
The jury considers whether \Cut{the evidence is so convincing (strong) that there is no }\Add{there is evidence beyond a }reasonable doubt \Add{(the significance level) }regarding the person's guilt. That is, the skeptical perspective (null hypothesis) is that the person is innocent until evidence is presented that convinces the jury that the person is guilty (alternative hypothesis).
\end{example}

Jurors examine the evidence to see whether it convincingly shows a defendant is guilty. \Add{Notice that a jury finds a defendent either guilty or not guilty.  They either reject the null claim or they do not reject the null claim.  They never prove the null claim, that is, they never find the defendant innocent.}  If a jury finds a defendant \emph{not guilty}, this does not necessarily mean the jury is confident in the person's innocence. They are simply not convinced of the alternative that the person is guilty.
...
\Comment{Made this into a box}

\begin{termBox}{\tBoxTitle{Type I and Type II Errors}
A \term{Type~1 Error} is rejecting the null hypothesis when $H_0$ is actually true. When we reject the null hypothesis, it is possible that we make a Type~1 Error.  

A \term{Type~2 Error} is failing to reject the null hypothesis when $H_A$ is actually true.  When we do not reject the null hypothesis, it is possible that we make a Type~2 Error.}
\end{termBox}
...
\Comment{we never introduced the idea that significance level alpha is the probability of a Type 1 error!   Added the paragraph as well as the term box and the three examples.}

\Add{If $H_0$ is true, what is the probability that we will incorrectly reject it?  In our test of hypothesis, we assume that $H_0$ is true and we reject it if we get a p-value smaller than the significance level $\alpha$.  Thus $\alpha$ \emph{is} the probability of making a Type~1 Error.  The choice of what to make $\alpha$, then, is not arbitrary.  It depends on the gravity of the consequences of a Type~1 Error.  }

\begin{termBox}{\tBoxTitle{Probabiliy of Type I and Type II Errors}
The probability of a Type~1 Error is called $\alpha$ and corresponds to the significance level of a test.
The probability of a Type~2 Error is called $\beta$.
As we make $\alpha$ smaller, $\beta$ gets larger, and vice versa.}
\end{termBox}

\begin{example}{If making a Type~1 Error is especially dangerous or especially costly, should we choose a smaller significance level or a higher significance level?}Under this scenario, we want to be very cautious about rejecting the null hypothesis, so we demand very strong evidence before we are willing to reject the null hypothesis.  Therefore, we want a smaller significance level, maybe an $\alpha$ of 0.01.
\end{example}

\begin{example}{If making a Type~1 Error is especially dangerous, as in the case of convicting an innocent person, should we reduce our significance level to 0.000001?}While doing this would mean that we would almost never convict innocent people, the standard of proof would be so high that we would not convict anyone!  Thus our Type~1 Errors would be extremely low, but our Type~2 Errors (letting guilty people free) would be astronomically high.  It is a challenging balancing act.
\end{example}

\begin{example}{If making a Type~2 Error is especially dangerous or especially costly, should we choose a smaller significance level or a higher significance level?}We should choose a higher significance level (e.g. 0.10). Here we want to be cautious about failing to reject $H_0$ when the null is actually false.
\end{example}


\Cut{Choosing a significance level for a test is important in many contexts, and the traditional level is 0.05. However, it is sometimes helpful to adjust the significance level based on the application. We may select a level that is smaller or larger than 0.05 depending on the consequences of any conclusions reached from the test.
...
\Comment{moved this section to connect to Decision errors}
...
The Type 2 Error rate $\beta$ and the magnitude of the error for a point estimate are controlled by the sample size. \Add{As the sample size $n$ goes up, the Type 2 Error rate goes down, and power goes up.}  Real differences from the null value, even large ones, may be difficult to detect with small samples. However, if we take a very large sample, we might find a statistically significant difference but the magnitude might be so small that it is of no practical value.
