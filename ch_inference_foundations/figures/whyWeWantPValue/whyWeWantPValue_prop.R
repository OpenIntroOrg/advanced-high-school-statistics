...
pdf('whyWeWantPValue_prop.pdf', 6.3, 2.5)
...
axis(1, at=c(-10, -5, 0, 5), expression('', 'null value - '*5*' x SE   ', 'null value', '')) #at=c(-4, -2, 0, 2, 4))
abline(h=0) #, col='#00000088')
yMax <- max(Y)

text(0, yMax/2-0.02, expression('Distribution of '*hat(p)), cex=1.1, col=COL[5])
text(0, yMax/3-0.01, expression('if '*H[0]*' was true'), cex=1.1, col=COL[5])

arrows(-5,yMax/3, -5,yMax/20, length=0.1, lwd=1.5)
text(-5, yMax/3, expression('observed '*hat(p)), cex=1.1, pos=3)
