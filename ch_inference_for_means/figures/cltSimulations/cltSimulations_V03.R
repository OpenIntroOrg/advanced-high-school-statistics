...
plot(0:1, 0:1, axes=FALSE, type='n',yaxs="i")
text(0.5, 0.47, 'population\ndistributions', cex=1.4)

par(mar=c(3, 1, 0, 1))
plot(X, Yu, type='s', xlab='', ylab='', yaxs="i",xlim=c(-2, 6), ylim=c(0, 0.75), axes=FALSE, col=COL[1], lwd=2)
text(1, 0.77, 'uniform', pos=1, cex=1.7)
axis(1, at=seq(-2, 6, 2), cex.axis=cex.axis)
abline(h=0)

plot(X, Ye, type='l', xlab='', ylab='', yaxs="i",xlim=c(-2, 6), ylim=c(0, 83/60), axes=FALSE, col=COL[1], lwd=2)
text(1, 86/60, 'right skewed', pos=1, cex=1.7)
axis(1, at=seq(-2, 6, 2), cex.axis=cex.axis)
abline(h=0)

plot(Xn, Yn, type='l', xlab='', ylab='', yaxs="i",xlim=c(-2, 6), ylim=c(0, 83/60), axes=FALSE, col=COL[1], lwd=2)
...
for(i in 1:length(n)){
	...
	if(n[i] != 2){
		...
		R <- range(c(0, d$y))*1.1
		plot(d, main='', xlab='', ylab='', yaxs="i",axes=FALSE,
		...
		plot(X, Y, axes=FALSE, type='l',yaxs="i",
	...
	plot(d$x, d$y, type='l', main='',yaxs="i",
