...
myPDF('netTime1000SamplingDistribution17.pdf', 6.5, 3.2,
      mar = c(4, 4, 1, 1),
      mgp = c(2.7,0.7,0))
...
means <- c()
for (i in 1:1000) {
  temp <- sample(nrow(run17), 100)
  means[i] <- mean(run17$net_sec[temp], na.rm=TRUE)
}
...
plot(0, 0,
     ...
     yaxs="i",
     axes = FALSE)
m <- mean(run17$net_sec, na.rm = TRUE)
s <- sd(run17$net_sec, na.rm = TRUE)/10
histPlot(means, col = COL[1], breaks = 25, add = TRUE)
abline(h = 0)
axis(1, at = seq(0, 200, 10))
axis(1, at = seq(0, 200, 10) + 5, rep("", 21), tcl = -0.15)
