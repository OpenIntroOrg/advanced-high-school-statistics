...
%\Comment{need to add in problem about finding the minimum sample size for a certain margin of error and confidence level such as 49 and 50 from original 04e.tex from openintro.  However, for the ap problems, sigma is just given, so that students don't have to use a n to find t*, they can just use z* - this should maybe be changed.}
%\Comment{need to add in conceptual questions about ci for mean, such as 9-12 of original 04e.tex from openintro }
%\Comment{deleted first set of questions on the sections prior to the t distribution}.
