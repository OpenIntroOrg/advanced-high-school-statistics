...
\begin{figure}
\centering
  \Figure[A t-distribution with 18 degrees of freedom is shown, where the region below -2.10 is shaded and appears to represent very roughly 2\% to 5\% of the distribution. For the most part, when the degrees of freedom are larger than about 10, like in this case, the differences between the t-distribution and the normal distribution are visually subtle, even if the distinction remains important for our calculations.]
{0.53}{tDistDF18LeftTail2Point10}
...
\begin{figure}
\centering
 \Figure[Two t-distributions are shown on two separate plots. The first plot shows a t-distribution with 3 degrees of freedom with the region below approximately -3 and above positive 3 shaded.  Each tail appears to be roughly 2.5\% of the total distribution area. The second plot shows a t-distribution with 20 degrees of freedom with the region above 1.65 shaded.  The area in this right-hand tail appears to be approximately 5 or 6\% of the total distribution area.]
{0.93}{tDistDF3_and_20}
...
%%
\subsection[Technology: finding area under the $t$-distribution]{Technology: finding area under the \pmb{$t$}-distribution}
...
Using the $t$-distribution for inference on a mean requires that the theoretical \textbf{sampling distribution for the sample mean $\bar{x}$ is nearly normal}.  In practice, we check whether this assumption is reasonable by verifying that certain conditions are met.  
...
\item[Sample size / nearly normal population.]
  We saw in Section~\ref{distributionofxbar} that in order for
  the sampling distribution for a sample mean to be nearly normal,
...
For larger samples, it is less necessary to check for skew in the data.  If the sample size is 30 or more, it is no longer necessary that the population distribution be nearly normal.  When the sample size is large, the Central Limit Theorem tells us that the sampling distribution for the sample mean will be nearly normal regardless of the distribution of the population.
...
\begin{itemize}
\item[] To estimate a single mean we use a \termsub{1-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a mean}.
\end{itemize}
\inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
%%
\subsection[Technology: the 1-sample $t$-interval]{Technology: the 1-sample \pmb{$t$}-interval}
\label{1SampTint}
...
The data come from a random sample from a large population, so the observations are independent. Do we need to check for skew in the data? No -- with a sample size of \cherryblossomn{}, well over 30, the Central Limit Theorem tells us that the sampling distribution for $\bar{x}$ will be nearly normal.
...
\begin{itemize}
\item[] To test hypotheses about a single mean we use a \termsub{1-sample \pmb{$t$}-test}{t-test@$t$-test!for a mean}.
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
%%
\subsection[Technology: the 1-sample $t$-test]
           {Technology: the 1-sample $\pmb{t}$-test}
\label{1SampTtest}
...
\begin{itemize}
\item Estimate $\mu$ at the C\% confidence level using a \termsub{1-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a mean}.
\item Test $H_0$: $\mu=\mu_0$ at the $\alpha$ significance level using a \termsub{1-sample \pmb{$t$}-test}{t-test@$t$-test!for a mean}. 
\end{itemize}

\item The one-sample $t$-interval and $t$-test require that the sampling distribution for $\bar{x}$ be nearly normal.  For this reason we must check that the following conditions are met.
\begin{itemize}
...
\item Recognize that inference procedures with paired data use the same one-sample $t$-procedures as in the previous section, and that these procedures are applied using the \emph{differences} of the paired observations.

\item Carry out a complete hypothesis test with paired data.

\item Carry out a complete confidence interval procedure with paired data.
...
\caption{Histogram of the difference in price for
   ...
   Explore this data set on Tableau Public\tableauhref{tableau-hist-textbookdiff-summ}.}
...
for $\bar{x}_{\text{\emph{diff}}}$ is drawn as though
...
\begin{figure}[h]
\centering
\Figures[A bell-shaped distribution is shown, with a center of mu-sub-0, which has a value of 0. The area under the distribution above x-bar-sub-diff equals 3.58 is shaded, as is the corresponding tail below -3.58.]{0.65}{textbooksF18}{textbooksF18HTTails}
...
\inferencestep{Choose} Choose the appropriate test procedure and identify it by name.  \vspace{-1mm}
\begin{itemize}
\item[] To test hypotheses about a mean of differences we use a \termsub{1-sample \pmb{$t$}-test}{t-test@$t$-test!for a mean of differences} with paired data.
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}_{\text{\emph{diff}}}$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
{0.38}{satImprovementHTDataHistogram}
...
\item[\inferencestep{Choose}] Because we have paired data and the parameter to be estimated is a mean of differences, we will use a 1-sample $t$-test with paired data.

\item[\inferencestep{Check}] We have a random sample of students and have paired data on them.  We will assume that this sample of size 30 represents less than 10\% of the total population of such students.  Finally, the number of differences is $n_{\text{\emph{diff}}}=30\ge 30$, so we can proceed with the 1-sample $t$-test.  
 
\item[\inferencestep{Calculate}]  We will calculate the test statistic, $df$, and p-value.
\begin{align*}
T = \frac{\text{point estimate } - \text{ null value}}{SE \text{ of estimate}}
\end{align*}
The point estimate is the sample mean of differences:  $\bar{x}_{\text{\emph{diff}}}  = 135.9$\\[1mm]
The $SE$ of the sample mean of differences is:  $\frac{s_{\text{\emph{diff}}} }{\sqrt{n_{\text{\emph{diff}}} }} = \frac{82.2}{\sqrt{30}} = 15.0$ \\[1mm]
Because this is a one-sample $t$-test, the degrees of freedom is $n_{\text{\emph{diff}}}-1$.
\begin{align*}
...
%%
\subsection[Technology: the 1-sample $t$-test with paired data]
           {Technology: the 1-sample $\pmb{t}$-test with paired data}
...
\begin{onebox}{Casio fx-9750GII: 1-sample T-test with paired data}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\begin{onebox}{Constructing a confidence interval for a mean of differences}
...
\inferencestep{Choose} Choose the appropriate interval procedure and identify it by name. \vspace{-1mm}
\begin{itemize}
\item[] To estimate a mean of differences we use a \termsub{1-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a mean of differences} with paired data.
\end{itemize}
\inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}_{\text{\emph{diff}}}$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\item[\inferencestep{Choose}] Because we have paired data and the parameter to be estimated is a mean of differences, we will use a 1-sample $t$-interval with paired data.

\item[\inferencestep{Check}] We have a random sample of students with paired observations on them.  We will assume that these 30 students represent less than 10\% of the total number of such students.  Finally, the number of differences is $n_{\text{\emph{diff}}}=30\ge 30$, so we can proceed with the 1-sample $t$-interval.  
...
\caption{SAT score after course minus the SAT score before course. }
...
\subsection[Technology: the 1-sample $t$-interval with paired data]
           {Technology: the 1-sample $\pmb{t}$-interval with paired data}

When carrying out a  1-sample $t$-interval with paired data,
make sure to use the sample \emph{differences} or the summary statistics
for the \emph{differences}. 
...
\begin{onebox}{Casio fx-9750GII: 1-sample T-interval with paired data}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item Compute the differences of the paired observations.
...
\begin{itemize}
\item Estimate $\mu_{\text{\emph{diff}}}$ at the C\% confidence level using a \termsub{1-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a mean of differences} with paired data.
\item Test $H_0$: $\mu_{\text{\emph{diff}}}=0$ at the $\alpha$ significance level using a \termsub{1-sample \pmb{$t$}-test}{t-test@$t$-test!for a mean of differences} with paired data. 
\end{itemize}

\item The one-sample $t$-interval and $t$-test with paired data require the sampling distribution for $\bar{x}_{\text{\emph{diff}}}$ to be nearly normal.  For this reason, we must check that the following conditions are met.
\begin{itemize}
...
%%
\subsection{Sampling distribution for the difference of two means (review)}
\label{differenceOfTwoMeans}
In this section we are interested in comparing the means of two independent groups.
We want to estimate how far apart $\mu_1$ and $\mu_2$ are and test whether their difference is zero or not.
Before we perform inference for the difference of means, let's review the sampling distribution for $\bar{x}_1-\bar{x}_2$, which will be used as the point estimate for $\mu_1 - \mu_2$.
We know from Section~\ref{distributionofdifference} that when the
independence condition is satisfied, the sampling distribution for
$\bar{x}_1-\bar{x}_2$ is centered on $\mu_1-\mu_2$ and has standard
deviation of
\begin{align*}
\sigma_{\bar{x}_{1} - \bar{x}_{2}}= \sqrt{\frac{\sigma_{1}^2}{n_{1}} + \frac{\sigma_{2}^2}{n_{2}}}
\end{align*}
When the individual population standard deviations are unknown, we estimate the standard deviation of $\bar{x}_1-\bar{x}_2$ using the Standard Error, abbreviated $SE$, by plugging in the sample standard deviations as our best guesses of the population standard deviations:
\begin{align*}
SE_{\bar{x}_{1} - \bar{x}_{2}}= \sqrt{\frac{s_{1}^2}{n_{1}} + \frac{s_{2}^2}{n_{2}}}
\end{align*}

The difference of two sample means $\bar{x}_1-\bar{x}_2$ follows a nearly normal distribution when certain conditions are met.  First, the sampling distribution for each sample mean must be nearly normal, and second, the observations must be independent, both within and between groups. Under these two conditions, the sampling distribution for $\bar{x}_1 - \bar{x}_2$ may be well approximated using the normal model.



%%%
\subsection{Checking conditions for inference on a difference of means}
When comparing two means, we carry out inference on a difference of means, $\mu_1-\mu_2$.  We will use the $t$-distribution just as we did when carrying out inference on a single mean.  In order to use the $t$-distribution, we need the \textbf{sampling distribution for $\bar{x}_1-\bar{x}_2$ to be nearly normal}.  We check whether this assumption is reasonable by verifying the following conditions.
\begin{description}
\setlength{\itemsep}{0mm}
\item[Independence.]
  Observations can be considered independent when the data are
  collected from two independent random samples or from a
  randomized experiment with two treatments.
  Randomly assigning subjects to treatments is equivalent
  to randomly assigning treatments to subjects.
  When sampling without replacement, the observations can be
  considered independent when the sample size is less than 10\%
  of the population size for both samples.
\item[Sample size / nearly normal population.]
  ...
  we look at the data for excessive skew or outliers.
  If we do not find excessive skew or outliers in either group,
  the assumption that the populations are nearly normal to be
  reasonable is typically considered reasonable.
\end{description}
...
\end{itemize}
 \inferencestep{Choose} Choose the appropriate interval procedure and identify it by name.  \vspace{-1mm}
\begin{itemize}
\item[] To estimate the difference of means we use a \termsub{2-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a difference of means}.
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}_1-\bar{x}_2$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
%%
\subsection[Technology: the 2-sample $t$-interval]
           {Technology: the 2-sample $\pmb{t}$-interval}
\label{2SampTint}
...
\end{itemize}
 \inferencestep{Choose} Choose the appropriate test procedure and identify it by name. \vspace{-1mm}
\begin{itemize}
\item[] To test hypotheses about the difference of means we use a \termsub{2-sample \pmb{$t$}-test}{t-test@$t$-test!for a difference of means}.
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution for $\bar{x}_1-\bar{x}_2$ to be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
%%
\subsection[Technology: the 2-sample $t$-test]
           {Technology: the 2-sample $\pmb{t}$-test}
\label{2SampTtest}
...
\begin{itemize}\vspace{-1mm}
\setlength{\itemsep}{0mm}
\item Estimate $\mu_1-\mu_2$ at the C\% confidence level using a \termsub{2-sample \pmb{$t$}-interval}{t-interval@$t$-interval!for a difference of means}.
\item Test $H_0$: $\mu_1-\mu_2=0$ at the $\alpha$ significance level using a \termsub{2-sample \pmb{$t$}-test}{t-test@$t$-test!for a difference of means}.
\end{itemize}

\item The 2-sample $t$-test and $t$-interval require the sampling distribution for $\bar{x}_1-\bar{x}_2$ to be nearly normal.  For this reason we must check that the following conditions are met.
\begin{itemize}
...
%%%%%Section exercises
{\input{ch_inference_for_means/TeX/inference_for_the_difference_of_two_means.tex}}
