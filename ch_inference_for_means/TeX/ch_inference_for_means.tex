...
\Add{
\begin{example}{For the $t$ distribution with 18 degrees of freedom, what percent of the curve is contained between -1.330 and +1.330?}  Using row 18, we find 1.330 in the table.  The area in each tail is 0.100, so that leaves 0.800 in the middle.  We can easily see this by following 1.330 down to the bottom row and see that it says 80\%.  So 80\% of the $t$ curve with 18 degrees of freedom falls between -1.330 and +1.330.
\end{example}
}

\Add{
\begin{example}{For the $t$ distribution with 3 degrees of freedom, what value of $t$ captures the middle 95\% of the area of the curve?}We can look at the column that says 95\% at the bottom (2.25\% in each tail) and trace it up to row 3 to find that the $t$ such that 95\% of the area is between $-t$ and $+t$ is 2.535.
\end{example}
}
...
\Comment{favor cutting this example and therefore right pannel of figure 7.5}
\Cut{
...
\Add{What should we do if the desired degrees of freedom is not listed on the table?  To be conservative, we will just round the degrees of freedom down to the next smallest value that is on the table.}
...
Dolphins are at the top of the oceanic food chain, which causes dangerous substances such as mercury to concentrate in their organs and muscles. This is an important problem for both dolphins and other animals, like humans, who occasionally eat them. \Cut{For instance, this is particularly relevant in Japan where school meals have included dolphin at times.}
...
The observations are a simple random sample and consist of less than 10\% of the population, therefore independence is reasonable. \Add{To check the normality condition of the population, we would like to graph the data from the sample.  However, we do not have all of the data so we will have to look at the summary statistics provided in }Table~\ref{summaryStatsOfHgInMuscleOfRissosDolphins}.  These summary statistics do not suggest any skew or outliers; all observations are within 2.5 standard deviations of the mean. Based on this evidence, the normality assumption seems reasonable.
\end{example}
...
\Comment{L: I added the actual formula for SE to the term box below.}
...
\Comment{ in example below, I cut that part because we are using the SD of the population.}
...
\Comment{I broke up the equation in the box below, because students look at the longer equation and don't understand where to plug in stuff - they don't get that they don't need to plug in something for ME and for m.}
...
\begin{exercise}
The data come from a simple random sample from less than 10\% of all participants, so the observations are independent. However, should we be worried about skew in the data? A histogram \Cut{of the differences was shown in the left panel of}\Add{of the times is shown in } Figure~\ref{run10SampHistogramsInHTForMeanSubsection}.~\footnote{Since the sample size 100 is greater than 30, we do not need to worry about slight skew in the data.}
\end{exercise}
