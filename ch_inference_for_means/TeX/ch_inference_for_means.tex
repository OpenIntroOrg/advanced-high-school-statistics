...
%__________________
\section[Inference for a single mean with the $t$-distribution]{Inference for a single mean with the $t$-distribution \sectionvideohref{coursera_video-t_distribution} \sectionvideohref{coursera_video-inf_for_mean} \sectionslideshref{gdoc_aps_slides_7-1}}
...
We will find it very useful to become familiar with the $t$-distribution, because it plays a very similar role to the normal distribution during inference for numerical data. We use a \term{$t$-table}, partially shown in Table~\ref{tTableSample_ch_inf_means}, in place of the normal probability table for numerical data when the population standard deviation is unknown, especially when the sample size is small. A~larger table is presented in Appendix~\ref{tDistributionTable}.
...
$t^{\star}_{df}$\vspace{1mm}\\\footnotesize Multiplication\\factor for\\$t$-interval]{\raggedright\vspace{-9mm}
...
\begin{termBox}{\tBoxTitle{Finding a $t$-interval for the mean}
...
\begin{termBox}{\tBoxTitle{Constructing a confidence interval for a mean}
To carry out a complete confidence interval procedure to estimate a single \mbox{mean $\mu$,}
\\
\\
\inferencestep{Identify} \vspace{-1mm}
\begin{itemize} 
\item[] the parameter, e.g. the true mean (or average) miles per gallon of a 2018 Honda Civic, and the confidence level, C\%, to be used.
\end{itemize}
...
\item[] 2. $n\ge 30$, OR population is known to be approximately normal, OR 
\item[] \quad \ population could be approximately normal because data show no strong 
\item[] \quad \ skew or outliers.  (If arguing the third case, make sure to also sketch a 
\item[] \quad \ graph of the data!)
\end{itemize}
...
\begin{itemize}
\item[] We are C\%  confident that the true \emph{mean} of [...] is between \underline{\ \ \ \ \ } and  \underline{\ \ \ \ \ }. If applicable, draw a conclusion based on whether the interval is entirely above, is entirely below, or contains the value of interest. 
\end{itemize}}
\end{termBox}
...
\end{itemize}
 \inferencestep{Conclude}   \vspace{-1mm}
\begin{itemize}
\item[] The p-value is $< \alpha$, so we reject $H_0$; there is sufficient evidence that [$H_A$ in context]. \quad OR
...
%__________________
\section[Inference for paired data]{Inference for paired data \sectionvideohref{coursera_video-inf_paired_means} \sectionslideshref{gdoc_aps_slides_7-2}}
...
\begin{itemize}
\setlength{\itemsep}{0mm}
\item[] 1. There is paired data from a random sample or from a matched pairs
 \item[] \quad \  experiment.
\item[] 2. $n_{\text{\emph{diff}}}\ge 30$, OR population of differences is known to be approximately
 \item[] \quad \  normal, OR population of differences could be approximately normal \item[] \quad \ because sample differences show no strong skew or outliers.  (If arguing 
\item[] \quad \ the third case, make sure to also sketch a graph of the sample differences!)
\end{itemize}
 \inferencestep{Calculate}  
\begin{itemize}
...
We construct a 95\% confidence interval for the average price difference between books at the UCLA bookstore and books on Amazon. Conditions have already verified and the standard error computed in Example~\ref{htForDiffInUCLAAndAmazonTextbookPrices}. To find the interval, identify $t
...
\begin{termBox}{\tBoxTitle{Constructing a confidence interval for paired data}
To carry out a complete confidence interval procedure to estimate a mean of differences $\mu_{\text{\emph{diff}}}$,
\\
\\
\inferencestep{Identify} \vspace{-1mm}
\begin{itemize} 
\item[] the parameter, e.g. the true mean of the differences in county population (year 2018 $-$ year 2017), and the confidence level, C\%, to be used.
\end{itemize}
\inferencestep{Choose} \vspace{-1mm}
\begin{itemize}
...
\begin{itemize}
\setlength{\itemsep}{0mm}
\item[] 1. There is paired data from a random sample or paired data from a matched \item[] \quad \ pairs experiment.
\item[] 2. $n_{\text{\emph{diff}}}\ge 30$, OR population of differences is known to be approximately
 \item[] \quad \  normal, OR population of differences could be approximately normal \item[] \quad \ because sample differences show no strong skew or outliers.  (If arguing 
\item[] \quad \ the third case, make sure to also sketch a graph of the sample differences!)
\end{itemize}
...
\item[] point estimate: the sample mean of differences $\bar{x}_{\text{\emph{diff}}}$
\item[] $SE$ of estimate:  $\frac{s_{\text{\emph{diff}}}}{\sqrt{n_{\text{\emph{diff}}}}}$
\item[] $t^{\star}$: use a $t$-table at row $df = n_{\text{\emph{diff}}} - 1$ and confidence level C\%
\end{itemize}
\item[] (\underline{\ \ \ \ \ }, \underline{\ \ \ \ \ })
\end{itemize}
\inferencestep{Conclude}\vspace{-1mm}
...
\begin{exercise}
Use the first 7 values of the ~\ref{textbooksDF} data set produced above and calculate the $T$ score and p-value to test whether, on average, Amazon's textbook price is cheaper that UCLA's price.\footnote{Create a list of the differences, and use the data or list option to perform the test. Let $\mu_0$ be 0, and select the appropriate list. Freq should be 1, and the test sidedness should be $>$. $T=3.076$ and p-value$=0.0109$.}
\end{exercise}

\begin{exercise}
Use the data from Table~\ref{textbooksDFPartial} to calculate a 95\% confidence interval for the average difference in textbook price between Amazon and UCLA.\footnote{Choose a C-Level of 0.95, and the final result should be (0.80354, 7.0507).}
\end{exercise}

%__________________
\section[Difference of two means using the $t$-distribution]{Difference of two means using the $t$-distribution \sectionvideohref{coursera_video-inf_two_means}~\sectionslideshref{gdoc_aps_slides_7-3}}
...
\item[] 2.  $n_1\ge 30$ and $n_2\ge 30$, OR both populations are known to be approximately\item[] \quad \  normal, OR both populations could be approximately normal because 
\item[] \quad \ both data sets show no strong skew or outliers. (If arguing the third case, 
\item[] \quad \ make sure to also graph both data sets!)
\end{itemize}
 \inferencestep{Calculate}  
\begin{itemize}
...
%__________________
\section[Comparing many means with ANOVA (special topic)]{Comparing many means with ANOVA\\(special topic) \sectionslideshref{gdoc_aps_slides_7-4}}
