...
\sectionintro{
\Add{
\noindent In this section we turn our attention to numerical variables and we answer questions such as the following:  
\begin{itemize}
...
%%
%%
\subsection{Using a normal distribution for inference when $\sigma$ is known}
...
We will find it useful to become familiar with the $t$-distribution, because it plays a very similar role to the normal distribution during inference.  We use a \termsub{$\pmb{t}$-table}
    {t-table@$t$-table}, partially shown in Table~\ref{tTableSample_ch_inf_means}, in place of the normal probability table when the population standard deviation is unknown, especially when the sample size is small. A~larger table is presented in Appendix~\ref{tDistributionTable}.
...
\begin{figure}
\centering
\href{\oiRedirectUrl{tableau-hist-textbookdiff-summ}}{
