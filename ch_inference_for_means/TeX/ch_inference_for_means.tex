...
\caption{The larger the degrees of freedom, the more closely the $t$-distribution resembles the standard normal distribution.\textA{\vspace{-3mm}}}
\label{tDistConvergeToNormalDist}
\end{figure}

\begin{onebox}{Degrees of freedom (df)}
The degrees of freedom describe the shape of the $t$-distribution. The larger the degrees of freedom, the more closely the distribution resembles the standard normal~distribution.\end{onebox}

When the degrees of freedom is large, about 100 or more, the $t$-distribution is nearly indistinguishable from the normal distribution. In Section~\ref{tDistSolutionToSEProblem}, we will see how degrees of freedom relates to sample size.
...
\begin{description}
\setlength{\itemsep}{0mm}
\item[Independent.] Observations can be considered independent when the data are collected from a \emph{random process}, such as tossing a coin, or from a \emph{random sample} from a large population.\footnote{Recall from Chapter~\ref{ch_inference_for_props} that when sampling without replacement, the observations can be considered independent when sampling less than 10\% of the population.  However, with a random sample of greater than 10\% of the population the assumption of independence leads to more conservative estimates for the standard error and therefore our \mbox{$t$-procedures} are still reasonable.  In this case though, one would want to use a modified formula for the standard error that better reflects the expected error in the point estimate when sampling without replacement.}  Without a random sample or process, the standard error formula would not apply, and it is unclear to what population the inference would apply. 
...
\begin{exercisewrap}\begin{nexercise}Recall that the 95\% confidence interval for the average mercury content in croaker white fish was (0.249, 0.325). Discuss whether the conclusion of the hypothesis test is consistent or inconsistent with the conclusion of the confidence interval.\footnotemark
\end{nexercise}\end{exercisewrap}
...
\end{itemize}
 \inferencestep{Check} Check conditions for the sampling distribution of $\bar{x}_{\text{\emph{diff}}}$ is be nearly normal.\vspace{-1mm}
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\begin{exercisewrap}\begin{nexercise}
Based on the interval calculated previously: (105.2, 166.6), can we say that 95\% of student scores increased between 105.2 and 166.6 points after taking the company's course?  

\index{data!SAT prep company|)}

\end{nexercise}\end{exercisewrap}
\footnotetext{No.  This interval is attempting to capture the \emph{average} increase.  It is not attempting to capture 95\% of the increases.  Looking at Figure~\ref{satImprovementHTDataHistogram}, we see than only a small percent had increases between 015.2 and 166.6.}
...
\end{align*}
The typical error in our estimate of $\mu_1-\mu_2$, the real difference in mean salary that the faculty would offer John vs Jennifer, is \$\johnjenniferSE{}.
\end{nexample}
\end{examplewrap}

We see that the difference of sample means of \$3730 is more than 3 $SE$ above 0, which makes us think that the difference being 0 is unreasonable.  We would like to construct a 95\% confidence interval for the theoretical difference in mean salary that would be offered to John versus Jennifer.  The one piece of information we still need is the degrees of freedom associated with a two-sample $t$-test.  
...
Based on this interval, we are 95\% confident that the true difference in salary that these faculty would offer John vs Jennifer is between \$1,495 and \$6,055, that is, we are 95\% confident that the faculty would offer John between \$1,446 and \$6,014 \emph{more} in salary for a lab manager position.

The results of these studies and others like it are are alarming and disturbing.\footnote{A similar study sent out identical resumes with different names to investigate the importance of perceived race.  Resumes with a name commonly perceived to be for a White person (e.g. Emily) were 50\% more likely to receive a callback than the same resume that with a name commonly perceived to be for a Black person (e.g. Lakisha).  \oiRedirect{textbook-emily-lakisha}{https://www.nber.org/papers/w9873}}   One aspect that makes this bias so difficult to address is that the experiment, as well-designed as it was, cannot send us much signal about \emph{which} faculty are
...
\item[] \quad \ check for strong skew or outliers in either data set.  If none are found, the condition 
...
\begin{exercisewrap}
\begin{nexercise}
Use the data below and a calculator to find the the test statistics and p-value for a one-sided test, testing whether there is evidence that embryonic stem cells (ESCs) help improve heart function for sheep that have experienced a heart attack.\footnotemark
\end{nexercise}
\end{exercisewrap}
