...
Chapters~\ref{foundationsForInference} and~\ref{inferenceForCategoricalData} introduced us to inference for proportions using the normal model, and in Section~\ref{oneWayChiSquare}, we encountered the chi-square distribution, which is useful for working with categorical data with many levels. In this chapter, our focus will be on numerical data, where we will encounter two more distributions: the $t$ distribution (looks a lot like the normal distribution) and the $F$ distribution. Our general approach will be:
\begin{enumerate}
...
\item Apply the hypothesis and confidence interval techniques from Chapter~\ref{foundationsForInference} using the distribution from step 2.
\end{enumerate}
...
The sampling distribution associated with a sample mean or difference of two sample means is, if certain conditions are satisfied, nearly normal.

\begin{termBox}{\tBoxTitle{Central Limit Theorem, informal definition}
The distribution of $\bar{x}$ approaches the normal distribution as $n$ increases.  Generally, if the sample size $n\ge 30$, the distribution $\bar{x}$ will be well-approximated by the normal distribution, even for skewed populations.}
\end{termBox}

However, this becomes more complex when the sample size is small, where \emph{small} here typically means a sample size smaller than 30 observations. For this reason, we'll use a new distribution called the $t$ distribution that will often work for both small and large samples with numerical data.
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{07/figures/tDistConvergeToNormalDist/tDistConvergeToNormalDist}
...
\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{07/figures/tDistDF18LeftTail2Point10/tDistDF18LeftTail2Point10}
...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{07/figures/tDistDF20RightTail1Point65/tDistDF20RightTail1Point65}
...
. Thus, we often just use the sample standard deviation $s$ instead of~$\sigma$. \Add{When we make this substitution, we call the quantity Standard Error (SE) rather than SD.  }\Cut{When the sample size is smaller than 30, we will need to use a method to account for extra uncertainty in the standard error. If the skew condition is not met, a larger sample is needed to compensate for the extra skew. These topics are further discussed in Section~\ref{cltSection}}.
...
\Add{When a population is normal, the distribution of $\bar{x}$ is normal.  Even when the population values are skewed, the normal model for the sample mean tends to be very good when the sample consists of at least 30 independent observations.}\Cut{The normal model for the sample mean tends to be very good when the sample consists of at least 30 independent observations and the population data are not strongly skewed.}The Central Limit Theorem provides the theory that allows us to make this assumption.

\begin{termBox}{\tBoxTitle{Central Limit Theorem, informal definition \color{red} CUT THIS BOX}
...
\begin{termBox}{\tBoxTitle{Central Limit Theorem, informal definition \color{red} ADD THIS BOX}
The distribution of $\bar{x}$ becomes approaches the normal distribution as $n$ increases.  Generally, if the sample size $n\ge 30$, the distribution $\bar{x}$ will be well approximated by the normal distribution, even for skewed populations.}
\end{termBox}
...
\begin{figure}
   \centering
   \includegraphics[width=\textwidth]{05/figures/cltSimulations/cltSimulationsNewest}
...
\begin{figure}[ht]
   \centering
   \includegraphics[height=58mm]{05/figures/pokerProfitsCanApplyNormalToSampMean/pokerProfitsCanApplyNormalToSampMean}
...
150.06 &\leq n \\
 n = 151
\end{align*}
The minimum sample size that meets the condition is 151. We round up because the sample size must be \emph{greater than or equal to} 150.06 and sample size must be an integer.
\end{example}
...
%__________________
\section{Paired data and the matched pairs t-test}
\label{pairedData}
...
$$\text{point estimate} \ \pm\ t^{\star}SE \quad\to\quad 12.76 \ \pm\ 2.00\times 1.67 \quad\to\quad (9.42, 16.10)$$
...
\item Down arrow and choose 8: TInterval.
\item Choose Data if you have all the data or Stats if you have the mean and standard deviation.
\item Let $\mu_0$ be the null or hypothesized value of $\mu_d$.
\begin{itemize}
\item If you choose Data, let List be L1 or the list in which you entered the differences (don't forget to enter the differences!) and let Freq be 1. 
...
\text{S$_x$} &\quad \text{the sample SD of the differences} \\
...
\end{description}
\begin{tabular}{l l}
\text{( $\_$ , $\_$ )} &\quad \text{the confidence interval for the difference} \\
$\hat{p}$ &\quad \text{the sample mean of the differences} \\
\text{n} &\quad \text{the sample size of the differences}
\end{tabular}
\end{enumerate}
}
...
\begin{example}{Use the data from Table~\ref{summaryStatsForSheepHeartDataWhoReceivedMiceESCs} and $df=8$ to evaluate the hypotheses for the ESC experiment described in Guided Practice~\ref{exerciseToEvaluteWhetherESCsAreHelpfulInImprovingHeartFunctionInSheep}.}
...
\begin{exercise}
In Guided Practice~\ref{exerciseToEvaluteWhetherESCsAreHelpfulInImprovingHeartFunctionInSheep}, you found that the point estimate, $\bar{x}_{esc} - \bar{x}_{control} = 7.83$, has a standard error of 1.95. Using $df=8$, create a 99\% confidence interval for the improvement due to ESCs.\footnote{We know the point estimate, 7.83, and the standard error, 1.95. We also verified the conditions for using the $t$ distribution in Guided Practice~\ref{exerciseToEvaluteWhetherESCsAreHelpfulInImprovingHeartFunctionInSheep}. Thus, we only need identify $t^{\star}_8$ to create a 99\% confidence interval: $t^{\star}_{8} = 3.36$. The 99\% confidence interval for the improvement from ESCs is given by
\begin{eqnarray*}
...
$\bar{x_1}$ &\quad \text{the mean of sample 1} \\
$\bar{x_2}$ &\quad \text{the mean of sample 2} \\
\text{S$_x1$} &\quad \text{the SD of sample 1} \\
\text{S$_x2$} &\quad \text{the SD of sample 2} \\
...
\begin{termBox}{\tBoxTitle{TI calculator: Calculating the 1 sample t Confidence Interval}Use \textbf{STAT, TESTS, 2-SampTInt}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
