...
%__________________
%\section{Difference of two means - moved}
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{07/figures/babySmokePlotOfTwoGroupsToExamineSkew/babySmokePlotOfTwoGroupsToExamineSkew}
...
\begin{eqnarray*}
SE = \sqrt{\frac{s_n^2}{n_n} + \frac{s_s^2}{n_s}}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{07/figures/distOfDiffOfSampleMeansForBWOfBabySmokeData/distOfDiffOfSampleMeansForBWOfBabySmokeData}
\end{center}
\end{example}
...
\end{enumerate}
These conditions are required so we can adequately estimate the standard deviation  and so we can ensure the distribution of sample means is nearly normal. However, if the population is known to be nearly normal, the sample mean is always nearly normal (this is a special case of the Central Limit Theorem). If the standard deviation \Add{of the population} is also known, then conditions (2) and (3) are not necessary for those data.
...
The population is nearly normal, the population standard deviation is known, and the heights represent a random sample from a much larger population, satisfying the independence condition. Therefore the sample mean of the heights will follow a nearly normal distribution with mean $\mu_{\bar{x}}=70.43$ inches and standard error $SD_{\bar{x}}=\sigma/\sqrt{n} = 2.73/\sqrt{5}=1.22$ inches.
\end{example}
...
The motivation in Chapter~\ref{foundationsForInference} for requiring a large sample was two-fold. First, a large sample ensures that the sampling distribution of $\bar{x}$ is nearly normal. We will see in Section~\ref{normalityCond} that if the population data are nearly normal, then $\bar{x}$ is also nearly normal regardless of the sample size. The second motivation for a large sample was that we get a better estimate of the standard deviation when using a large sample. The standard deviation estimate will not generally be accurate for smaller sample sizes, and this motivates the introduction of the $t$ distribution, which we introduce in Section~\ref{introducingTheTDistribution}.
...
The second reason we previously required a large sample size was so that we could accurately estimate the standard deviation using the sample data. In the cases where we will use a small sample to calculate the standard deviation, it will be useful to rely on a new distribution for inference calculations: the $t$ distribution. A $t$ distribution, shown as a solid line in Figure~\ref{tDistCompareToNormalDist}, has a bell shape. However, its tails are thicker than the normal model's. This means observations are more likely to fall beyond two standard deviations from the mean than under the normal distribution.\footnote{The standard deviation of the $t$ distribution is actually a little more than 1. However, it is useful to always think of the $t$ distribution as having a standard deviation of 1 in all of our applications.} These extra thick tails are exactly the correction we need to resolve the problem of a poorly estimated standard deviation.

\begin{figure}
\centering
\includegraphics[height=45mm]{07/figures/tDistCompareToNormalDist/tDistCompareToNormalDist}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{07/figures/tDistConvergeToNormalDist/tDistConvergeToNormalDist}
...
\Comment{t table on ap formula sheet has one tail header, with Confidence level C \% footer at bottom.  Edited table to use 3 decimal places and look like ap t table, but couldn't get ``Confidence level C" to be centered at bottom row.}
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{07/figures/tDistDF18LeftTail2Point10/tDistDF18LeftTail2Point10}
...
\begin{figure}
\centering
\includegraphics[width=0.85\textwidth]{07/figures/tDistDF20RightTail1Point65/tDistDF20RightTail1Point65}
...
\begin{termBox}{\tBoxTitle{The t distribution and the SE of a mean \color{red}ADD THIS BOX}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.65\textwidth]{07/figures/rissosDolphin/rissosDolphin.jpg}  \\
\addvspace{2mm}
...
$t^{\star}_{df}$\vspace{1mm}\\\footnotesize Multiplication\\factor for\\$t$ conf. interval}The sample mean \Cut{and estimated standard error are}is computed just as before ($\bar{x} = 4.4$ \Cut{and $SE = s/\sqrt{n} = 0.528$)}. \Add{In place of the standard deviation of $\bar{x}$, we use the standard error of $\bar{x} =s/\sqrt{n} = 0.528$.}
...
\item We look in the $t$ table on page~\pageref{tTableSample}, find the column with \Cut{area totaling 0.05 in the two tails (third column),}\Add{95\% along the bottom row} and then the row with 18 degrees of freedom: $t^{\star}_{18} = 2.10$.
\end{itemize}
...
Looking in the column \Cut{where two tails is 0.100 (for a 90\% confidence interval)}\Add{with with a confidence level of 90\% } and row $df=14$, we identify $t^{\star}_{14} = 1.761$.
\end{example}

\begin{exercise}
Using the results of Exercise~\ref{croakerWhiteFishPacificExerConditions} and Example~\ref{croakerWhiteFishPacificExerSEDFTStar}, compute a 90\% confidence interval for the average mercury content of croaker white fish (Pacific).\footnote{$\bar{x} \ \pm\ t^{\star}_{14} SE \ \to\  0.287 \ \pm\  1.761\times 0.0178\ \to\ (0.256, 0.318)$. We are 90\% confident that the average mercury content of croaker white fish (Pacific) is between 0.256 and 0.318 ppm.}

\index{data!white fish and mercury|)}

\end{exercise}

\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for a mean \color{red} ADD THIS BOX}\vspace{-1mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\item $df = n-1$
\item Find critical value $t^*$ using the t table at row$=n-1$.
\item SE =  $\frac{s}{\sqrt{n}}$
\end{itemize}
\item Evaluate the CI and \textbf{write in the form ( $\_$ , $\_$ )}.
\item \textbf{Interpret the interval}:  ``We are [XX]\% confident that the true average of [...] is between [...] and [...]."
\item State your \textbf{conclusion} to the original question.
\end{enumerate}}
\end{termBox}

\Comment{add full example}
...
150.06 &\leq n \\
 n = 151
\end{align*}
\Cut{This suggests we should choose a sample size of at least 151 employees. }\Add{The minimum sample size that meets the condition is 151.  }We round up because the sample size must be \emph{greater than or equal to} 150.06. \Add{and sample size must be an integer}.
\end{example}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.54\textwidth]{07/figures/satImprovementHTDataHistogram/satImprovementHTDataHistogram}
...
\begin{eqnarray*}
T = \frac{\bar{x} - \text{null value}}{SE} = \frac{135.9 - 100}{82.2/\sqrt{30}} = 2.39 \quad \quad \quad \text{with }df=29
\end{eqnarray*}
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{07/figures/pValueShownForSATHTOfOver100PtGain/pValueShownForSATHTOfOver100PtGain}
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for a mean\color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
...
\item Population is known to be normal OR $n\ge 30$ OR graph of data is approximately symmetric with no outliers, making the assumption that population is normal a reasonable one
 \end{itemize}
\item Calculate the \textbf{test statistic} and $df$.
...
\end{eqnarray*}
for each book. It is important that we always subtract using a consistent order; here Amazon prices are always subtracted from UCLA prices. \Add{If this difference is positive, the UCLA price is higher.  If ths difference is negative, the Amazon price is higher.  If this difference is zero, the two prices are equal.  }A histogram of these differences is shown in Figure~\ref{diffInTextbookPricesS10}. Using differences between paired observations is a common and useful way to analyze paired data.

\begin{figure}
\centering
\includegraphics[width=0.83\textwidth]{07/figures/textbooksS10/diffInTextbookPricesS10}
...
\Comment{changed to one sided to test to match up with stated conclusion}.

\begin{example}{Set up and implement a hypothesis test to determine whether\Cut, on average, there is a difference between Amazon's price for a book and the UCLA bookstore's price.} \Add{there is evidence that, on average, Amazon's price for a book is cheaper than the UCLA bookstore's price.}
\label{htForDiffInUCLAAndAmazonTextbookPrices}
\Cut{There are two scenarios: there is no difference or there is some difference in average prices. The \emph{no difference} scenario is always the null hypothesis:
\begin{itemize}
\setlength{\itemsep}{0mm}
...
\Add{There are two scenarios: Amazon's price is cheaper, on average, or Amazon's price is the same (or greater), on average.  The \emph{no difference} scenario is always the null hypothesis:
\begin{itemize}
\setlength{\itemsep}{0mm}
\item[$H_0$:] $\mu_{diff}=0$. There is no difference in the average textbook price.
\item[$H_A$:] $\mu_{diff} > 0$. Amazon's price is cheaper, on average.
\end{itemize}
}
\Cut{
Can the normal model be used to describe the sampling distribution of $\bar{x}_{diff}$? We must check that the differences meet the conditions established in Chapter~\ref{foundationsForInference}. 
}
\Add{The standard deviation of all of the differences in unknown, so we will use the standard deviation of the sample differences.  }The observations are based on a simple random sample from less than 10\% of all books sold at the bookstore, so independence is reasonable; the distribution of differences, shown in Figure~\ref{diffInTextbookPricesS10}, is strongly skewed, but this amount of skew is reasonable for this sized data set ($n=73$). Because all three conditions are reasonably satisfied, we can conclude the \Cut{sampling distribution of $\bar{x}_{diff}$ is nearly normal and our estimate of the standard error will be reasonable.} \Add{t test is reasonable.}
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{07/figures/textbooksS10/textbooksS10HTTails}
...
\Comment{added the calculator values}.

To find the tail areas, we compute the test statistic, which is the \Cut{Z}\Add{t} score of $\bar{x}_{diff}$ under the null condition that the actual mean difference is 0:
$$t = \frac{\bar{x}_{diff} - 0}{SE_{x_{diff}}} = \frac{12.76 - 0}{1.67} = 7.59  \quad \quad \quad df=72$$
This t score is so large it isn't even in the table, which ensures the single tail area will be 0.0002 or smaller. \Add{A calculator gives a tail area as $4.5\times 10^{-11}$.}Since the p-value corresponds to both tails in this case and the $t$ distribution is symmetric, the p-value can be estimated as twice the one-tail area:
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for paired data\color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
...
$$\text{t} = \frac{\text{point estimate} - \text{null value}}{\text{SE of estimate}}$$
\begin{itemize}
\item point estimate = $\bar{x}_{diff}$
\item SE = $\frac{s_{diff}}{\sqrt{n_{diff}}}$
\item $df=n_{diff}-1$
\end{itemize}
\item Find the \textbf{p-value} and compare it to $\alpha$.
\item State the \textbf{conclusion}.  
...
\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for paired data\color{red} ADD THIS BOX}\vspace{-1mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the CI} being used.
\begin{itemize}
...
\item point estimate = $\bar{x}_{diff}$
\item $df=n_{diff}-1$
\item Find critical value $t^*$ using the t table at row $n_{diff}-1$.
\item SE =  $\frac{s_{diff}}{\sqrt{n_{diff}}}$
\end{itemize}
\item Evaluate the CI and \textbf{write in the form ( $\_$ , $\_$ )}.
\item \textbf{Interpret the interval}:  ``We are [XX]\% confident that the true mean of the difference in [...] is between [...] and [...]."
\item State your \textbf{conclusion} to the original question.
\end{enumerate}}
\end{termBox}

\Comment{add a full example}

\subsection{Calculator: The matched pairs t test and CI}

\begin{termBox}{\tBoxTitle{TI calculator: Carrying out the matched pairs t test}Use \textbf{STAT, TESTS, T-Test}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
\item Down arrow and choose 2: T-Test.
...
\begin{termBox}{\tBoxTitle{TI calculator: Calculating the matched pairs t Confidence Interval}Use \textbf{STAT, TESTS, TInterval}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
\item Down arrow and choose 8: TInterval.
...
We would like to \Cut{estimate} \Add{know} the average difference in run times for men and women using the 2009 Cherry Blossom Run data.  Table~\ref{cherryBlossomRun2009SampleOf180SummaryStats} presents relevant summary statistics, and box plots of each sample are shown in Figure~\ref{cbrRunTimesMenWomen}.

\Comment{replace statistics with parameter for this section}
\begin{table}[h]
\centering
\begin{tabular}{l rr}
\hline
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{07/figures/cbrRunTimesMenWomen/cbrRunTimesMenWomen}
\caption{Side-by-side box plots for the sample of 2009 Cherry Blossom Run participants.}
\label{cbrRunTimesMenWomen}
\end{figure}

The two populations (men and women) are independent of one-another, so the data are not paired. Instead a point estimate of the difference in average 10 mile times for men and women, $\mu_{w-m}$, can be found using the formulas from Chapter 2.
\begin{align*}
\mu_{w-m} &=\mu_{w} - \mu_{x}\\
&= 102.13 - 87.65 \\
&=14.48
...
We can quantify the variability in the point estimate, $\bar{x}_{w} - \bar{x}_{m}$, using the following formula for its standard deviation:
\index{standard error!difference in means}
\begin{align*}
SD_{\bar{x}_{w} - \bar{x}_{m}}
    &= \sqrt{(SD_{\bar{x}_{w}})^2 +(SD_{\bar{x}_{m}})^2 } \\
    &= \sqrt{(\frac{\sigma_{\bar{x}_{w}}}{\sqrt{n_w}})^2 +(\frac{\sigma_{\bar{x}_{m}}}{\sqrt{n_m}})^2 } \\
...
\end{eqnarray}
Because we will use the $t$ distribution, we will need to identify the appropriate degrees of freedom. This can be done using \Add{a calculator or }computer software. An alternative technique is to use the smaller of $n_1 - 1$ and $n_2 - 1$.\Cut{ which is the method we will apply in the examples and exercises.}\footnote{This technique for degrees of freedom is conservative with respect to a Type 1 Error; it is more difficult to reject the null hypothesis using this $df$ method.}
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{07/figures/pValueOfTwoTailAreaOfExamVersionsWhereDFIs26/pValueOfTwoTailAreaOfExamVersionsWhereDFIs26}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{07/figures/stemCellTherapyForHearts/stemCellTherapyForHearts}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{07/figures/stemCellTherapyForHeartsPValue/stemCellTherapyForHeartsPValue}
...
\begin{termBox}{\tBoxTitle[]{Hypothesis test for the difference of two means\color{red} ADD THIS BOX}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the test} being used.  
\begin{itemize}
...
\begin{termBox}{\tBoxTitle[]{Constructing a confidence interval for the difference of two means \color{red} ADD THIS BOX}\vspace{-1mm}
\begin{enumerate}
\setlength{\itemsep}{0mm}
\item State the \textbf{name of the CI} being used.
\begin{itemize}
...
\item point estimate = $\bar{x}_1-\bar{x}_2$
\item $df \to$ get from calculator.
\item Find critical value $t^*$ using the t table at row = $df$.
\item SE =  $\sqrt{\frac{s^2_1}{n_1}+\frac{s^2_2}{n_2}}$
\end{itemize}
\item Evaluate the CI and \textbf{write in the form ( $\_$ , $\_$ )}.
\item \textbf{Interpret the interval}:  ``We are [XX]\% confident that the true difference in the mean of [...] is between [...] and [...]."
\item State your \textbf{conclusion} to the original question.
\end{enumerate}}
\end{termBox}
...
\begin{termBox}{\tBoxTitle{TI calculator: Carrying out the 2 sample t test}Use \textbf{STAT, TESTS, 2-SampTTest}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
\item Choose 4: 2-SampTTest
\item Choose Data if you have all the data or Stats if you have the means and standard deviations.
\begin{itemize}
...
$\bar{x}_1$ &\quad \text{the mean of sample 1} \\
$\bar{x}_2$ &\quad \text{the mean of sample 2} \\
\text{Sx1} &\quad \text{the SD of sample 1} \\
\text{Sx2} &\quad \text{the SD of sample 2} \\
\text{n1} &\quad \text{the size of sampel 1} \\
\text{n2} &\quad \text{the size of sampel 2} \\
\end{tabular}
\end{enumerate}
...
\begin{termBox}{\tBoxTitle{TI calculator: Calculating the 2 sample t Confidence Interval}Use \textbf{STAT, TESTS, 2-SampTInt}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
...
\begin{exercise}Use the data from the ESC example and a calculator to construct a 90\% confidence interval.\footnote{The interval is $(4.3543, 11.307)$ with $df=12.2$.}
\end{exercise}

\begin{exercise}Use the data from the ESC example and a calculator to find the t statistic, degrees of freedom, and p-value of a two-sided hypothesis test.\footnote{$t=4.008$, $df=12.2$, and p-value$=0.00168$.}
\end{exercise}

\Comment{deleted subsection ``Pooled standard deviation estimate (special topic)"}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.65\textwidth]{07/figures/toyANOVA/toyANOVA}
...
\begin{figure}
\centering
\includegraphics[width=0.85\textwidth]{07/figures/mlbANOVA/mlbANOVABoxPlot}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{07/figures/fDist3And323/fDist3And323}
...
The test statistic for the baseball example is $F=1.994$. Shade the area corresponding to the p-value in Figure~\ref{fDist3And323}. \footnote{\ \vspace{-4mm}\\\includegraphics[height=20mm]{07/figures/fDist3And323/fDist3And323Shaded}}
\end{exercise}
...
\begin{figure}[hhh]
\centering
\includegraphics[width=\textwidth]{07/figures/mlbANOVA/mlbANOVADiagNormalityGroups}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{07/figures/classData/classDataSBSBoxPlot}
