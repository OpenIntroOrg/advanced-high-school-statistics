...
\begin{figure}
   \centering
   \includegraphics[width=0.6\textwidth]{08/figures/perfLinearModel/perfLinearModel}
...
\begin{figure}
   \centering
   \includegraphics[width=0.85\textwidth]{07/figures/scattHeadLTotalL/scattHeadLTotalL}
...
\begin{figure}
   \centering
   \includegraphics[width=0.8\textwidth]{07/figures/possumPic/possumPic}
...
\begin{figure}
   \centering
   \includegraphics[width=0.8\textwidth]{07/figures/scattHeadLTotalLLine/scattHeadLTotalLLine}
...
\begin{figure}
   \centering
   \includegraphics[width=0.73\textwidth]{07/figures/scattHeadLTotalLResidualPlot/scattHeadLTotalLResidualPlot}
...
\begin{figure}
   \centering
   \includegraphics[width=\textwidth]{07/figures/sampleLinesAndResPlots/sampleLinesAndResPlots}
...
\Comment{addeded word coefficient and used lower case r to correspond to ap formula sheet}

\begin{termBox}{\tBoxTitle{Correlation Coefficient: strength of a linear relationship}
\termsub{Correlation}{correlation}, which always takes values between -1 and 1, describes the strength of the linear relationship between two variables. We denote the correlation coefficient by $r$.}
...
\Comment{replace e's with y-yhat}
...
, respectively.
\item If $\bar{x}$ is the mean of the horizontal variable (from the data) and $\bar{y}$ is the mean of the vertical variable, then the point $(\bar{x}, \bar{y})$ is on the least squares line.  \Add{Plugging this point in for $x$ and $y$ in the least squares equation and solving for $b_0$ gives
\begin{align*}
\bar{y} &= b_0 + b_1\bar{x}
\end{align*}
\begin{eqnarray}
...
\Comment{added solving for y-intercept}

\begin{exercise} \label{findingTheSlopeOfTheLSRLineForIncomeAndAid}
Using the summary statistics in Table~\ref{summaryStatsOfSATGPAData}, compute the slope \Add{and $y$-intercept} for the regression line of gift aid against family income.\footnote{Apply Equation~\eqref{slopeOfLSRLine} with the summary statistics from Table~\ref{summaryStatsOfSATGPAData} to compute the slope:
\begin{eqnarray*}
b_1 &= r\frac{s_y}{s_x} = (-0.499)\frac{5.46}{63.2} = -0.0431 \\
b_0 &  = \bar{y} - b_1\bar{x} = 19.94 - (-0.043)(101.8) = 24.2744
...
\Comment{edited termBox a bit}

\begin{termBox}{\tBoxTitle{Interpreting parameters estimated by least squares}
\begin{itemize}
\item The slope, $b1$, describes the estimated difference in the $y$ variable if the explanatory variable $x$ for a case happened to be one unit larger. 
...
\begin{termBox}{\tBoxTitle{TI calculator: finding $b_0$, $b_1$, $R^2$, and $r$ for a linear model}Use \textbf{STAT, CALC, LinReg(a + bx)}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to CALC.
\item Down arrow and choose 8: LinReg(a+bx).  \newline (Caution: choosing 4:LinReg(ax+b) will reverse $a$ and $b$.)
\item Let Xlist be L1 and Ylist be L2 (don't forget to enter the $x$ and $y$ values in L1 and L2 before doing this calculation).  
\item Let FreqList be 1.
\item Leave Store RegEQ blank.
\item Choose Calculate and hit ENTER.
\begin{description}
\item[] This returns:
\end{description}
\begin{tabular}{l l}
a & \quad $b_0$, the y-intercept of the best fit line \\
b & \quad $b_1$, the slope of the best fit line \\
r$^2$ & \quad \text{$R^2$, the explained variance} \\
r & \quad$r$, the correlation coefficient
\end{tabular}
\begin{itemize}
\item[TI-83: ] Do steps 1 - 3, then enter the $x$ list and $y$ list separated by a comma, e.g. LinReg(a+bx) L1, L2, then hit ENTER.  
\end{itemize}
\end{enumerate}
}
\end{termBox} 

\begin{tipBox}{\tipBoxTitle{What to do if $r^2$ and $r$ do not show up on a TI calculator}If $r^2$ and $r$ do now show up when doing STAT, CALC, LinReg, the \emph{diagnostics} must be turned on.  This only needs to be once and the diagnostics will remain on.
\begin{enumerate}
\item Hit 2ND 0 (i.e. CATALOG).
\item Scroll down until the arrow points at DiagnosticOn
\item Hit ENTER and ENTER again.
\begin{description}
\item[] The screen should now say: 
\end{description}
\begin{tabular}{l l}
DiagnosticOn & \\
& Done \\
...
\begin{tipBox}{\tipBoxTitle{What to do if a TI calculator returns: ERR: DIM MISMATCH }This error means that the lists, generally L1 and L2, do not have the same length.
\begin{enumerate}
\item Choose 1: Quit
\item Choose STAT, Edit and make sure that the lists have the same number of entries.
\end{enumerate}
}
\end{tipBox} 
...
\begin{exercise}The table above contains values of federal spending per capita (rounded to the nearand percent of population in poverty for seven counties.  This is a a subset of the ~\ref{countyDF} data set from Chapter 1.  Use the table a calculator to find the equation of the least squares regression line for this partial data set.\footnote{$a=5.136$ and $b=1.056$, therefore $\hat{y}=5.136 + 1.056x$.}
\end{exercise}

\subsection{Categorical predictors with two levels (special topic)}
\Comment{made section special topic}
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{07/figures/marioKartNewUsed/marioKartNewUsed}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{07/figures/outlierPlots/outlierPlots}
...
%__________________
\section{Inference for linear regression: t-test for the slope of regression line}
...
\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{07/figures/unemploymentAndChangeInHouse/unemploymentAndChangeInHouse}
...
\begin{figure}
\centering
\includegraphics[width=0.82\textwidth]{07/figures/oneSidedTailForMidtermUnemploymentHT/oneSidedTailForMidtermUnemploymentHT}
...
\begin{termBox}{\tBoxTitle{TI calculator: The Linear Regression t-Test on $\beta_1$}Use \textbf{STAT, TESTS, LinRegTTest}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
\item Down arrow and choose F: LinRegTest. (On TI-83 it is E: LinRegTTest).
\item Let Xlist be L1 and Ylist be L2 (don't forget to enter the $x$ and $y$ values in L1 and L2 before doing this test).  
\item Let Freq be 1.
\item Choose $\ne$, $<$, or $>$ to correspond to H$_A$.
\item Leave RegEQ blank.
\item Choose Calculate and hit ENTER.
\begin{description}
\item[] This returns:
\end{description}
...
t & \quad t statistic \\
p & \quad \text{p-value} \\
df & \quad \text{degrees of freedom associated with this test} \\
a & \quad $b_0$, the y-intercept of the best fit line of the sample data\\
b & \quad $b_1$, the slope of the best fit line of the sample data \\
s & \quad \text{the standard deviation of the regression line} \\
r$^2$ & \quad \text{$R^2$, the explained variance} \\
r & \quad$r$, the correlation coefficient\\
\end{tabular}
\end{enumerate}
...
\begin{termBox}{\tBoxTitle{TI calculator: Calculating a Confidence Interval for $\beta_1$}Use \textbf{STAT, TESTS, LinRegTInt}.
\begin{enumerate}
\item Choose STAT.
\item Right arrow to TESTS.
\item Down arrow and choose G: LinRegTest. (This test is not built into the TI-83).
\item Let Xlist be L1 and Ylist be L2 (don't forget to enter the $x$ and $y$ values in L1 and L2 before doing this interval).  
\item Let Freq be 1.
\item Enter the desired confidence level.
\item Leave RegEQ blank.
\item Choose Caculate and hit ENTER.
\begin{description}
\item[] This returns:
\end{description}
\begin{tabular}{l l}
( $\_$ , $\_$ )& \quad the confidence interval \\
b & \quad \text{$b_1$, the slope of best fit line of the sample data} \\
df & \quad \text{degrees of freedom associated with this test} \\
s & \quad \text{the standard deviation of the regression line} \\
a & \quad $b_0$, the y-intercept of the best fit line of the sample data \\
r$^2$ & \quad \text{$R^2$, the explained variance} \\
...
\Comment{deleted subsection ``an alternative test statistic"}
